...
class EmbeddedNetwork extends ExprAndTypePrinter {
	...
	def String printDim(String name, Type type, List<Integer> dimensions, int i, List<String> index){
		val j = i + 1
		var List<Integer> newDimension = new ArrayList<Integer>(dimensions)
		newDimension.remove(0)
		index.add("i" + i)
		
		'''
			for(int i«i»=0; i«i»<«dimensions.get(0)»; i«i»++){
				«name»«FOR idx : index»[«idx»]«ENDFOR» = new «type.doSwitch»«FOR k : 1 .. newDimension.size - 1»*«ENDFOR»[«newDimension.get(0)»];
				«IF newDimension.size > 1»«printDim(name, type, newDimension,j,index)»«ENDIF»
			}	
		'''
	}
		...
		'''
			...
			int main(int argc, char *argv[]){
				...
				«IF network.hasAttribute("network_shared_variables")»
					...
					«FOR v : network.getAttribute("network_shared_variables").objectValue as Set<Var>»
						«v.type.doSwitch» «IF v.type.list»«FOR dim : v.type.dimensions»*«ENDFOR»«ELSE»*«ENDIF»«v.name» = new «v.type.doSwitch»«IF v.type.list»«FOR i : 1 .. (v.type.dimensions.size - 1)»*«ENDFOR»«»[«v.type.dimensions.get(0)»]«ENDIF»;
						«printDim(v.name, v.type, v.type.dimensions,0,new ArrayList<String>())»
					«ENDFOR»
				«ENDIF»
				
				// Actors
				«FOR actor : network.children.filter(typeof(Actor))»
					«actor.name» *act_«actor.name» = new «actor.name»(«IF actor.hasAttribute("actor_shared_variables")»«FOR v : actor.getAttribute("actor_shared_variables").objectValue as Set<Var> SEPARATOR ", "»«v.name»«ENDFOR»«ENDIF»«FOR variable : actor.parameters SEPARATOR ", "»«variable.initialValue.doSwitch»«ENDFOR»);
				«ENDFOR»
