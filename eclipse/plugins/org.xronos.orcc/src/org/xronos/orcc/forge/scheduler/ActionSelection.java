...
public class ActionSelection extends DfVisitor<List<Block>> {
	...
	public class InputPatternBlocks extends DfVisitor<List<Block>> {
	...
	public class OutputPatternBlocks extends DfVisitor<List<Block>> {
		@Override
		public List<Block> casePattern(Pattern pattern) {
			List<Block> blocks = new ArrayList<Block>();
			for (Port port : pattern.getPorts()) {
				if (pattern.getNumTokens(port) > 1) {
					BlockBasic blockBasic = IrFactory.eINSTANCE
							.createBlockBasic();
					Var target = null;
					if (scheduler.getLocal(port.getName() + "PortStatus") != null) {
						target = scheduler.getLocal(port.getName()
								+ "PortStatus");
					} else {
					...
					if (scheduler.getLocal(port.getName() + "Portvalue") != null) {
						...
						target = IrFactory.eINSTANCE.createVar(type,
								port.getName() + "Portvalue", true, 0);
						scheduler.addLocal(target);
					}
					...
					Expression E2 = IrFactory.eINSTANCE.createExprInt(pattern
							.getNumTokens(port));
					Expression condition = IrFactory.eINSTANCE
							.createExprBinary(E1, OpBinary.LE, E2,
									IrFactory.eINSTANCE.createTypeBool());

					// -- Create While
					BlockWhile blockWhile = IrFactory.eINSTANCE
							.createBlockWhile();
	...
	public List<Block> caseAction(Action action) {
		...
		if (action.getOutputPattern() != null) {
			if (action.getOutputPattern().getPorts().size() == 1) {
				blocks.addAll(new OutputPatternBlocks().doSwitch(action
						.getOutputPattern()));
			} else {
				BlockMutex mutex = XronosIrFactory.eINSTANCE.createBlockMutex();
				mutex.getBlocks().addAll(
						new OutputPatternBlocks().doSwitch(action
								.getOutputPattern()));
				blocks.add(mutex);
			}
		}
