...
package org.xronos.openforge.forge.library.pipe;
...
import org.xronos.openforge.forge.api.timing.Timing;
 ...
 * <code>IPCore</code> which contains a 15 deep, programmable width,
 ...
 * all Xilinx FPGA families that implement LUT dual port ram.
 * 
 * @see org.xronos.openforge.forge.api.ipcore.IPCore
 */
public class PetiteMultiClockPipe implements HDLWriter {
	...
	/**
	 * Creates a new <code>PetiteMultiClockPipe</code> instance with 32 bit
	 * width and the GLOBAL Clock used for the write and read side. This
	 * configuration produces functional but not optimal hardware since the read
	 * and write clocks are identical. This configuration is supplied for
	 * initial coding, testing, and the unique situations that arise during
	 * development but is not recommended for production designs.
	 */
	...
	/**
	 * Creates a new <code>PetiteMultiClockPipe</code> instance with the
	 * supplied data path size with the read and write clocks associated with
	 * the supplied read and write <code>EntryMethod</code>s.
	 * 
	 * @param dataPathSize
	 *            an <code>int</code> representing the width of the data path.
	 *            Can be any value from 1 through 64.
	 * @param writeSide
	 ...
	 *            from.
	 */
	public PetiteMultiClockPipe(int dataPathSize, EntryMethod writeSide,
	 ...
	 *            pipe.
	 */
	public PetiteMultiClockPipe(int dataPathSize, ClockPin writeClock,
			ClockPin readClock) {
		// verify dataPathSize is 1,8,16,32, or 64. Throw
		// apiexception if not.
		if ((dataPathSize < 1) || (dataPathSize > 64)) {
			throw new org.xronos.openforge.forge.api.ForgeApiException(
					"Illegal data path size to PetiteMultiClockPipe: "
							+ dataPathSize + ", expected 1 through 64");
		}
	...
	/**
	 * Retrieves 1/4 th full count estimate, i.e. the 2 bit number when
	 * multiplied by 1/4 the fifo depth that estimates how much data is
	 ...
	 * to go from 3/4 th full to empty in a single clock for example. Due to the
	 * way this value is calculated, care must be taken to not rely absolutely
	 * on the value, for example, when writing bursts of 4 words, checking for
	 * 3/4 full might cause an overflow, but checking for 2/4 or less and
	 * writing 4 words subsequent will always work. The rule of thumb is to not
	 * write more than 1/2 the distance to full from the current count estimate.
	 * Therefore if the fifo is registering 1/2 full, don't write more than 1/4
	 * the depth. This method should only be accessed by code referenced to the
	 * write clock domain.
	 * 
	 * The decoding of this value is:
	 * 
	 * 11 - 3/4 full 10 - 1/2 full 01 - 1/4 full 00 - <1/4 full
	 * 
	 * @return an <code>int</code> value indicating the 1/4 th full status of
	 *         the internal fifo.
	 */
	 ...
	 * read side could appear to jump from 1/4 th full to 3/4 full in a single
	 * clock for example. Due to the way this value is calculated, care must be
	 * taken to not rely absolutely on the value, for example, when reading
	 * bursts of 4 words, checking for 1/4 full might cause an underflow, but
	 * checking for 1/2 or more and reading 4 words subsequent will always work.
	 * The rule of thumb is to not read more than 1/2 the distance to empty from
	 * the current count estimate. Therefore if the fifo is registering 1/2
	 * full, don't read more than 1/4 the depth. This method should only be
	 * accessed by code referenced to the read clock domain.
	 * 
	 * The decoding of this value is:
	 * 
	 * 11 - 3/3 full 10 - 1/2 full 01 - 1/4 full 00 - <1/4 full
	 * 
	 * @return an <code>int</code> value indicating the 1/4 th full status of
	 *         the internal fifo.
	 */
	...
	private void writeLutRams(PrintWriter pw, int dataSize) {
		...
		for (int ram = 0; ram < dataSize; ram++) {
			pw.println("   RAM16X1D bram" + (ram + 1)
					+ " ( .DPRA0(read_addr[0]),");
			...
			// state machine expects synchronous read, so clock the
			// read output of the memory. This improves timing too.
			if (dataSize == 1) {
				pw.println("  FDE fdbram"
						+ (ram + 1)
						+ " ( .C(read_clock), .CE(read_allow), .D(int_read_data), .Q(read_data);");
			} else {
				pw.println("  FDE fdbram"
						+ (ram + 1)
						+ " ( .C(read_clock), .CE(read_allow), .D(int_read_data["
						+ ram + "]), .Q(read_data[" + ram + "]));");
			}
