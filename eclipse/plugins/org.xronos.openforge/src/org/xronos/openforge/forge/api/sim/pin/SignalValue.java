...
/**
 * SignalValue is used to describe signal, which may be Z, X, or an integer
 * value. For the test side, X equates to a don't care.
 */
public class SignalValue {
	...
	private SignalValue(boolean isX, boolean isZ, boolean isDC) {
		...
		if ((isX) && (isZ)) {
			...
			throw new IllegalArgumentException(
					"SignalValue can't be X, & Z at the same time!");
		}
	}
	...
	/**
	 * Get the long value contained by this SignalValue. Undefined if X or Z
	 * 
	 * @return value
	 */
	 ...
	 * 
	 * @param v
	 *            other SignalValue to test against
	...
	/**
	 * Parse a String representation of a SignalValue. Legal values are x, z,
	 * 0xNNNN (hex), 0bNNNN (binary), NNNN (decimal); MMM.NNN (Double), MMM.NNNd
	 * (Double), MMM.NNNf (Float). Case is irrelevent.
	 * 
	 * @param s
		...
		while (st.hasMoreTokens()) {
			...
			} else if (tok.indexOf('.') >= 0) // does it have a decimal point?
			{
				...
				} else // a double then
				{
					...
					{
						tok = tok.substring(0, tok.length() - 1); // remove
																	// it...
					}
