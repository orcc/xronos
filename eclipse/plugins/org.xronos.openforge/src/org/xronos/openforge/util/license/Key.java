...
public final class Key {
	 ...
	 * 
	 * 1. Input 4 digit year, 2 digit month and week of month (1-4) for
	 * expiration; use, September 1, 3949 as perpetual number which requires the
	 * keygen program to input 8/30/3949 as the expiration date 2. Input 5 digit
	 * serial number and 1 digit feature code. 3. Build month/week code by
	 * multiplying the week by the month creating a 2 digit result. 4. Extract
	 * the decimal digits from all numbers input and organize into an array
	 * according to the following locations chart:
	 * 
	 * Key digit locations:
	 * 
	 * For multi digit numbers, digit 0 is the msd, so we process left to right
	 * 
	 * 0 = CRC1 - 0 1 = CRC2 - 1 2 = Year - 0 3 = Serial - 3 4 = Feature Code 5
	 * = Year - 2 6 = Serial - 4 7 = Month - 1 8 = Year - 3 9 = Month - 0 10 =
	 * Serial - 1 11 = Serial - 0 12 = CRC2 - 0 13 = CRC1 - 1 14 = Year - 1 15 =
	 * Serial - 2
	 * 
	 * 5. Scramble the array with the modulo array, do this by adding each
	 * element of the modulo array to the key array and wrap modulo 10. 6.
	 * Caclulate the CRC2 digits by summing up the digits and extracting the
	 * bottom two decimal digits of the sum and loading them into the CRC2
	 * locations 7. Calculate the CRC1 digits by xoring all the even key digits
	 * and all the odd ones separately, and storing the 2 digit result in the
	 * two CRC1 locations.
	 * 
	 * To recover a key:
	 * 
	 * 1. During Installation, perform the CRC1 calculation of the given key and
	 * if the the result is 0, then the value is valid, unless all the input
	 * numbers are 0. 2. Perform the CRC2 summing, ignoring the CRC1 and CRC2
	 * locations and compare the result against the CRC2 digits in the key, if
	 * they don't match, its invalid 3. Perform unscrambling using the modulo
	 * array by inverting each location and adding it to the key and use the
	 * modulo10 routine. 4. Extract the date, serial number and feature code by
	 * rearranging the digits. 5. Turn the Month/week into Month/day by
	 * assigning week 1 to be day 1, week 2 to be day 8, week 3 to be day 15,
	 * and week 4 to be day 22.
	 * 
	 * You can now check the recovered data for perpetual or expiration Data
	 * information.
	 * 
	 * Keep in mind that months are 1-12 and weeks are 1-4, so we don't get any
	 * * 0!
	 */
	...
	public static final int[] modulo = { 0, 0, 3, 4, 1, -2, -3, 1, -2, 4, -5,
			6, 0, 0, 2, -2 };

	/*
	 * This method determines if the current license key is valid and if the
	 * product is still licensed to run by checking the key expiration time.
	 */
	public static boolean isLicenseCurrent(String key) {
		// Parse and validate license, then check if the current data is after
		// the
	...
	private static boolean isCRC1Valid(String key) {
		...
		for (int i = 0; i < 16; i = i + 2) {
			// System.out.println("isCRC1Valid: keyints[" + i + "] = " +
			// keyints[i]);
			// System.out.println("isCRC1Valid: keyints[" + (i + 1) + "] = " +
			// keyints[i + 1]);
		...
		// System.out.println("isCRC1Valid: crctop sum: " + crctop +
		// ", crcbot sum: " + crcbot);
	...
	public static String parseKey(String key) {
		...
		// just the characters representing decimal digits in the first line of
		// the file.
		String result = "";
