...
class ComponentNode extends Record {
	...
	protected String getBodyLabel() {
		...
		labelBuf.append("\\n");
		labelBuf.append(org.xronos.openforge.util.naming.ID.glob(component));
	...
	protected void graphPorts(Record.Port boundingBox) {
		if (needPortGraph()) {
			...
			if (DO_CLKRESET) {
				org.xronos.openforge.lim.Port reset = component.getResetPort();
				if (reset.isConnected()) {
					graphPort(reset, entryBox, "reset", "R");
				}
				org.xronos.openforge.lim.Port clock = component.getClockPort();
