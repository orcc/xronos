...
public class NumericPromotionOp extends UnaryOp implements Emulatable {
	 ...
	 * 
	 * @param size
	 *            , the number of bits of the output (the promoted size).
	 * @param signed
	 *            a boolean, true if the promoted value (output) is a signed
	 *            value.
	 */
	 ...
	 * 
	 * @return an int, the number of bits of the type being promoted to.
	 */
	...
	public boolean pushValuesBackward() {
		...
		Value newPushBackValue = new Value(Math.min(getResultBus().getSize(),
				getDataPort().getSize()), getDataPort().getValue().isSigned());
	...
	/**
	 * Retrieves the correct numeric promotion op for the given types. Types may
	 * be one of 'byte', 'char', 'short', 'int', 'long', 'float', or 'double'.
	 * 
	 * @param inType
	 ...
	 */
	public static NumericPromotionOp getNumericPromotionOp(String inType,
			String outType) {
		final String b = "byte";
		final String c = "char";
		...
		if (inType.equals(b) && outType.equals(f))
			...
			if (outType.equals(b))
				...
				size = 16;
			else if (outType.equals(i))
				size = 32;
			else if (outType.equals(l))
				size = 64;
	...
	private static class FloatNumericPromotionOp extends
			FloatTypeNumericPromotionOp {
		public FloatNumericPromotionOp() {
			super(32);
		}
	}

	private static class DoubleNumericPromotionOp extends
			FloatTypeNumericPromotionOp {
		public DoubleNumericPromotionOp() {
			super(64);
		}
	}

	private static final class NumericPromoteB2F extends
			FloatNumericPromotionOp {
	}

	private static final class NumericPromoteC2F extends
			FloatNumericPromotionOp {
	}

	private static final class NumericPromoteS2F extends
			FloatNumericPromotionOp {
	}

	private static final class NumericPromoteI2F extends
			FloatNumericPromotionOp {
	}

	private static final class NumericPromoteL2F extends
			FloatNumericPromotionOp {
	}

	private static final class NumericPromoteB2D extends
			DoubleNumericPromotionOp {
	}

	private static final class NumericPromoteC2D extends
			DoubleNumericPromotionOp {
	}

	private static final class NumericPromoteS2D extends
			DoubleNumericPromotionOp {
	}

	private static final class NumericPromoteI2D extends
			DoubleNumericPromotionOp {
	}

	private static final class NumericPromoteL2D extends
			DoubleNumericPromotionOp {
	}

	private static final class NumericPromoteF2D extends
			DoubleNumericPromotionOp {
	}
}
