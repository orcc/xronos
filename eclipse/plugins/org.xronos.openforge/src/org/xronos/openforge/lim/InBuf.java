...
/**
 * An InBuf is used to bring a structural flow to the inside of a {@link Module}
 * from its outside. A single InBuf is created automatically for each
 * {@link Module}, and adding a data {@link Port} to the {@link Module} also
 * adds a corresponding {@link Bus} to the InBuf. The InBuf itself has no
 * {@link Port Ports} and no {@link Entry Entries}. Logically, the {@link Port
 * Ports} of the {@link Module} are the continued internally by the InBuf's
 * {@link Bus Buses}.
 * <P>
 * The InBuf's single {@link Exit} is created with the usual done {@link Bus}
 * and at least two data {@link Bus Buses}. The done {@link Bus} is the
 * continuation of the {@link Module Module's} go {@link Port}; that is why it
 * is also known as the "go {@link Bus}" and can be retrieved with the method
 * {@link InBuf#getGoBus()}. The {@link Module Module's} clock and reset
 * {@link Port} signals are represented by the first two data {@link Bus Buses};
 * this is because this is the only case in which a clock or a reset will exit a
 * {@link Component}. In addition to being the first two buses in the list
 * returned by {@link Exit#getDataBuses()}, these buses can also be obtained
 * with the methods {@link InBuf#getClockBus()} and {@link InBuf#getResetBus()},
 * respectively. The remaining data buses carry regular data values; these buses
 * are the only ones returned by the method {@link InBuf#getDataBuses()}.
 * 
...
public class InBuf extends Component implements Emulatable {
	...
	/**
	 * The gate depth of this inbuf. Always 0 except at the periphery of the
	 ...
	 * Gets the data buses.
	 * 
	 * @return a collection of Bus, each of which represents one of the module's
	 *         data ports; note that this list is not the same as that returned
	...
	/**
	 * Updates the physical Port-Bus connections, based on current Structural
	 * Dependencies. InBufs have no Entries of their own, so this is a no-op.
	 */
	...
	/**
	 * Sets the gate depth for this InBuf, called by the Pipeliner to set a
	...
	/**
	 * Propagates {@link Value Values} through this InBuf in the forward
	 * direction.
	 * <P>
	 * Rather than using {@link Bus#pushValueForward(Value)} as usual, this
	 * component propagates each {@link Bit} (or not} itself, according to the
	 * following rules.
	 * <P>
	 * If the owner {@link Module} returns true for {@link Module#isOpaque()},
	 * then only constant {@link Bit Bits} are propagated from the
	 * {@link Module Module's} {@link Port Ports} to the InBuf's {@link Bus
	 * Buses}.
	 * <P>
	 * Otherwise, both constant and "owned" {@link Bus} {@link Bit Bits} will be
	 * propagated.
	 * 
	...
	public boolean pushValuesForward() {
		...
		for (Bus bus : getBuses()) {
			...
			if (pushedValue != null) {
				 ...
				 * since an InBuf is part Module and part Component. Compare
				 * each pair of Bits.
				 */
				for (int i = 0; i < pushedValue.getSize(); i++) {
					final Bit pushedBit = pushedValue.getBit(i);
					final Bit busBit = busValue.getBit(i);
					if (!pushedValue.bitEquals(i, busValue, i)
							&& busBit.isCare()) {
						if (getOwner().isOpaque()) {
							/*
							 * If this InBuf lives in an opaque Module, then we
							 * have to avoid pushing through external non-global
							...
							if (pushedBit.isConstant()) {
								...
								|| !pushedBit.isGlobal()) {
							/*
							 * Otherwise, this InBuf lives in a transparent
	...
	/**
	 * Determines the 'consumed' value for each {@link Bus} of the InBuf and
	...
	/**
	 * Cloning of InBufs is not allowed. They are created as needed in cloned
	 * {@link Module Modules}.
	 * 
