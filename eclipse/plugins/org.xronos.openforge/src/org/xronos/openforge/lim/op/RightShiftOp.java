...
public class RightShiftOp extends ShiftOp implements Emulatable {
	 ...
	 * RightShiftOp according to these rules:
	 * 
	 ...
	 * shift beyond our inputs size, then our output is simply the sign bit of
	 * the input. 2. if we shift within our inputs size, then the MSB bits are
	 * all signed bit of data input value, and the Next 'y' bits are all
	 * passthrough of the top 'y' bits of data input value. Result is signed if
	 * data value is signed.
	 * 
	...
	/**
	 * Reverse constant prop on a RightShiftOp applies only if the shift
