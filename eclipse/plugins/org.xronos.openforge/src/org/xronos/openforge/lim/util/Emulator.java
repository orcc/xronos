...
public abstract class Emulator {
	 ...
	 */
	protected List<ID> componentList = null;

	/*
	 * Collection of unEmulatable components that the module has is cached here
	 * for
	 */
	protected Collection<Component> unEmulatable = new ArrayList<Component>();

	/**
	 * Map of each port to its input bus
	 **/
	protected Map<Port, Bus> inputMap = new HashMap<Port, Bus>();
	...
	/**
	 * Gets the {@link Bus} that drives a given {@link Port}. null is returned
	 * if the component has 0 entries or more than 1 entry except LoopBody for
	 * which the feedback entry is chosen. If the component has more than 1
	 * dependency, null is returned.
	 * 
	 * @param port
	...
	protected Bus getInputBus(Port port) {
		...
		final Component entryOwner = isInBuf(port.getOwner()) ? port.getOwner()
				.getOwner() : port.getOwner();
	...
	/**
	 * Update the inputMap object. For each component in the componentList,
	 * pairs of (input port, data providing bus) are saved in the inputMap
	 * object. This helps later when looking up the bus that provides the data
	 * for a subcomponents port.
	 */
	...
	/**
	 * Get a map of (input bus, value of bus) pairs from a map of (port, value)
	 * pairs
	 * 
	 * @param portMap
	 *            (port, value) pairs
	 ...
	 */
	protected Map<Bus, SizedInteger> portToBusValues(
	...
	/**
	 * Get a map of (Port port of component, SizedInteger value) for the given
	 * component from a map of (Bus bus , SizedInteger value) pairs. The
	 * busValues map may contain other busses that do not provide data to any of
	 * the components ports also.
	 * 
	 * SideEffect: If the component is not emulatable, it gets added to the
	 * unEmulatable Collection.
	 * 
	 * @param component
	 *            component whose input ports have to be mapped to values
	 * @param busValues
	 *            map of (Bus bus,SizedInteger value) pairs
	 * @return map of the components ports to SizedInteger values.
	 */
	protected Map<Port, SizedInteger> busToPortValues(Component component,
			Map<Bus, SizedInteger> busValues) {
		/*
		 * Collect the input values for the component from its input buses.
		 */
		Map<Port, SizedInteger> portValues = new HashMap<Port, SizedInteger>();
		...
		for (Port dataPort : dataPorts) {
			...
			if (dataBus != null) {
				...
				boolean isIOBuf = component instanceof InBuf
						|| component instanceof OutBuf;
				boolean inputUnknown = busValues.get(dataBus) == null;
				boolean emulateable = (component instanceof Emulatable)
						|| isModule;
	 ...
	 */
	protected Collection<Component> getUnEmulatable() {
		return unEmulatable;
	}
	 ...
	 * 
	 * @param unEmulatable
	 *            the new unEmulatable collection
	 */
	protected void setUnEmulatable(Collection<Component> unEmulatable) {
	 ...
	 */
	public List<ID> getComponentList() {
	 ...
	 * 
	 * @param componentList
	 *            new list of subcomponents
	 */
	public void setComponentList(List<ID> componentList) {
