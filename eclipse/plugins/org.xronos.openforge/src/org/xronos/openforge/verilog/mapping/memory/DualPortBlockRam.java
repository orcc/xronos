...
import org.xronos.openforge.verilog.model.Concatenation;
...
import org.xronos.openforge.verilog.pattern.StatementBlock;
...
public abstract class DualPortBlockRam extends DualPortRam {
	...
	public StatementBlock initialize() {
		...
		initial_block
				.add(new InlineComment(
						" Initialization of Dual Port Block ram now done through explicit parameter setting.",
						Comment.SHORT));
	...
	private void initialize(ModuleInstance instance) {
		...
		// This parameter now set in DualPortBlockWriter based on
		// LogicalMemoryPort.getWriteMode
		// if(isBlockRam16())
		// {
		// instance.addParameterValue(new ParameterSetting("WRITE_MODE_A",
		// "\"READ_FIRST\""));
		// instance.addParameterValue(new ParameterSetting("WRITE_MODE_B",
		// "\"READ_FIRST\""));
		// }
		...
		if (getParityWidth() != 0) {
			...
			for (int i = 0; i < (getDepth() * getWidth()); i = i + getWidth()) {
				for (int j = 0; j < getParityWidth(); j++) {
					hexDigitsPar[digitIndex] |= (getInitBit(i
							+ (getWidth() - getParityWidth()) + j)) << shift;
					shift++;
					if (shift == 4) {
						shift = 0;
						digitIndex++;
				...
				if (digitIndex == hexDigitsPar.length) {
					...
					for (int digit = (hexDigitsPar.length - 1); digit >= 0; digit--) {
						initValue += Integer
								.toHexString(hexDigitsPar[digit] & 0xf);
					}
	...
	private void writePortInstantiation(ModuleInstance inst, int index,
			String id) {
		...
		} else if (getLibAddressWidth() <= addrWidth
				&& addrName[index].length() > 0) {
			if (!addrScalar[index]) {
		...
		if (nonpardatwidth > dataAttachWidth[index]
				&& dataInName[index].length() > 0) {
			// padded with zeros
			...
			di_exp = padded_di;
		} else if (nonpardatwidth <= dataAttachWidth[index]
				&& dataInName[index].length() > 0) {
			if (!datScalar[index]) {
		...
		if (nonpardatwidth > dataAttachWidth[index]
				&& dataOutName[index].length() > 0) {
			// padded with zeros
			int zerobits = nonpardatwidth - dataAttachWidth[index];
			Concatenation padded_do = new Concatenation();
			padded_do.add(new Wire(dataOutExtraName[index], zerobits
					+ getParityWidth()).getRange(zerobits - 1, 0));
			if (!datScalar[index]) {
		...
		if (getParityWidth() != 0) {
			...
			if (getParityWidth() > pardatwidth && zerobits == getParityWidth()) {
				dop_exp = new Wire("", getParityWidth());
					...
					&& dataOutName[index].length() > 0) {
				// padd with zeros
				Expression dox_exp = new Wire(dataOutExtraName[index], zerobits)
						.getFullRange();
