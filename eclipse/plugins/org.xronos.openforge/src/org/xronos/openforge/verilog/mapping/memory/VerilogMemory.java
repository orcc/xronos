...
public abstract class VerilogMemory {
	 ...
	 * 
	 * @param memBank
	 *            the {@link MemoryBank} which is being instantiated(accessed).
	 */
	...
	public Ram getLowestCost(Ram[] availableMaps) {
		...
		for (int i = 0; i < availableMaps.length; i++) {
			...
			if (new_match == true) {
				...
				// System.out.println(" element array: " + elements_width +
				// " X " + elements_depth);
	...
	/**
	 * Retrieves the init values from the {@link MemoryBank} and converts them
	 * to a BitSet.
	 * 
	...
	protected List<BigInteger> getInitValuesByLine() {
		...
		for (int line = 0; line < getDepth(); line++) {
			...
			ret.add(AddressableUnit.getCompositeValue(lineValue, getMemBank()
	...
	protected void debugContents(MemoryModule memoryModule) {
		if (EngineThread.getGenericJob().getUnscopedBooleanOptionValue(
				OptionRegistry.DEBUG_MEM_LOCATIONS)) {
			memoryModule.state(new InlineComment(getClass().toString(),
					Comment.SHORT));
