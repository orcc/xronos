...
import org.xronos.openforge.lim.CloneListener;
...
import org.xronos.openforge.lim.memory.MemoryBuilder;
import org.xronos.openforge.lim.memory.MemoryImplementation;
import org.xronos.openforge.optimize.CompactSRL16Visitor;
import org.xronos.openforge.optimize.IOBRegisterVisitor;
import org.xronos.openforge.optimize.IODepthVisitor;
import org.xronos.openforge.optimize.Optimizer;
import org.xronos.openforge.optimize.constant.TwoPassPartialConstant;
import org.xronos.openforge.optimize.io.FifoDataOutputOptimizer;
import org.xronos.openforge.optimize.memory.DualPortRamAllocator;
import org.xronos.openforge.optimize.pipeline.PipelineOptimization;
import org.xronos.openforge.schedule.loop.LoopCycleOptimizer;


/**
 * Scheduler is the entry point for scheduling a LIM design. The intention is
 * that a new Scheduler will be created for each scheduling trial over a given
 * design. When created the Scheduler should contain all the necessary
 * characteristics for the scheduling pass (including pipeline metric, balanced,
...
public class Scheduler {
	...
	public static Design schedule(Design design) {
		 ...
		 * widths. This will allow GateDepthAccumulator to run (some gate depths
		 * depend upon the widths of the inputs).
		 */
		...
		/*
		 * Resolve memory accesses so we can see which accesses refer to which
		 * memories.
		 */
		 ...
		 */
		org.xronos.openforge.optimize.DeadComponentVisitor.pruneMemories(design);
		...
		/*
		 * Build implementations for the memories that remain. MemoryBuilder
		 * must come before global resource sequencer.
		 */
		...
		// if
		// (gj.getUnscopedBooleanOptionValue(OptionRegistry.LOOP_BRANCH_BALANCE))
		// {
		 ...
		 * SimplePinWrites to fifo output data pins when there is only a single
		 * writer to that pin, allowing the pin data value to track intermediate
		 * values when not qualified.
		 */
		...
		/*
		 * Characterize the types and numbers of accesses in each task of the
		 * design and set the characteristics of global entities accordingly.
		 */
		if (_schedule.db)
			_schedule
		...
		/*
		 * Traverse the 'wrapper' function (top level function) of the design
		 * and find any loops whose signatures match the block IO handling loops
		 * and mark those loop flops as removable. FIXME. IDM 03.02.2007 Check
		 * if this optimization is needed.
		 */
		...
		/*
		 * Find any/all multipliers in the design and add pipeline registers to
		 * their outputs as defined by the user.
		 */
		...
		/*
		 * Perform schedule pass which connects inputs of all components and
		 ...
		 * 
		 * Attention XXX: This visitor should not be moved other place because
		 * the Regs chain got removed and replaced by SRL16. In order to resolve
		 * the correct ownership of port's Value bits, this visitor needs to
		 * placed between Pipeliner and PostScheduleOptimize.
		 */
		...
		/*
		 * We need to do this here so that constant prop runs _after_ we are
		 * done, because we may decompose an SRL16 (remove it's last register)
		 * in order to attach an IOB attribute. The Bits from the Reg need to be
		 * propagated correctly. But we also need to be after the
		 * GlobalConnector because the pins must already be hooked up.
		 */
		...
		/*
		 * Make sure any new hardware has appropriate connection Values. This
		 * must be run AFTER all new hardware has been inserted by scheduling,
		 * pipelining, and any other optimizations.
		 */
