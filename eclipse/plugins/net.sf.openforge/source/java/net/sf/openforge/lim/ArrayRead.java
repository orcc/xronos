...
/**
 * ArrayRead is an {@link OffsetMemoryRead} which gets its offset
 * value from a data {@link Port} and may add an additional offset to
 * the runtime offset to skip over the length field of the array.  The
 * MemoryReducer optimization may remove this additional +1 if the
 * length field is pruned from the memory.
 *
...
public class ArrayRead extends OffsetMemoryRead implements ArrayAccess
{
     ...
     * base address and the first data element.
     */
     ...
     * @param addressableLocations, the number of addressable locations read.
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     * @param skip the number of housekeeping addressable locations
     * between the array base address and the first data element
     * @param policy, the address stride policy for this access
     */
    public ArrayRead (int addressableLocations, int maxAddressWidth, boolean isSigned, AddressStridePolicy policy)
    {
     ...
     * @param policy, the address stride policy for this access
     */
    public ArrayRead (int addressableLocations, int maxAddressWidth, boolean isSigned, int skip, AddressStridePolicy policy)
    {
        this(new MemoryRead(false, (addressableLocations * policy.getStride()), isSigned),
            addressableLocations, maxAddressWidth, skip);
    }
    
    /**
     * Constructs an ArrayRead with the specified memory read access.
     * @param memoryRead the actual {@link MemoryRead} used to read
     * the memory, contained in this Module.
     * @param addressableLocations, the number of addressable locations read.
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     * @param skip the number of housekeeping locations between the array base
        ...
        int maxAddressWidth, int skip)
    {
         ...
         * addressable location count to obtain the number of offset
         * locations. 
         */
        final Constant addressableLocationsConstant = new SimpleConstant(addressableLocations, maxAddressWidth, true);
        insertComponent(addressableLocationsConstant, 0);
        final Entry constEntry = addressableLocationsConstant.makeEntry(getInBuf().getExit(Exit.DONE));
        constEntry.addDependency(addressableLocationsConstant.getGoPort(),
            new ControlDependency(constEntry.getDrivingExit().getDoneBus()));

        final MultiplyOp multiplier = new MultiplyOp(maxAddressWidth);
        insertComponent(multiplier, 1);
        final Entry multEntry = multiplier.makeEntry(getInBuf().getExit(Exit.DONE));
        multEntry.addDependency(multiplier.getLeftDataPort(), new DataDependency(offsetBus));
        multEntry.addDependency(multiplier.getRightDataPort(),
            new DataDependency(addressableLocationsConstant.getValueBus()));
        multEntry.addDependency(multiplier.getGoPort(),
            new ControlDependency(multEntry.getDrivingExit().getDoneBus()));

        this.skip = skip;
        this.skipConstant = new SimpleConstant(skip, maxAddressWidth, false);
        insertComponent(skipConstant, 2);
        final Entry skipEntry = skipConstant.makeEntry(getInBuf().getExit(Exit.DONE));
        ...
        addEntry.addDependency(addOp.getRightDataPort(), new DataDependency(multiplier.getResultBus()));
        final Port skipPort = addOp.makeDataPort();
        addEntry.addDependency(skipPort, new DataDependency(skipConstant.getValueBus()));
    }
    ...
    {
        /*
        final MemoryRead cloneRead = new MemoryRead(false, getMemoryRead().getWidth(), getMemoryRead().isSigned());
    ...
    protected void cloneNotify (Module moduleClone, Map cloneMap)
    {
        ...
        ((ArrayRead)moduleClone).skipConstant = (Constant)cloneMap.get(this.skipConstant);
        ((ArrayRead)moduleClone).offsetPort = getPortClone(this.offsetPort, cloneMap);
    }
}
