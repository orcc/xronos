...
import net.sf.openforge.util.license.*;


/**
 * Forge is the main command-line entry point for the
 * Forge compiler.
 *
 *
...
public class Forge implements ForgeDebug
{
    ...
    private static final String _RCS_ = "$Rev: 557 $";
    ...
    public void preprocess(GenericJob gj)
    {
        ...
        if (gj.getUnscopedBooleanOptionValue(OptionRegistry.X_WRITE_CYCLE_C_VPGEN))
        {
            ...
            {
                msgln(OptionRegistry.X_WRITE_CYCLE_C_VPGEN.getHelpFormattedKeyList() + " requires option " +
                    OptionRegistry.WRITE_CYCLE_C.getHelpFormattedKeyList() + " to be set true");
                //System.exit(0);
                throw new ForgeFatalException("Cycle accurate C VPGen requires generation of cycle accurate C");
            }
        }
    ...
    /**
     * Certain options may need to modify other options based on the
     * way that they are set, this method accomplishes that task.
     */
    private void updateOptions (NewJob job)
    {
        ...
        if (job.getUnscopedBooleanOptionValue(OptionRegistry.NOLOG))
        {
            ...
            final OptionList optionList = (OptionList)job.getOption(OptionRegistry.LOG);
            final ArrayList modified = new ArrayList();
            for (Iterator iter = optionList.getValueAsList(CodeLabel.UNSCOPED).iterator(); iter.hasNext();)
            {
            ...
            optionList.replaceValue(CodeLabel.UNSCOPED,OptionList.toString(modified));
        }

        SimpleDateFormat df = new SimpleDateFormat("EEE d MMM yyyy HH:mm:ss Z");
        String runDate = df.format(new Date());
        job.getOption(OptionRegistry.RUN_DATE).replaceValue(CodeLabel.UNSCOPED, runDate);
    }
    
    
    /**
     * Certain options expect that other options are set in certain ways.
     * This method accomplishes that task.
     */
    public static void checkOptions (GenericJob job)
    {
        ...
        boolean noblockio=((OptionBoolean)option).getValueAsBoolean(CodeLabel.UNSCOPED);
        option = job.getOption(OptionRegistry.MODULE_BUILDER);
        boolean module_builder=((OptionBoolean)option).getValueAsBoolean(CodeLabel.UNSCOPED);
        option = job.getOption(OptionRegistry.CHANNEL_DESCRIPTOR);
        boolean channel_descriptor=!option.getValue(CodeLabel.UNSCOPED).equals("null");
        ...
        // check to see that -noblockio has either a -channel_descriptor or a -Xmodule_builder
        ...
        if (job.getUnscopedBooleanOptionValue(OptionRegistry.SYSGEN))
        {
            if (!job.getUnscopedBooleanOptionValue(OptionRegistry.WRITE_CYCLE_C))
            {
                job.fatalError("Option '" +
                    OptionRegistry.SYSGEN.getHelpFormattedKeyList() + "' requires that the option '" +
                    OptionRegistry.WRITE_CYCLE_C.getHelpFormattedKeyList() + "' be specified");
            }
        }
    }
    ...
    public boolean compile(GenericJob gj)
    {
        ...
        if(_gapp) gapp.ln("Compile() called: ");
        ...
        //Proceed further with creating a CEngine only if there is atleast one source file.
...
//             gj.info("          Destination Directory: " + dest.getAbsolutePath());
            ...
            catch(RecursiveMethodException e)
            {
                ...
                gj.decAll();
                gj.error("");
                gj.error("Forge did not compile target: " + targetFile.getAbsolutePath());
            }
            catch(XflowException e)
            {
                ...
                gj.error(e.getMessage());
                gj.error("");
                gj.error("Forge did not finish xflow for target: " + targetFile.getAbsolutePath());
            }
            catch(Throwable t)
            {
                exitError = true;
                ...
                gj.error("Forge compilation ended with fatal internal error: " + t.toString()
                    + "\n" + traceBuf.toString());
        ...
        boolean proj_file = gj.getUnscopedBooleanOptionValue(OptionRegistry.PROJ_FILE);
        boolean proj_file_desc = gj.getUnscopedBooleanOptionValue(OptionRegistry.PROJ_FILE_DESC);
        ...
        if (proj_file || proj_file_desc)
        {
            ...
            {                   
                ...
                if(!proj.exists())
                {
                    //proj = new File(System.getProperty("user.dir") + File.separator + projectName);
                    proj = new File(projectName);
                }
                ForgeProjectWriter fpw = new ForgeProjectWriter(proj, gj, engine, proj_file_desc);
                fpw.write();
            }
            catch (Exception e)
            {
                errorln("Failed to create custom project because -- " + e.toString());
            }
        }
    ...
    /**
     * Set the PrintWriter to use for messages
     * directed to stdout.
     *
     * @param writer the non-null stdout PrintWriter
     */
     ...
     * directed to stderr.
     *
     * @param writer the non-null stderr PrintWriter
     */
    ...
    public class Monitor implements JobListener
    {
        ...
        {
            eng.getGenericJob().info(eng.getCurrentHandler().getStageName() + " " + je.getMessage());
        }
