 ...
 * SRL16
 *
 * A SRL16 now has input ports for n-bit data, 4-bits shifting stage(from 1
 * to 16), one output port for n-bit result with optional enable port. 
 *
...
public class SRL16 extends Primitive
{
    ...
    /**
     * Constructs a SRL16 with stages equals size of registers chain
     * and given id
     *
     */
    ...
    /**
     * Bit states propagate straight through, except that pass through
     * bits become care bits.
     */
    public boolean pushValuesForward ()
    {
        ...
        Value newValue = new Value(drivenValue.getSize(), drivenValue.isSigned());
        
        for (int i=0; i < drivenValue.getSize(); i++)
        {
            if (!drivenValue.getBit(i).isCare() || drivenValue.getBit(i).isConstant())
            {
    ...
    /**
     * Any bit of the consumed output that is dont care or constant
     * becomes a dont care or that constant on the inputs, all other
     * bits are care.
     */
