...
public class LeftShiftOp extends ShiftOp implements Emulatable
{
    ...
    /**
     * Constructs a logical left shift operation with a given maximum
     * number of stages.
     *
     * @param maxStages the maximum number of stages (shifts by 2^n)
     * necessary to implement this shift
     */
    ...
    public Map emulate (Map portValues)
    {
        ...
        final SizedInteger leftSize = SizedInteger.valueOf(getLeftDataPort().getValue().getSize(),rvalSize, true);
        final SizedInteger maskedRval = rval.and(leftSize.subtract(one));
        
        return Collections.singletonMap(getResultBus(), lval.shiftLeft(maskedRval));
    }
    ...
    /**
     * Pushes care, and constant information forward through
     * this LeftShiftOp according to these rules:
     * 
     * If both inputs are all care bits (or have some care bits):
     * All result bits are care.
     *
     * If shift magnitude is a constant value:
     * LSB bits are all 0's. Next 'y' bits are all passthrough of the
     * data input value. y is the width of the data input up to a max
     * of 'result width' total bits (including lsb 0's)
     *
    ...
    /**
     * Reverse constant prop on a LeftShiftOp has two cases:
     * 1. If both inputs are all care bits (or have some care bits),
     *    port 0 size is result size.
     * 2. If shift magnitude is a constant value, result (consumed)
     *    bit states can push back directly to port states shifted by
     *    'n' positions.
     *
     * ABK: Also, the shift magnitude can be limited to the maximum
     * stages annotated on the shift.
     * 
     * @return a value of type 'boolean'
     */
    public boolean pushValuesBackward ()
    {
        ...
        Value newValue0 = new Value(resultBusValue.getSize(), resultBusValue.isSigned());
        ...
        if (inValue1.isConstant())
        {
            ...
            else
            {
                ...
                for (int i = (newValue0.getSize() - shiftMag); i < newValue0.getSize(); i++)
                {
                    newValue0.setBit(i, Bit.DONT_CARE);
                }
            }
        ...
        {
            /*
             * Since we're shifting left, we can keep chop off
             * don't-care bits from the left end.
             */
