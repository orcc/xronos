...
/**
 * LGraph uses <a href="http://www.research.att.com/sw/tools/graphviz/">GraphViz</a>
 * to generate an ASCII description of a LIM graph.  A graph viewer, <i>dotty</i> may
 * then be used to view the generated ASCII.  It may also be printed by:
 * <i> dot -Tps file.dot | lpr </i>
 *
...
public class LGraph extends FilteredVisitor implements Visitor
{
    ...
    private void graph (Component component)
    {
        if (graphPhysical)
        {
            ...
            {
                int gindex = 0;
                for (Iterator go_ports = ((Mux)portOwner).getGoPorts().iterator(); go_ports.hasNext();)
                {
                    Port go_port = (Port)go_ports.next();
            ...
            for (Iterator iter = portOwner.getDataPorts().iterator(); iter.hasNext();)
            {
            ...
            if (!(component instanceof InBuf)) // don't graph connections to inbuf because the inbuf's owners will graph them
            {
                //graph from the buses that feed the portowners ports to the ports
                for (Iterator iter = portOwner.getPorts().iterator(); iter.hasNext();)
                {
                    Port port = (Port)iter.next();
                    if (!drawCR && ( (port == portOwner.getClockPort()) ||
                                     (port == portOwner.getResetPort())))
                    {
                        continue;
                    }
                    if (port.getBus() != null)
    ...
    private void graphPostVisit (Module module, boolean feedback)
    {
        ...
        if (graphDependencies)
        {
            ...
            {
                Node node = (Node)nodeMapDeps.get(iter.next());
                                 ...
                                 boolean feedback)
    {
        ...
        {
            Node entryNode = (Node)nodeMapDeps.get(iter.next());
            Edge edge;
            if (feedback)
            {
            ...
            {
                /*
                 * If connecting to something inside the owner,(ie an InBuf) then
                 * use a dashed line.
                 */
...
//              }
    ...
    private void connectPorts (Component component, boolean feedback)
    {
        ...
        {
            Port port = (Port)iter.next();
            if (port.getBus() == null || !drawCR && ( (port == component.getClockPort()) ||
                    (port == component.getResetPort())))
            {
                continue;
            }
    ...
    private void graphExit (Exit exit, int id)
    {
        ...
        record.setTitle(getName(exit)+"::"+Integer.toHexString(exit.hashCode()));
    ...
    private void graphDependency (Dependency dep, boolean feedback)
    {
        ...
        if (graphLogical 
            && ((graphControl && dep instanceof ControlDependency) ||
                (graphData && dep instanceof DataDependency)) 
            && logicalBus != null)
        {
    ...
    private void graphPhysical (Bus bus, Port port, boolean feedback)
    {
        ...
        assert target != null : "Problem with missing target in graphPhysical("+bus+","+port+")";
    ...
    public void filterAny(Component c) { graph(c); }
}
