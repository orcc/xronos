...
/**
 * SimplePinStall is a LIM module that provides the ability to stall
 * the LIM wavefront (GO/DONE wavefront) until a pin is non-zero.
 *
...
public class SimplePinStall extends SimplePinAccess
{
    ...
    public SimplePinStall (SimplePin pin)
    {
        ...
        else
        {
            ...
            // is non-zero
            // DONE = (GO | GO') & pin
            // GO' = (GO' | GO) & !pin // IDM modified.  This does not handle back to back GO assertions where the DONE is not combinational 
            // GO' = GO | (GO' & !pin)
            Or goOr = new Or(2);
            ...
            ((Port)goPrimeAnd.getDataPorts().get(0)).setBus(goReg.getResultBus());
            ((Port)goPrimeAnd.getDataPorts().get(1)).setBus(not.getResultBus());
            
            ((Port)goPrimeOr.getDataPorts().get(0)).setBus(getGoPort().getPeer());
            ((Port)goPrimeOr.getDataPorts().get(1)).setBus(goPrimeAnd.getResultBus());
