...
public class XorOp extends BinaryOp implements Emulatable
{
     ...
     * this XorOp according to these rules:
     * <pre>
     * x ^ * = x
     * 0 ^ c = c
     * 1 ^ c = c
    ...
    public boolean pushValuesForward ()
    {
        ...
        if (getResultBus().getValue() != null)
        {
            ...
            {
                int compactedSize = Math.min(newSize, Math.max(in0.getCompactedSize(), in1.getCompactedSize()));
    ...
    /**
     * Reverse constant prop on a XorOp simply propagates the
