...
/**
 * XDesignFactory is a factory class used to generate a fully
 * populated {@link Design} object from an XLIM XML document.  The
 * design includes one or more {@link LogicalMemory} objects for the
 * state variables and a {@link Task} for each Action module.
 *
 *
 * <p>Created: Tue Jul 12 12:07:03 2005
 *
...
public class XDesignFactory extends XFactory
{
    ...
    private static final Configurable UNSCOPED_CONFIG = new Configurable()
            {
                public Configurable getConfigurableParent () { return null; }
                public String getOptionLabel () { return null; }
                public SearchLabel getSearchLabel () { return CodeLabel.UNSCOPED; }
                public GenericJob getGenericJob () { return EngineThread.getGenericJob(); }
            };
    ...
    /**
     * Build the {@link Design} object and populate it with the
     * structures necessary to implement the XLIM specification in the
     * document. 
     *
     * @param doc a non-null Document, XLIM representation.
    ...
    public Design buildDesign (Element designElement)
    {
        ...
        {
            designName = "xlimApp_" + Integer.toHexString(designElement.hashCode());
        }
        
        Design design = new Design();
        design.setIDLogical(designName);
        GenericJob job = EngineThread.getGenericJob();
        job.getOption(OptionRegistry.TOP_MODULE_NAME).setValue(design.getSearchLabel(), designName);
        
        NodeList directChildren = designElement.getChildNodes();
        List topLevelModules = new ArrayList();
        List topLevelOps = new ArrayList();
        List topLevelConnections = new ArrayList();
        Map<LogicalValue, Node> stateVars = new HashMap();
        for (int i=0; i < directChildren.getLength(); i++)
        {
            ...
            if (node.getNodeName().equals(SLIMConstants.ACTOR_PORT))
            {
                ...
                ActionIOHandler ioHandler = new ActionIOHandler.FifoIOHandler(node);
                ioHandler.build(design);
                resources.addIOHandler(node, ioHandler);
            }
            else if (node.getNodeName().equals(SLIMConstants.INTERNAL_PORT))
            {
                // <internal-port name="internal_action1_go" typeName="bool" size="1" tag="idm0"/>
                ActionIOHandler ioHandler = new ActionIOHandler.InternalPinHandler(node);
                ioHandler.build(design);
                resources.addIOHandler(node, ioHandler);
            }
            else if (node.getNodeName().equals(SLIMConstants.STATE_VAR))
            {
                ...
                if (logicalMemory == null)
                {
                    ...
                    // width
                    logicalMemory = new LogicalMemory(32);
                    logicalMemory.createLogicalMemoryPort();
                    design.addMemory(logicalMemory);
                }
                // Create a 'location' for the state var that is
                // appropriate for its type/size.
                Allocation loc = logicalMemory.allocate(createInitialValue(node));
            ...
            else if (node.getNodeType() == Node.TEXT_NODE) { } // Do nothing with text nodes
            else
            {
                System.out.println("WARNING!!! Unknown element type at design level " + node.getNodeName());
            }
        }
        ...
        Map<Integer, LogicalMemory> memories = new HashMap();
        ...
        final Map taskModules = new HashMap();
        for (Iterator iter = topLevelModules.iterator(); iter.hasNext();)
        {
            Node moduleNode = (Node)iter.next();
            ...
            String taskName = ((Element)moduleNode).getAttribute(SLIMConstants.NAME);
            String autostart = ((Element)moduleNode).getAttribute(SLIMConstants.AUTOSTART);
        ...
        {
            final String taskID = taskCallNode.getAttribute(SLIMConstants.RESOURCE_TARGET);
            final Task task = (Task)taskModules.get(taskID);
            if (task == null)
                throw new IllegalStateException("Task Call to non-existant task "+SLIMConstants.RESOURCE_TARGET+": " + taskID);
            final TaskCall taskCall = resources.getTaskCall(taskCallNode);
            taskCall.setTarget(task);
        }

        connectDesign(topLevelOps, topLevelConnections, design, portCache, resources);
        ...
        if (_parser.db)
        {
            System.out.println("Created design with " + design.getLogicalMemories());
    ...
    /**
     * Generates a {@link LogicalValue} from a
     * {@link SLIMConstants#STATE_VAR} Node.  
     *
     * @param node a Node of type {@link SLIMConstants#STATE_VAR}
     * @return the LogicalValue representation of the initial value of
     * the state var node.
     */
    private LogicalValue createInitialValue (Node node)
    {
        ...
        final List contained = getChildNodesByTag(element, SLIMConstants.IVALUE);
    ...
    private LogicalValue makeLogicalValue (Element initValue)
    {
        ...
        if (initValue.getAttribute(SLIMConstants.TYPENAME).equalsIgnoreCase(SLIMConstants.IVALUE_LIST_TYPE))
        {
            List children = getChildNodesByTag(initValue, SLIMConstants.IVALUE);
            List subElements = new ArrayList(children.size());
        ...
        else // scalar
        {
            ...
            final String sizeString = initValue.getAttribute(SLIMConstants.IVALUE_SIZE);
            final int bitSize = Integer.parseInt(sizeString);
            String valueString = initValue.getAttribute(SLIMConstants.IVALUE_VALUE);
            if (valueString.length() == 0)
            {
                System.out.println("WARNING.  Uninitialized state var.  Defaulting to 0");
                valueString = "0";
            }
        ...
        if (_parser.db) System.out.println("Created logical value");
        if (_parser.db) System.out.println(logicalValue.toString());
        return logicalValue;
    }

    private static void connectDesign (Collection ops, Collection conns, Design design, PortCache portCache, ResourceCache resources)
    {
        XOperationFactory opFactory = new XOperationFactory(resources);
        for (Iterator iter = ops.iterator(); iter.hasNext();)
        {
            Element node = (Element)iter.next();
            assert node.getAttribute(SLIMConstants.ELEMENT_KIND).equals("pinRead") || node.getAttribute(SLIMConstants.ELEMENT_KIND).equals("pinWrite");
        ...
        for (Iterator iter = conns.iterator(); iter.hasNext();)
        {
            ...
            String sourceName = connection.getAttribute(SLIMConstants.CONN_SOURCE);
            ...
            if (dest.isConnected())
                throw new IllegalStateException("Connection defined for previously connected port");
            if (_parser.db) System.out.println("Connecting source: " + source + " to dest: " + dest + " of " + source.getOwner().getOwner().show() + " to " + dest.getOwner().show());
            dest.setBus(source);
        }
    }
    ...
    /**
     * <code>applyConfiguration</code> uses the configuration map from
     * the {@link ResourceCache} to process all LIM Components/nodes
     * which have configuration information attached.  Each entry in
     * the map has at least one {@link SLIMConstants#CONFIG_OPTION}.
     * These options are annotated to the options database as
     * appropriate. 
     *
     * @param des a <code>Design</code> value
     * @param res a <code>ResourceCache</code> value
     */
    private static void applyConfiguration (Design des, ResourceCache res)
    {
        ...
        // any config options and apply them.
        final Map configMap = res.getConfigurableMap();
        for (Iterator iter = configMap.keySet().iterator(); iter.hasNext();)
        {
            Configurable config = (Configurable)iter.next();
            ...
            for (int i=0; i < elementChildren.getLength(); i++)
            {
                ...
                if (!node.getNodeName().equalsIgnoreCase(SLIMConstants.CONFIG_OPTION))
                {
                    continue;
                }
                ...
                if (name.equalsIgnoreCase(SLIMConstants.CONFIG_LOOPUNROLL))
                {
                    ...
                    if (_parser.db) System.out.println("Setting loop unroll limit of " + config + " w/ search label: ->" + config.getSearchLabel() + "<- to " + valueInt);
                    String enable = "true";
                    if (valueInt.intValue() < 0)
                    {
                        enable = "false";
                    }
                    config.getGenericJob().getOption(OptionRegistry.LOOP_UNROLLING_ENABLE).setValue(config.getSearchLabel(), enable);
                    config.getGenericJob().getOption(OptionRegistry.LOOP_UNROLLING_LIMIT).setValue(config.getSearchLabel(), valueInt);
                }
                else
                {
                    ...
                    for (OptionKey key : OptionRegistry.OPTION_KEYS)
                    {
                        ...
                        {
                            match = true;
                            if (_parser.db) System.out.println("Setting config option " + key.getKey() + " of config scope " + config + " w/ search label: ->" + config.getSearchLabel() + "<- to " + value);
                            config.getGenericJob().getOption(key).setValue(config.getSearchLabel(), value);
                            break;
                        }
                    }
                    if (!match)
                    {
                        throw new IllegalArgumentException("The " + SLIMConstants.CONFIG_OPTION + " with name \"" + name + "\" is unknown");
                    }
                }
            }
