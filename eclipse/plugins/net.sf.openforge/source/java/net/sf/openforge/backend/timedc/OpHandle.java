...
/**
 * OpHandle is a class which handles maintaining unique String identifiers for
 * each Bus associated with a given {@link Component} as well as ensuring that
 * each identifier is only declared one time.
 * 
 * <p>
...
public class OpHandle {
	...
	/**
	 * If set to true an ifdef will be written out with a printf for each
	 * assignment that is handled here.
	 */
	...
	public OpHandle(Component comp, CNameCache cache) {
		if (comp == null) {
			throw new IllegalArgumentException(
					"Cannot construct a handle with a null component");
		}
		...
		for (Exit exit : comp.getExits()) {
			List<Bus> dataBuses = exit.getDataBuses();
			busNames.put(exit.getDoneBus(), baseName + "_" + exitCount
					+ "_done");
	...
	boolean isDeclared(Bus b, String postFix) {
		/*
		 * Debug of BusTuple. Used to ensure that the bus tuple works as a
		 * hashmap key BusTuple t = new BusTuple(b, postFix);
		 * System.out.println("Testing " + b + " and -" + postFix + "-" + t +
		 * " " + this.declaredBuses.contains(t)); for (Iterator iter =
		 * this.declaredBuses.iterator(); iter.hasNext();) { Object o =
		 * iter.next(); System.out.println(t + " " + o);
	...
	/**
	 * Assigns the specified value to the bus and has the side-effect of
	 * declaring the bus if it is not already declared.
	 */
	...
	/**
	 * Assigns the specified value to the bus and has the side-effect of
	 * declaring the bus if it is not already declared, the bus name is
	 * postpended with the postfix.
	 */
	public String assign(Bus b, String rightHandSide, String postFix) {
		final String assignment = getBusDecl(b, postFix) + " = "
				+ rightHandSide + ";";
	...
	/**
	 * Gets a correct declaration for the specified bus with initial value of 0.
	 */
	...
	/**
	 * Gets a correct declaration for the specified bus with the specified
	 * initial value.
	 */
	...
	/**
	 * Irrevesibly replaces the generated name for the given bus with the
	 * supplied value, as expected this will not affect any value retrieved
	 ...
	 *             if b or name are null
	 */
	public void overrideName(Bus b, String name) {
		if (b == null || name == null) {
			throw new IllegalArgumentException(
					"Illegal use of null value in name override");
		}
	...
	/**
	 * Returns the C data type necessary to cover the specified bit width, ie
	 * char, short, int, or long long for 8, 16, 32, or 64 bits respectively.
	 * 
	 * @param size
	 *            a value of type 'int'
	...
	/**
	 * This class is used as a key into the declaredbuses set. MODIFY WITH GREAT
	 * CAUTION! The intent here is that any instance of this class with the same
	 * combination of bus plus postfix string will cause a match in
