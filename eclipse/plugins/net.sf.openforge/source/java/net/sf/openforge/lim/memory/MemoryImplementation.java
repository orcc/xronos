...
import net.sf.openforge.verilog.mapping.memory.DualPortRam;
import net.sf.openforge.verilog.mapping.memory.Ram;


/**
 * MemoryImplementation is a class that looks at the specific memory
 * to be implemented and determines the optimal implementation type
 * (lut vs block and RAM vs ROM).  
 *
 * <p>Created: Mon Dec  2 16:12:19 2002
 *
...
public abstract class MemoryImplementation 
{
    ...
    /**
     * Returns true if this memory, in dual port mode, resolves
     * address conflicts by a read-first strategy.  Returns false if
     * the strategy is write-first.  NOTE that this is the behavior
     * across the two ports, NOT single port behavior. 
     */
    ...
    public static void setMemoryImplementation (LogicalMemory mem)
    {
        ...
        mem.setImplementation(
            getMemoryImplementation(
                mem.getSizeInBytes(), readOnly, mem.getLogicalMemoryPorts().size()
                ));
    }
     ...
     *
     * @param bitWidth the width of the memory in bits
     * @param depth the depth of the memory in lines
     * @param readOnly, true if the memory is a read only memory
     * @param portCount, the number of access ports to the memory.
     */
    public static MemoryImplementation getMemoryImplementation (
        int bytes, boolean readOnly, int portCount)
    {
...
//         int lut_width = new Integer(op.getValue(CodeLabel.UNSCOPED).toString()).intValue();
//         op = gj.getOption(OptionRegistry.MAX_LUT_DEPTH);
//         int lut_depth = new Integer(op.getValue(CodeLabel.UNSCOPED).toString()).intValue();
        op = gj.getOption(OptionRegistry.MAX_LUT_BYTES);        
        final int lut_bytes = ((OptionInt)op).getValueAsInt(CodeLabel.UNSCOPED);
        /* If set to true, our memory writer will add a register
         * between the RAM primitive DOUT and the output of our
         * memory. */
        boolean lutsAreCombRead = gj.getUnscopedBooleanOptionValue(OptionRegistry.COMBINATIONAL_LUT_MEM);
        ...
        {
            throw new IllegalArgumentException("All memories must be either single or dual ported. " + portCount);
        }

        return (isROM ?
            ((MemoryImplementation)new DefaultROM()) :
            ((MemoryImplementation)new DefaultRAM()));
    }

    private static MemoryImplementation getType (boolean isROM, boolean isLUT, boolean combLutRead)
    {
        if (isLUT)
        {
            if (combLutRead)
            {
                return (isROM ?
                    ((MemoryImplementation)new LutROM()) :
                    ((MemoryImplementation)new LutRAM()));
            }
            else
            {
                return (isROM ?
                    ((MemoryImplementation)new RegisteredLutROM()) :
        ...
        {
            return (isROM ?
                ((MemoryImplementation)new BlockROM()) :
    ...
    public boolean equals (Object o)
    {
        ...
        if (!this.getReadLatency().equals(test.getReadLatency())) return false;
        if (this.isROM() != test.isROM()) return false;
        if (!this.isROM())
        {
            if (!this.getWriteLatency().equals(test.getWriteLatency())) return false;
        }
        if (this.isLUT() != test.isLUT()) return false;
        if (this.isDPReadFirst() != test.isDPReadFirst()) return false;
        if (this.isDefault() != test.isDefault()) return false;
    ...
    public int hashCode ()
    {
        ...
        if (!isROM()) base += getWriteLatency().hashCode();
        if (isLUT()) base += 285;
        if (isROM()) base += 385;
        if (isDPReadFirst()) base += 485;
        if (isDefault()) base += 585;
        return base;
    }
    ...
    {
        public Latency getReadLatency () { return Latency.ZERO; }
        public Latency getWriteLatency () { return Latency.ONE; }
        public boolean isLUT () { return true; }
        public boolean isROM () { return false; }
        public boolean isDPReadFirst () { return true; }
        public boolean isDefault() { return false; }
    }
    
    private static class RegisteredLutRAM extends MemoryImplementation
    {
        public Latency getReadLatency () { return Latency.ONE; }
        public Latency getWriteLatency () { return Latency.ONE; }
        public boolean isLUT () { return true; }
        public boolean isROM () { return false; }
        public boolean isDPReadFirst () { return true; }
    ...
    {
        public Latency getReadLatency () { return Latency.ZERO; }
        public Latency getWriteLatency () { throw new UnsupportedOperationException("Cannot write to a ROM"); }
        public boolean isLUT () { return true; }
        public boolean isROM () { return true; }
        public boolean isDPReadFirst () { return true; }
        public boolean isDefault() { return false; }
    }
    ...
    private static class BlockRAM extends MemoryImplementation
    {
        public Latency getReadLatency () { return Latency.ONE; }
        public Latency getWriteLatency () { return Latency.ONE; }
        public boolean isLUT () { return false; }
        public boolean isROM () { return false; }
