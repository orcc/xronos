...
public class DivideOpRule 
{
    ...
    public static boolean halfConstant(DivideOp op, Number[] consts, ComponentSwapVisitor visit)
    {
        ...
        if (p1 != null && p1.longValue() == 0)
        {
            ...
            // 0 / a = 0.  Simply delete the component and replace with constant 0
        ...
        else if(p2 != null && p2.longValue() == 0)
        {
            ...
            EngineThread.getGenericJob().warn("    Found devide by 0 at line " + op.getIDSourceInfo().getSourceLine() + ", replacing with 0.");
        ...
        else if(p2 != null && isAllOnes(p2, p2size)) // p2 is -1
        {
            ...
            portCorrelation.put(op.getDataPorts().get(0), mop.getDataPorts().get(0));
        ...
        else if(p2 != null && p2.longValue() > 1)
        {
            ...
            if(power != -1)
            {
                ...
                // Decision
                //
                Constant zero = new SimpleConstant(0, argValue.getSize(), argValue.isSigned());
                ...
                gte_entry.addDependency((Port)testComponent.getDataPorts().get(0), dep0);
                Dependency dep1 = new DataDependency(zero.getValueBus());
                gte_entry.addDependency((Port)testComponent.getDataPorts().get(1), dep1);
                ...
                testb_entry.addDependency((Port)testBlock.getDataPorts().get(0), testb_dep0);
                // sizing info
...
//                  ((Port)decision.getNot().getDataPorts().get(0)).setValue(new Value(1));
                // end making Decision
                
                //
                // True Branch
                //
                Constant true_power = new SimpleConstant(power, argValue.getSize(), argValue.isSigned());
                RightShiftUnsignedOp trueShift = new RightShiftUnsignedOp(getLog2(op.getNaturalSize()));
                ...
                Exit trueBranch_exit = (Exit)trueBranch.getExits().iterator().next();
                ...
                ts_entry.addDependency((Port)trueShift.getDataPorts().get(0), ts_dep0);
                Dependency ts_dep1 = new DataDependency(true_power.getValueBus());
                ts_entry.addDependency((Port)trueShift.getDataPorts().get(1), ts_dep1);
                OutBuf trueBranch_out = (OutBuf)trueBranch.getOutBufs().iterator().next();
                Entry trueBranch_outEntry = (Entry)trueBranch_out.getEntries().get(0);
                Dependency trueBranch_outDep = new DataDependency(trueShift.getResultBus());
                trueBranch_outEntry.addDependency((Port)trueBranch_out.getDataPorts().get(0), trueBranch_outDep);
                // sizing info
//                 assert false : "New constant prop: fix these lines. --SGE";
...
//                  Component.getDataBus(trueBranch).setBits(argValue.size());
                ...
                Constant false_power = new SimpleConstant(power, argValue.getSize(), argValue.isSigned());
                RightShiftUnsignedOp falseShift = new RightShiftUnsignedOp(getLog2(op.getNaturalSize()));
                ...
                Exit falseBranch_exit = (Exit)falseBranch.getExits().iterator().next();
                ...
                fs_entry.addDependency((Port)falseShift.getDataPorts().get(0), fs_dep0);
                Dependency fs_dep1 = new DataDependency(false_power.getValueBus());
                fs_entry.addDependency((Port)falseShift.getDataPorts().get(1), fs_dep1);
                // make minusShift dependency
                Entry ms_entry = (Entry)minusShift.getEntries().get(0);
                Dependency ms_dep0 = new DataDependency(falseShift.getResultBus());
                ms_entry.addDependency(minusShift.getDataPort(), ms_dep0);
                // make OutBuf dependency
                OutBuf falseBranch_out = (OutBuf)falseBranch.getOutBufs().iterator().next();
                Entry falseBranch_outEntry = (Entry)falseBranch_out.getEntries().get(0);
                Dependency falseBranch_outDep = new DataDependency(minusShift.getResultBus());
                falseBranch_outEntry.addDependency((Port)falseBranch_out.getDataPorts().get(0), falseBranch_outDep);
                // sizing info
//                 assert false : "New constant prop: fix these lines. --SGE";
...
//                  ((Port)falseBranch_out.getDataPorts().get(0)).setValue(argValue);
                ...
                //
                // Branch, and make dependency between decision, true & false branches
                //
...
//                 branch_exit.makeDataBus();
                ...
                dec_entry.addDependency((Port)decision.getDataPorts().get(0), dec_dep0);
                Entry true_entry = (Entry)trueBranch.getEntries().get(0);
                Dependency true_dataDep = new DataDependency(branch_port.getPeer());
                true_entry.addDependency((Port)trueBranch.getDataPorts().get(0), true_dataDep);
                Entry false_entry = (Entry)falseBranch.getEntries().get(0);
                Dependency false_dataDep = new DataDependency(branch_port.getPeer());
                false_entry.addDependency((Port)falseBranch.getDataPorts().get(0), false_dataDep);


                OutBuf branch_out = (OutBuf)branch.getOutBufs().iterator().next();
                Entry forTrueBus_entry = (Entry)branch_out.getEntries().get(0);
                Dependency outTrue_dep0 = new DataDependency(Component.getDataBus(trueBranch));
                forTrueBus_entry.addDependency((Port)forTrueBus_entry.getDataPorts().get(0), outTrue_dep0);
                Entry forFalseBus_entry = (Entry)branch_out.getEntries().get(1);
                Dependency outFalse_dep0 = new DataDependency(Component.getDataBus(falseBranch));
                forFalseBus_entry.addDependency((Port)forFalseBus_entry.getDataPorts().get(0), outFalse_dep0);
                ...
                portCorrelation.put(nonConstantPort, branch.getDataPorts().get(0));

                Map busCorrelation = new HashMap();
                busCorrelation.put(exit.getDataBuses().get(0), branch_exit.getDataBuses().get(0));
...
//                 portCorrelation.put(op.getDataPorts().get(0), shift.getDataPorts().get(0));
...
//                 Map busCorrelation = new HashMap();
//                 busCorrelation.put(exit.getDataBuses().get(0), shift.getResultBus());
//                 assert shift.getExits().size() == 1 : "Only expecting one exit on MinusOp";
//                 busCorrelation.put(exit.getDoneBus(), ((Exit)shift.getExits().iterator().next()).getDoneBus());                
//                 assert shift.getExits().size() == 1 : "Only expecting one exit on node to be replaced";
//                 Map exitCorrelation = new HashMap();
...
//                 Entry div_entry = (Entry)constantPort.getOwner().getEntries().get(0);
...
//                 //visit.replaceComponent(constantPort.getOwner(), new Constant(power));
...
//                 op.disconnect();
    ...
    /**
     * Returns true if the specified Number consists of 'busSize'
     * consecutive ones LSB aligned.
     *
     * @param num the Number to test.
     * @param busSize the number of LSB bits to test.
