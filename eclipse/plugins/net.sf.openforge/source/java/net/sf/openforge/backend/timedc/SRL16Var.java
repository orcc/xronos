...
/**
 * SRL16Var maintains the stateful variables for an SRL16.  This
 * includes an array for maintaining the stages of the SRL16 as well
 * as a variable to emulate the state of the enable signal.
 *
 *
...
public class SRL16Var extends IndexedVar implements StateVar
{
     ...
     * array initialization data for the SRL stages and the enable
     * variable.  The SRL16 is assumed to initialize to 0 for each
     * stage.  If the SRL16 is NOT an enabled SRL16, then the enable
    ...
    public void declareGlobal (PrintStream stream)
    {
        ...
        String declType = StateVar.STORAGE_CLASS + getTypeDeclaration(this.srl.getResultBus().getValue());
        int stages = getDepth();
        stream.println(declType + " " + getDataIn() + " = 0;");
        if (this.srl.getEnablePort().isUsed())
        {
            stream.println(StateVar.STORAGE_CLASS + "char " + getEnable() + " = 0;");
        }
        else
        {
            stream.println(StateVar.STORAGE_CLASS + "char " + getEnable() + " = 1;");
        }
     ...
     * {@link StateVar} to the specified stream.  The state array is
     * shifted by one stage if the enable signal is true.
     *
     * @param ps a non-null PrintStream
     * @throws NullPointerException if ps is null
     */
    public void writeTick (PrintStream ps)
    {
        ...
        */
        ...
        ps.println("\t\t\t" + getMemoryName() + "[i] = " + getMemoryName() + "[i-1];");
        ps.println("\t\t}");
