...
/**
 * A Module is a {@link Component} that can be a container for
 * other {@link Component Components}.
 *
...
public abstract class Module extends Component implements Cloneable
{
    ...
    /**
     * The component which provides continuation buses for
     * module ports
     */
    private InBuf inBuf;
     ...
     *
     * @param dataCount the number of data ports to be created
     *          initially for the module; a supporting InBuf
     *          will also be created and added to the module
     */
    ...
    /**
     * Returns true if the component in this Module are mutually
     * exclusive in their execution and thus do not need any
     * additional dependencies beyond standard data/control
     * dependencies. {@see MutexBlock}
     *
     * @return false
     */
    public boolean isMutexModule () { return false; }
    ...
    /**
     * Adds a component to this Module.  This will also call
     * the component's {@link Component#setOwner(Module)} method
     * with this object as the argument.
     *
     * @param component the {@link Component Component} to be added
     */
    ...
    /**
     * Adds a collection of components to this Module.  This will also
     * call each component's {@link Component#setOwner(Module)} method
     * with this object as the argument.
     *
     * @param components the {@link Component Components} to be added
     */
    ...
    /**
     * Removes a component from this Module.  This will also call
     * the component's {@link Component#setOwner(Module)} method
     * with a null argument.
     *
     * @param component the {@link Component Component} to be removed
    ...
    /**
     * Performs a module specific replacement of the given component.
     * Returns true if the 'removed' Component was removed and the
     * 'inserted' component was added to this module.
     */
    public abstract boolean replaceComponent (Component removed, Component inserted);

    /**
     * Tests whether a given component is contained by this Module.
     * Components are not recursively queried.
     *
     * @param component the component to test
     * @return true if the component is contained by this module,
     *         false otherwise
     */
    ...
    {
        /*
         * By default, any Module that is not owned by another
         * Module will be translated as an instantiatable module
         * (e.g., a Verilog 'module').
         */
     ...
     * the points in this Module that are feedback points within this
     * module, it does not report on any feedback contained within any
     * sub-modules.
     *
    ...
    /**
     * Makes a new {@link Exit} for this module.  Each {@link Exit} in
    ...
    public Exit makeExit (int dataCount, Exit.Type type, String label)
    {
        ...
        assert exits.get(exit.getTag()) == null : "Duplicate Exit type: " + exit.getTag();
    ...
    /**
     * make a new data port for this module, which is tagged - ie for global access
     *
    ...
    /**
     * Sets whether this component requires a connection to its
     * <em>go</em> {@link Port} in order to commence processing.
     */
    ...
    /**
     * Sets whether this component produces a signal on the
     * done {@link Bus} of each of its {@link Exit Exits}.
     */
    ...
    /**
     * Sets whether the done signal, if any, produced by this
     * module (see {@link Module#producesDone()}) is
     * synchronous or not.  A true value means that the done signal
     * will be produced with the clock and no earlier than the go
     * signal is asserted.
    ...
    /**
     * Sets whether this component requires a connection to
     * its clock {@link Port}.
     */
    ...
    /**
     * Shortcut for adding dependencies to the clock, reset, and go of a
     * child component.
     *
     * @param entry the entry in which the dependencies are added; the {@link Component}
     *          is obtained with {@link Entry#getOwner()}
     * @param clockBus the bus for which a {@link ClockDependency} will be added for
     *          the component's clock {@link Port}
     * @param resetBus the bus for which a {@link ResetDependency} will be added for
     *          the component's reset {@link Port}
     * @param goBus the bus for which a {@link ControlDependency} will be added for
     *          the component's go {@link Port}
     */
    protected static void addDependencies (Entry entry, Bus clockBus, Bus resetBus, Bus goBus)
    {
        Component component = entry.getOwner();
        entry.addDependency(component.getClockPort(), new ClockDependency(clockBus));
        entry.addDependency(component.getResetPort(), new ResetDependency(resetBus));
     ...
     *
     * @param component the component whose exits are to be added
     * @param exitMap a map of {@link Exit.Tag} to {@link Collection} of {@link Exit}
     */
     ...
     * {@link Exit Exits} of its components.  For each {@link Exit.Tag Tag},
     * an {@link Exit} is created on the {@link Module}; the peer of that
     * {@link Exit} is then decorated with an {@link Entry} and a
     * {@link ControlDependency} for each child {@link Exit} with that
     * same {@link Exit.Tag Tag}.
     *
     * @param exitMap a map of exit tag to collection of child exits with
     *          that tag for all relevant components within the module
     * @param clockBus the clock bus for each new exit
     * @param resetBus the reset bus for each new exit
     */
    ...
    /**
     * Creates the {@link Exit Exits} of this module based upon the
     * {@link Exit Exits} of its components.  For each {@link Exit.Tag Tag},
     * an {@link Exit} is created on this module; the peer of that
     * {@link Exit} is then decorated with an {@link Entry} and a
     * {@link ControlDependency} for each child {@link Exit} with that
     * same {@link Exit.Tag Tag}.
     */
    ...
    /**
     * Creates the {@link Exit Exits} of this module based upon the
     * {@link Exit Exits} of a given set of child components.  For
     * each {@link Exit.Tag Tag},
     * an {@link Exit} is created on this module; the peer of that
     * {@link Exit} is then decorated with an {@link Entry} and a
     * {@link ControlDependency} for each child {@link Exit} with that
     * same {@link Exit.Tag Tag}.
     *
     * @param components a collection of child {@link Component Components}
     */
    ...
    public Object clone () throws CloneNotSupportedException
    {
        ...
        /*
         * convert the clones feedback points map to the right set of
         * components 
         */
     ...
     *
     * @param clone the clone of this module
     * @param cloneMap a map of each original {@link Component} and {@link Entry} to
     *          to its clone
     */
    ...
    /**
     * Clones the {@link Dependency Dependencies} and {@link Bus}
     * connections between the {@link Component Components} of a
     * cloned {@link Module}.
     *
     * @param clone the clone of this module
     * @param cloneMap the map of original components to cloned components;
     *          this method will add the mapping from original entries
     *          to cloned entries
     */
    private void cloneConnections (Module clone, Map cloneMap)
    {
        for (Iterator iter = components.iterator(); iter.hasNext();)
        {
             ...
             */
            for (Iterator eiter = component.getEntries().iterator();
                 eiter.hasNext();)
            {
                final Entry entry = (Entry)eiter.next();
                final Exit drivingExit = entry.getDrivingExit();
                final Exit drivingExitClone = (drivingExit == null ? null : getExitClone(drivingExit, cloneMap));
                final Entry entryClone = componentClone.makeEntry(drivingExitClone);
                cloneMap.put(entry, entryClone);

                for (Iterator piter = entry.getPorts().iterator(); piter.hasNext();)
                {
                         ...
                         diter.hasNext();)
                    {
                        ...
                        final Dependency dependencyClone = (Dependency)dependency.clone();
                        final Bus logicalBusClone = getBusClone(logicalBus, cloneMap);
             ...
             */
            for (Iterator piter = component.getPorts().iterator(); piter.hasNext();)
            {
                final Port port = (Port)piter.next();
                final Port portClone = getPortClone(port, cloneMap);
                if (port.isConnected())
     ...
     *
     * @param port the port from an original component in the clone map
     ...
     *
     * @param entry an original entry in the clone map
     * @param cloneMap a map of original components to cloned components
     *          and entries
     */
    ...
    /**
     * Returns true if this module has been fully constructed
     * or false if only the superclass constructor or clone
     * has been called.  Actually tests whether the {@link InBuf}
     * exists and is owned by this module.  If false,
     * {@link Module#makeDataPort()} will default to
     * {@link Component#makeDataPort()} and no {@link InBuf}
     * data buses will be created.
     *
    ...
    /**
     * Creates the {@link InBuf} with a data {@link Bus} for each
     * data {@link Port} on this Module.
     */
    private void createInBuf ()
    {
        ...
        /*
         * Create an InBuf and map all the module's ports
         * to its buses.
         */
