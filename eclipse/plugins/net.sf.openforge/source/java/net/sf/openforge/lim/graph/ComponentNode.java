...
/**
 * The {@link Node} that is created for a generic {@link Component} in
 * an {@link LXGraph}.  In addition to itself, the ComponentNode may create
 * additional nodes, one for each {@link Exit}, if the {@link Component} has
 * more than one.  These nodes can be retrieved with {@link ComponentNode#getExitNodes()}
 * so that they may also be added to the containing {@link Graph}.
 *
...
class ComponentNode extends Record
{
    ...
    private Collection exitNodes = new LinkedList();
     ...
     *
     * @param component the component to be represented
     ...
     * Nodes are created for each and are obtained with {@link ComponentNode#getExitNodes()}.
     */
    ...
    /**
     * Gets the external Nodes for the Exits, which are created if there is
     * more than one Exit on the component.
     */
    Collection getExitNodes ()
    {
        return exitNodes;
    }
    ...
    protected String getBodyLabel ()
    {
        ...
        if (component instanceof SimplePinRead || component instanceof SimplePinWrite)
        {
            labelBuf.append("\\n");
            labelBuf.append(((SimplePin)((Referencer)component).getReferenceable()).getName());
        }
        if (component instanceof Call || component instanceof Pin || component instanceof SimplePin || component instanceof Reg)
        {
            labelBuf.append("\\n");
    ...
    /**
     * Tests whether or not it is necessary to graph any Ports.  By default, this is
     * true only if there is at least one data port or a connected go port.
     */
    protected boolean needPortGraph ()
    {
        return !component.getDataPorts().isEmpty() || component.getGoPort().isConnected();
    }
    ...
    protected void graphExits (Record.Port boundingBox)
    {
        ...
        {
            Collection exits = component.getExits();
            if (exits.isEmpty())
            {
                return;
            }
     ...
     *
     * @param bus the bus to be graphed
     * @param parentNode the bounding box in which the bus's node is created
     * @param id the node identifier
     * @param label the node label; the size will be appended
     */
    protected void graphBus (Bus bus, Record.Port parentNode,
     ...
     *
     * @param port the port to be graphed
     * @param parentNode the bounding box in which the port's node is created
     * @param id the node identifier
     * @param label the node label; the size will be appended
     */
    protected void graphPort (net.sf.openforge.lim.Port port, Record.Port parentNode,
        String id, String label)
    {
        if (!DO_SIDEBAND && port.getTag() == net.sf.openforge.lim.Component.SIDEBAND)
        {
            return;
        }
