...
/**
 * A ShiftOp is a shift operation, based on a {@link BinaryOp},
...
//         assert (r_bus != null) : "Right operand port in math operation not attached to a bus.";
    ...
    {
        /**
         * XXX FIXME.  If any bit of the shift magnitude is a constant
         * value then it needs to NOT create the Mux for that stage,
         * instead, just wire through the previous wire or shifted
         * wire based on the constant value.
         */
        public Left(LeftShiftOp op)
        {
            ...
            assert (totalShiftStage <= op.getMaxStages()) : "too many shift stages: " + totalShiftStage + ":" + op.getMaxStages();
            ...
            {   
                ...
                for (int stageCount = 1; stageCount <= totalShiftStage; stageCount++)
                {
                    ...
                    Expression conExp = right_operand.getBitSelect(totalShiftStage-stageCount);
                    ...
                    if (shiftAmount <= maxShift)
                    {
                        ...
                        concat.add(prevStageWire.getRange(maxShift-shiftAmount, 0));
                        concat.add(new HexNumber(new HexConstant(0, shiftAmount)));
                        leftExp = concat;
                    } else
                    {
                     ...
                     */
                    Expression stageExp = new Conditional(new Group(conExp), leftExp, rightExp);
    
                    if(stageCount!=totalShiftStage)
                    {
                        currentStageWire=new Wire(ID.toVerilogIdentifier(ID.showLogical(op.getResultBus()) + 
                            "_stage_" + stageCount),
                            maxShift + 1);
    
                        produced_nets.add(currentStageWire);
                        prevStageWire = currentStageWire;
                        add(new Assign.Continuous((NetLValue)currentStageWire, stageExp));
                    }
                    else
                    {
                        add(new Assign.Continuous((NetLValue)result_wire, stageExp));
                    }
                }
            }
            else
            {
                // right_operand is a constant, so just re-wire the left operand by
                ...
                if (maxShift > shiftAmount)
                {
                    ...
                    rewire.add(prevStageWire.getRange(maxShift-shiftAmount, 0));
    ...
    public static final class Right extends ShiftOp
    {
        ...
        public Right(RightShiftOp op)
        {
            ...
            // add the left and right operands so the declarations will get written out
            ...
            if (!r_value.isConstant())
            {
                ...
                for (int stageCount = 1; stageCount <= totalShiftStage; stageCount++)
                {
                    ...
                    {
                        Concatenation concat  = new Concatenation();
                        concat.add(new Replication(shiftAmount, prevStageWire.getBitSelect(maxShift)));
                        concat.add(prevStageWire.getRange(maxShift, shiftAmount));
                        leftExp = concat;
                    } 
                    else
                    {
                        leftExp = new Replication(maxShift + 1, prevStageWire.getBitSelect(maxShift));
                    }
                    Expression rightExp = prevStageWire;
                    ...
                    if(stageCount!=totalShiftStage)
                    {
                        // need to size the intermediate stages larger than the result bus to handle high bits
                        //int size=resultSize+(totalShiftStage-stageCount)+1;
                        int size = stageExp.getWidth(); // ABK -- the result size of each stage should
                                                        // simply match the size of the stage expression
                        ...
                        prevStageWire = currentStageWire;
                        add(new Assign.Continuous((NetLValue)currentStageWire, stageExp));
                        // set maxShift to work with current size of prevStageWire
                    ...
                    else
                    {
                        ...
                        {
                            currentStageWire=new Wire(ID.toVerilogIdentifier(ID.showLogical(
                                op.getResultBus()) + "_stage_" + stageCount),
                                                  stageExp.getWidth());
                            produced_nets.add(currentStageWire);
                            add(new Assign.Continuous((NetLValue)currentStageWire, stageExp));
                            add(new Assign.Continuous((NetLValue)result_wire, 
                                currentStageWire.getRange(result_wire.getWidth()-1, 0)));
                        } 
                        else
                        {
                            add(new Assign.Continuous((NetLValue)result_wire, stageExp));
            ...
            else
            {
                ...
                if (maxShift > shiftAmount)
                {
                    ...
                    if ((shiftAmount + resultSize) < maxShift) top_bit = shiftAmount + resultSize;
                    ...
                    if ((maxShift - shiftAmount) < resultSize)
                        if (extra_bits > 0)
                        {
                            rewire.add(new Replication(extra_bits, prevStageWire.getBitSelect(maxShift)));
                        } 
                ...
                else
                {
                    ...
                    add(new Assign.Continuous((NetLValue)result_wire, prevStageWire.getBitSelect(maxShift)));
                }
            }
            ...
            else
            {
                ...
                if (maxShift > shiftAmount)
                {
                    ...
                    if ((maxShift - shiftAmount) < resultSize)
                        if (extra_bits > 0)
                        {
                            rewire.add(new HexNumber(new HexConstant(0, extra_bits)));
                        } 
                    rewire.add(prevStageWire.getRange(top_bit, shiftAmount));
                    add(new Assign.Continuous((NetLValue)result_wire, rewire));
                }
