 ...
 * {@link Component Components} that are logically executed
 * one after the other.
 *
...
{
    private static final String rcs_id = "RCS_REVISION: $Rev: 100 $";
     ...
     * Constructs a Block.
     *
     * @param sequence a list of Components in logical order of execution
     * @param isProcedureBody true if this is a {@link Procedure} body, false
     *          otherwise; the {@link Exit Exits} of a body are merged
    ...
    public SearchLabel getSearchLabel()
    {
    	...
    	{    	
    		...
    		{
    			EngineThread.getEngine().fatalError("Internal Error. Invariant on Block not met.");
    		}
    	}    	
    	assert sl!=null;    	
    	return sl;    	
     ...
     *
     * @param procedure the Procedure owner of this block 
     */
    ...
    {
        return ((isProcedureBody()) ? (Configurable)getProcedure() : (Configurable)getOwner());
    }
     ...
     * @param component the list of components
     * @param offset where in the sequence to insert the first element of components
     * @return true if succesful, else false
     */
    public boolean insertComponents (List components, int offset)
    {
        for (ListIterator iter=components.listIterator(components.size()); iter.hasPrevious();)
        {
     ...
     * @param offset where in the sequence to insert the component
     * @return true if succesful, else false
     */
    ...
    /**
     * Replaces a list of components with another list.
     * the user must take care of dependencies
     * assumes that each element in the sequence is unique, and that elements in the lists are sequential
     * @param remove list of components to be removed
     * @param insert list of components to insert instead
     * @return true if successful, else false
     */
    public boolean replaceComponents (List remove, List insert)
    {
        int offset=sequence.indexOf(remove.get(0));
        if (offset == -1 || !removeComponents(remove) || !insertComponents(insert, offset))
        {
            return false;
        }
        return true;
    ...
    public boolean replaceComponent (Component remove, Component insert)
    {
        ...
        if (offset == -1 || !removeComponent(remove) || !insertComponent(insert, offset))
        {
            return false;
        }
        return true;
     ...
     * of the children of this Block.  Also creates the {@link Exit Exits} for this
     * Block based upon the {@link Exit Exits} of the components in its sequence.
     */
    protected void setControlDependencies (boolean isProcedureBody)
    {
        /*
         * Assume the inputs to the Block have already been connected.  Start
         * on the inside with the InBuf.  Use its clock, reset, and go Buses
        ...
        for (Iterator iter = components.iterator(); iter.hasNext();)
        {
            ...
            /*
             * If the Component has multiple Exits or has no DONE Exit, then
             * it represents a change in the flow of control. Special action
             * is required!
             */
            if ((component.getExits().size() > 1) || (component.getExit(Exit.DONE) == null))
            {
                /*
                ...
                else
                {
                     ...
                     */
                    assert (components.indexOf(component) == components.size() - 1)
                    : "not at the end of a block: " + component;
                }
            }
        ...
        /*
         * Merge the non-DONE Exits of each Tag into a single Exit at the Block level.
         */
        final Collection doneExits = (Collection)exitMap.remove(Exit.getTag(Exit.DONE));
         ...
         * DONE Exit, then that is a possible normal exit from the Block.  Make
         * a DONE Exit that depends on all of these Exits.
         */
        if (doneExits != null)
        {
            ...
            outBufEntry.addDependency(outBuf.getClockPort(), new ClockDependency(clockBus));
            outBufEntry.addDependency(outBuf.getResetPort(), new ResetDependency(resetBus));
            for (Iterator iter = doneExits.iterator(); iter.hasNext();)
            {
                final Exit doneExit = (Exit)iter.next();
                outBufEntry.addDependency(outBuf.getGoPort(), new ControlDependency(doneExit.getDoneBus()));
            }
        }
        else if (components.isEmpty())
        {
             ...
             */
            addDependencies(makeExit(0, mainExitType).getPeer().makeEntry(drivingExit), clockBus,
                resetBus, goBus);
        }

        /*
         * If a procedure body has explicit RETURN Exits, merge them into
         * the main Exit of the Block.
         */
        if (isProcedureBody && (returnExits != null))
        {
            exitMap = Collections.singletonMap(Exit.getTag(mainExitType), returnExits);
            mergeExits(exitMap, clockBus, resetBus);
        }
    }
