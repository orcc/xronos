...
/**
 * Variable is an implementation of {@link Location} which describes
 * a memory region that is named by a variable in the user application.
 *
...
public abstract class Variable extends ID implements Location
{
     ...
     *
     * @param logicalMemory the memory to which this variable refers
     * @param size the size in addressable units of the variable
     ...
     *
     * @return the number of continguous addressable units in memory
    ...
    {
        return ((size == getAddressableSize()) && (delta == 0)) ? this : new Offset(size, this, delta);
    }
     ...
     * number of addressable units from this location; may return 
     ...
     *
     * <p>requires: Location.getBaseLocation() never null
     * <p>modifies: none
     * <p>effects: returns the root Location
     *
    ...
    /**
     * Determines if the parameter {@link Location} represents a
     * region of memory that is covered by this Location.
     * 
     * <p>requires: none
     * <p>modifies: none
     * <p>effects: returns true if loc overlaps this Location, false
     * otherwise 
     *
     * @param loc a Location
     * @return true if the parameter Location represents a region of
     * memory that is covered by this Location.
     * @throws IllegalArgumenException if 'loc' is null
     */
    public boolean overlaps (Location loc)
    {
        if (loc == null)
        {
            throw new IllegalArgumentException("Null location cannot test for overlap");
        }
    ...
    /**
     * Does nothing except throw a NullPointerException if
     * <code>loc</code> is null, or an IllegalArgumentException if
     * units is < 0.
     */
    ...
    /**
     * This method generates or retrieves a 'correlated' Location for
     * the given Location, meaning that the correlated Location has
     * exactly the same structure and 'base' hierarchy as the given
     * Location, but is based off of a different base Location as
     * specified by the correlation Map.  The 'base' hierarchy of the
     * given 'oldLoc' Location is traversed until a Location is found
     * that is a valid key into the correlation map.  The value stored
     * in the map for that key is the new base on which to build the
     * correlated Location.  From that new base a sequence of Offset
     * and Index Locations is created that parallels the structure
     * found in the old location.  Thus the returned correlated
     * Location is identical to the old location in every respect
     * except for the 'base' location.
     *
     * <p>The Map and oldLoc are not modified by this method.
     * 
     * @param correlation a non-null Map of Location to Location
     * @param oldLoc a non-null Location
     * @return a correlated Location
     * @throws NullPointerException if oldLoc is null
     * @throws IllegalArgumentException if at least one Location of
     * the 'base' hierarchy of 'oldLoc' is not contained in
     * correlation.  Or thrown if any Location is found in which the
     * maxDelta is less than the minDelta.
     */
    public static Location getCorrelatedLocation (Map correlation, Location oldLoc)
    {
        if (oldLoc == null)
        {
            throw new NullPointerException("Null Location not allowed in this context");
        }
        ...
        if (correlatedBase == null)
        {
            ...
            {
                throw new IllegalArgumentException("Correlation map for location correlation does not "
                    + "contain a necessary base location.");
            }
            else
            {
                correlatedBase = getCorrelatedLocation(correlation, baseLocation);
            }
        }
