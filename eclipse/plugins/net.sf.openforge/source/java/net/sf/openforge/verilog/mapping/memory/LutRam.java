...
{
    private static final String rcs_id = "RCS_REVISION: $Rev: 443 $";
    ...
    public StatementBlock initialize ()
    {
        ...
        initial_block.add(new InlineComment(" Initialization of LUT ram is accomplished through explicit parameter setting.", Comment.SHORT));
        ...
        else
        {
            ...
            for (int i=0; i<getWidth(); i++)
            {
                ...
                if (i < dataAttachWidth)
                {
                    if (!datScalar)
                    {
                        di_exp = dataIn.getRange(dataInStartBit + i, dataInStartBit + i);
                    }
                    else
                    {
                        dataIn = new Wire(dataInName, 1);
            ...
            for (int i=0; i<getWidth(); i++)
            {
                ...
                if (i < dataAttachWidth)
                {
                    if (!datScalar)
                    {
                        do_exp = dataOut.getRange(dataOutStartBit + i, dataOutStartBit + i);
                    }
                    else
                    {
                        dataOut = new Wire(dataOutName, 1);
