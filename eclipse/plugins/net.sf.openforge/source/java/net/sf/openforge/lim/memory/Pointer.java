...
/**
 * A Pointer is a {@link LogicalValue} which refers to a {@link Location}
 * in a {@link LogicalMemory}.
 *
 * @version $Id: Pointer.java 556 2008-03-14 02:27:40Z imiller $
 */
public class Pointer implements LogicalValue, MemoryVisitable, LocationValueSource
{
    /** Revision */
     ...
     *
     * @param target the location referenced by this pointer; null if
     *        this is a null pointer
     * @param policy the particular addressing policy used to govern
     * how this value is constructed
     */
    ...
    /**
     * Constructs a null <code>Pointer</code>.  A target may be
     * set later with {@link Pointer#setTarget(Location)}.
     */
    public Pointer ()
    {
        this((Location)null);
    ...
    /**
     * Gets the bitwise representation of this value, which is the
     * address of the target location in the containing memory.
     *
...
//         assert rep.length <= 4 : "Pointer size too wide";
        ...
        if (EngineThread.getGenericJob().getUnscopedBooleanOptionValue(OptionRegistry.LITTLE_ENDIAN))
        {
            return rep;
        }
     ...
     *
     * @param location the symbolic location which this pointer references
     */
    ...
    /**
     * Returns a new LogicalValue object (Pointer) which points to the
     * same Location object as the current LogicalValue.
     *
     * @return a new LogicalValue which points to the same Location
     * object.
     */
    ...
    /**
     * Throws a NonRemovableRangeException because no portion of a
     * Pointer may be eliminated while still preserving the validity
     * of the pointer.  
     *
     * @param min the offset of the least significant byte of the
     * range to delete.  0 based offset.
     * @param max the offset of the most significant byte to of the
     * range to delete.  0 based offset.
    ...
    {
        throw new NonRemovableRangeException("No portion of a pointer may be eliminated");
    }
    ...
    {
        return new LocationConstant(getTarget(), getBitSize(), this.getAddressStridePolicy());
    }
    ...
    {
        return "Pointer<" + Integer.toHexString(hashCode()) + ">->" + getTarget();
    }
