 ...
 * instantiating a Block ram based implementation for either dual
 * ported RAM or dual ported ROM.  All accesses (read or write) take 1
 * clock cycle.
 *
...
public class DualPortBlockWriter extends DualPortWriter
{
    ...
    public Module defineModule ()
    {
        ...
        if (extra_address_bits > 0)
        {
            if (mpA.reads())
            {
                // We need an output mux
                // ABKTODO: the original expression used ranges...
                // EventExpression event_list = new EventExpression(mpA.adr.getRange(getAddrWidth()-1, getAddrWidth()-extra_address_bits));
                ...
                CaseBlock case_block = new CaseBlock(mpA.adr.getRange(getAddrWidth()-1, getAddrWidth()-extra_address_bits));
                for (int d = 0; d < result_depth; d++)
                {
                    Decimal case_value = new Decimal(d, extra_address_bits);
                    case_block.add(case_value.toString(), new Assign.Blocking(mux_outa, pre_douta[d]));
                }
                HexConstant unknown = new HexConstant("0", getDataWidth());
                case_block.add("default", new Assign.Blocking(mux_outa, new HexNumber(unknown)));
            ...
            {    
                // We need an output mux
                // ABKTODO: again the original used ranges in the event expression...
                // EventExpression event_list = new EventExpression(mpB.adr.getRange(getAddrWidth()-1, getAddrWidth()-extra_address_bits));
                EventExpression event_list = new EventExpression(mpB.adr);
                ...
                CaseBlock case_block = new CaseBlock(mpB.adr.getRange(getAddrWidth()-1, getAddrWidth()-extra_address_bits));
                for (int d = 0; d < result_depth; d++)
                {
                    Decimal case_value = new Decimal(d, extra_address_bits);
                    case_block.add(case_value.toString(), new Assign.Blocking(mux_outb, pre_doutb[d]));
                }
                HexConstant unknown = new HexConstant("0", getDataWidth());
                case_block.add("default", new Assign.Blocking(mux_outb, new HexNumber(unknown)));
        ...
        else
        {
            if (mpA.reads())
            {
                ...
                for (int d = 0; d < result_depth; d++)
                {
                    pre_dout_event[d] = new EventExpression(pre_douta[d]);
                }
                ...
                sequential_block.add(new Assign.NonBlocking(mux_outa, pre_douta[0]));
                Always always_block = new Always(new ProceduralTimingBlock(pre_dout_ec, sequential_block));
            ...
            if (mpB.reads())
            {
                ...
                EventControl pre_dout_ec = new EventControl(new EventExpression(pre_dout_event));
                SequentialBlock sequential_block = new SequentialBlock();
                sequential_block.add(new Assign.NonBlocking(mux_outb, pre_doutb[0]));
                Always always_block = new Always(new ProceduralTimingBlock(pre_dout_ec, sequential_block));
                memoryModule.state(always_block);
            }
        }
        */
        ...
        SinglePortRamWriter.mergeResults(extra_address_bits, result_depth,
            ...
            pre_douta, mux_outa, memoryModule, mpA.adr, clkPort, readLatency,
            registerLutRead, "A");
        
        SinglePortRamWriter.mergeResults(extra_address_bits, result_depth,
            getAddrWidth(), getDataWidth(),
            pre_doutb, mux_outb, memoryModule, mpB.adr, clkPort, readLatency,
            registerLutRead, "B");
        ...
        if (mpA.writes() && mpA.reads())
            memoryModule.state(new Assign.Continuous(mpA.done, new Bitwise.Or(weaDone, reaDone)));
        else if (mpA.writes())
        ...
        if (mpB.writes() && mpB.reads())
            memoryModule.state(new Assign.Continuous(mpB.done, new Bitwise.Or(webDone, rebDone)));
        else if (mpB.writes())
