...
/**
 * OffsetMemoryRead is an {@link OffsetMemoryAccess} for a read of memory.
 * It adds a data {@link Bus} that provides the result of the memory read.
 *
...
public abstract class OffsetMemoryRead extends OffsetMemoryAccess
{
     ...
     *
     * @param memoryRead the underlying {@link MemoryRead} for
     *          this access.
     ...
     * address bus 
     */
    protected OffsetMemoryRead (MemoryRead memoryRead, int addressableLocations, int maxAddressWidth)
    {
        super(memoryRead, addressableLocations, maxAddressWidth);
        
        final Bus readBus = (Bus)memoryRead.getExit(Exit.DONE).getDataBuses().get(0);
    ...
    protected void cloneNotify (Module moduleClone, Map cloneMap)
    {
        ...
        clone.resultBus = ((resultBus == null)
            ? null
            : getBusClone(resultBus, cloneMap));
    }
}
