...
/**
 * <code>Slice</code> is a {@link LogicalValue} that denotes a contiguous portion
 * of another source {@link LogicalValue}.
 *
...
public class Slice implements LogicalValue
{
     ...
     *
     * @param targetValue the value from which this slice is taken
     * @param delta the offset in addressable units from the start of
     * <code>targetValue</code> at which the slice begins
     * @param size the size in addressable units of the slice
     */
    ...
    /**
     * Returns the address stride policy governing this slice,
     * deferring to the policy of the base logical value being
     * sliced. 
     */
    ...
    /**
     * Returns the number of addressable units offset from the source
     * logical value that this slice selects.
     *
    ...
    public LogicalValue removeRange (int min, int max) throws NonRemovableRangeException
    {
        ...
        else
        {
            ...
            elements.add(targetValue.getValueAtOffset((delta + max + 1), (getSize() - max - 1)));
...
/**
 * <code>SliceConstant</code> here is a constant that derives its value from
 * a slice of another target {@link MemoryConstant}.
 *
 * @version $Id: Slice.java 568 2008-03-31 17:23:31Z imiller $
 */
class SliceConstant extends MemoryConstant
{
     ...
     * <code>sourceConstant</code> or a component of it may be
     * deferred, the value representation is re-built each time that
     * the getRepBundle method is called, and consequently, only known,
     * fixed constant bits are propagated out to the bus on each call
     * to pushValuesForward.
     *
     * @param sourceConstant the constant from which the slice is
     * taken
     * @param delta the offset in units from the start of the source
     * constant
     * @param size the size in units of the slice
     */
    ...
    {
        return Collections.unmodifiableList(Collections.singletonList((Constant)this));
    }

    /**
     * Returns a non modifiable set containing this object as well as
     * the backing Constant.
     *
    ...
    /**
     * Returns the bundle defining the state of this constant.
     * Because the backing constants may or may not be fixed at any
     * point in the compilation, the AURep array is re-built on each
     * call to this method.  Thus any changes (locking) of any backing
     * constant will be propagated to this constants representation on
     * the next call.
     *
