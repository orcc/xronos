...
public class SinglePortRamWriter extends VerilogMemory 
{
    ...
    public SinglePortRamWriter (MemoryBank memory)
    {
        ...
        //this.moduleName = "forge_memory_" + getDepth() + "x" + getDataWidth() + "_"+ memory_module_id++;
    ...
    public Module defineModule ()
    {
        ...
        int result_width = (int)java.lang.Math.ceil((double)getDataWidth()/(double)match.getWidth());
        ...
        for (int w = 0; w < result_width; w++)
        {
            for (int d = 0; d < result_depth; d++)
            {
                ...
                // them all to it.
                new_element.setAddr("ADDR", getAddrWidth(), 0, getAddrWidth() == 1);
                ...
                else
                {
                    ...
                    new_element.setDataAttachWidth(new_element.getWidth() - spareBits);
        ...
        for (int d = 0; d < result_depth; d++)
        {
            ...
            if ((extra_dout_wires[d] != null) && (extra_dout_wires[d].getIdentifier().getToken().length() > 0))
            {
        ...
        for (int d = 0; d < result_depth; d++)
        {
            if (result_depth > 1)
            {
                ...
                Expression addr_equal = new Compare.Equals(adrPort.getRange(getAddrWidth()-1, getAddrWidth()-extra_address_bits), hex_d);
                Expression we_and_addr_equal = new Bitwise.And(wenPort, new Group(addr_equal));
                memoryModule.state(new Assign.Continuous(we[d], we_and_addr_equal));
            }
            else
            {
        ...
        final boolean registerLutRead = (
            (getMemBank().getImplementation().getReadLatency().getMinClocks() > 0) &&
            getMemBank().getImplementation().isLUT());
        mergeResults (extra_address_bits, result_depth,
            getAddrWidth(), getDataWidth(),
            pre_dout, mux_out, memoryModule,
            this.adrPort, clkPort, getReadLatency(), registerLutRead, "");
        ...
        if(getReadLatency().getMinClocks() == 0)
        {
            memoryModule.state(new Assign.Continuous(donePort, new
                Bitwise.Or(new Bitwise.And(renPort,new Unary.Not(wenPort)), wenDone)));
        }
        else
        {
            memoryModule.state(new Assign.Continuous(donePort, new
                Bitwise.Or(renDone,wenDone)));
        }
    ...
    /**
     * This code generates the necessary logic to 'merge' the result
     * values from each 'stacked' instantiated ram by muxing them
     * together based on the MSB bits of the address.  Note, however
     * that if the read latency of the memory is > 0 we must delay the
     * address by the correct number of cycles so that it aligns with
     * the data coming out.  This also breaks combinational paths
     * through the memory.
     *
     * @param extra_address_bits a value of type 'int'
     * @param result_depth a value of type 'int'
     * @param pre_dout a value of type 'Net[]'
     * @param mux_out a value of type 'Register'
     * @param memoryModule a value of type 'MemoryModule'
     */
    public static void mergeResults (int extra_address_bits, int result_depth,
        int adrWidth, int dataWidth,
        Net[] pre_dout, Register mux_out,
        MemoryModule memoryModule, Net adrPort,
        Net clkPort, Latency readLatency, boolean insertReadReg, String portId)
    {
        Net adrNet = adrPort;
        if (readLatency.getMinClocks() > 0)
        {
            ...
            if (readLatency.getMinClocks() != 1)
                throw new UnsupportedOperationException("Memory cannot have read latency > 1");

            Register adrReg = new Register(adrPort.getIdentifier().toString() + "_reg", adrPort.getWidth());
            ...
            Always always = new Always(new ProceduralTimingBlock(clkEvent, seqBlock));
        ...
        if (insertReadReg)
        {
            ...
            for (int i=0; i < pre_dout.length; i++)
            {
                ...
                Register reg = new Register(inNet.toString() + "_reg", inNet.getWidth());
        ...
        if (result_depth > 1)
        {
            ...
            CaseBlock case_block = new CaseBlock(adrNet.getRange(adrWidth-1, adrWidth-extra_address_bits));
            for (int d = 0; d < result_depth; d++)
            {
                Decimal case_value = new Decimal(d, extra_address_bits);
                case_block.add(case_value.toString(), new Assign.Blocking(mux_out, pre_dout[d]));
            }
            HexConstant unknown = new HexConstant("0", dataWidth);
            case_block.add("default", new Assign.Blocking(mux_out, new HexNumber(unknown)));
            
            SequentialBlock sequential_block = new SequentialBlock(case_block);
            
            Always always = new Always(new ProceduralTimingBlock(event_control, sequential_block));
            memoryModule.state(always);
        }
        else
        {
            ...
            EventControl pre_dout_ec = new EventControl(new EventExpression(pre_dout_event));
            SequentialBlock sequential_block = new SequentialBlock();
            sequential_block.add(new Assign.NonBlocking(mux_out, pre_dout[0]));
            memoryModule.state(new Always(new ProceduralTimingBlock(pre_dout_ec, sequential_block)));
        }
    ...
    public ModuleInstance instantiate (MemoryBank bank)
    {
        ...
        //MemoryBank.BankPort bankPort = (MemoryBank.BankPort)getMemBank().getBankPorts().get(0);
        MemoryBank.BankPort bankPort = (MemoryBank.BankPort)bank.getBankPorts().get(0);
...
//         if (donePort != null)
//         {
//             Net doneOutWire = new BusWire(bankPort.getExit(com.xilinx.hllc.lim.Exit.DONE).getDoneBus());
