...
/**
 * LogicalMemory is a symbolic representation of a memory space in
 * a {@link Design}.  The two primary attributes of a LogicalMemory
 * are its <i>allocations</i> and <i>accesses</i>.
 * <P>
 * An {@linkplain Allocation allocation} is a region of memory that is
 * defined by the program via a variable declaration.  Each allocation
 * has associated with it a size (the number of bytes needed to represent
 * the variable's type) and an {@linkplain LogicalValue initial value}.
 * Allocations may be added to and deleted from a LogicalMemory, as
 * well as queried.  Together the allocations represent the memory
 * space that is legally usable by the application.
 * <P>
 * An <i>access</i> is a read or write of LogicalMemory.  Accesses are
 * denoted by {@link LValue LValues}, each of which is either a read
 * or a write.  The LogicalMemory can record all of the
 * {@link Location Locations} that are referenced by a given {@link LValue},
 * and determine whether a given {@link Location} refers to all, part, or
 * none of an existing allocation.
 * <P>
 * Each accessing {@link LValue} is also associated with a
 * single {@link LogicalMemoryPort}, through which all the {@link LValue LValue's}
 * accesses to memory are mediated.  Accesses are added using
 * {@link LogicalMemoryPort#addAccess(LValue,Location)}.
 * New {@link LogicalMemoryPort LogicalMemoryPorts} may be created as needed
 * to represent the ports of multi-port memories.  If a {@link LogicalMemoryPort}
 * is removed from its memory, all of its accesses will be discarded along
 * with it.
 * <P>
 * To support memory optimizations, a LogicalMemory supports the addition
 * and removal of both {@link Allocation Allocations} and {@link LValue}
 * accesses, so that either may be moved among an arbitrary set of
 * LogicalMemory instances.
 * <P>
 * At any time, the method {@link #getSize()} may be called to determine
 * the number of addressable locations needed to create a physical
 * instantiation of a memory that can support the current allocation set.
 *
 * @version $Id: LogicalMemory.java 538 2007-11-21 06:22:39Z imiller $ 
 */
public class LogicalMemory extends net.sf.openforge.util.naming.ID implements MemoryVisitable, StateHolder, Referenceable
{
    /** Revision */
    private static final String _RCS_ = "$Rev: 538 $";

    /**
     * The width of the 'size' port on any memory in a design. 2 bits
     * is sufficient to cover all accesses up to a 64 bit wide memory
     * accessed as bytes.  HOWEVER, to ensure that the size is always
     * seen as a positive value it must be represented as 3 bits.
     */
    ...
    private Set logicalMemoryPorts;

    /**
     * Map of Location to Integer identifying the numeric base
     * address of the given Location.  Given in terms of bytes offset
     * from location 0.
     */
    private Map baseAddressMap = Collections.EMPTY_MAP;

    /**
     * The structural implementation of this memory.  Set by the
     * MemoryBuilder
     */
    private StructuralMemory structMem = null;

    /**
     * The specific implementation that identifies the types of
     * resources that will be used to build this memory.
     */
    private MemoryImplementation implementation = null;
    
    /**
     * Max number of bits in an address of this memory (language specific,
     * since really it is the size of a pointer).
     */
     ...
     * {@link Allocation Allocations}.  This object is constructed
     * using a maximum address width; the actuall width of all
     * address buses will eventually be downsized from this width
     * by constant propagation.
     *
     * @param maxAddressWidth the maximum number of bits needed to address
     *          this memory; should be the size of a pointer in the
     *          source language; must be at least 1
     *
     * @throws IllegalArgumentException if <code>maxAddressWidth</code> is
     *           less than 1
     */
    public LogicalMemory (int maxAddressWidth)
    {
        setIDLogical("Logical Memory (bits: "+maxAddressWidth+") ["+Integer.toHexString(hashCode())+"]");
        this.allocations = new LinkedHashSet();
        this.logicalMemoryPorts = new HashSet();

        if (maxAddressWidth < 1)
        {
            throw new IllegalArgumentException("invalid address width: " + maxAddressWidth);
        }
    ...
    /**
     * Gets the {@link LogicalMemoryPort LogicalMemoryPorts} that have been created
     * on this LogicalMemory.
     *
     ...
     *
     * @param logicalMemoryPort the memory port to remove
     *
     * @throws NullPointerException if <code>logicalMemoryPort</code> is null
     * @throws IllegalArgumentException if <code>logicalMemoryPort</code> does
     *           not belong to this LogicalMemory
     */
    public void removeLogicalMemoryPort (LogicalMemoryPort logicalMemoryPort)
    {
        ...
        for (Iterator iter = new ArrayList(getLValues()).iterator(); iter.hasNext();)
        {
    ...
    /**
     * Gets the {@link LogicalMemoryPort}, if any, that is accessed by
     * a given {@link LValue}.
     *
     * @param lvalue an {@link LValue} that references a {@link LogicalMemoryPort}
     *          of this memory
    ...
    public LogicalMemoryPort getLogicalMemoryPort (LValue lvalue)
    {
        ...
        final Accessor_ accessor = (Accessor_)this.accessors.getForLValue(lvalue);
        return accessor == null ? null : accessor.getLogicalMemoryPort();
    }


    /**
     * Defines the {@link StructuralMemory} constructed to implement
     * this LogicalMemory.
     */
    ...
    /**
     * Retrieves the {@link StructuralMemory} constructed to implement
     * this LogicalMemory as defined by the {@link MemoryBuilder}.
     */
    ...
    /**
     * Defines the type of resources that will be used in the
     * construction of this memory, which in turn identifies the
     * access characteristics to this memory.
     */
    ...
    /**
     * Returns the selected implementation for this memory, may be
     * null if not yet identified by the {@link MemoryBuilder}
     */
    ...
    /**
     * Allocates a new region of memory that is accessible to the
     * user application.  The {@link Allocation} that is returned may
     * be used to uniquely identify the new allocation within this memory.
     *
     * @param initialValue the initial value of the region of memory;
     * the region that is allocated will be the size in bytes of the
     * this value, i.e.
     * {@link LogicalValue#getAddressableSize() initialValue.getAddressableSize()}
     * @return a new allocation instance
     *
     * @throws NullPointerException if <code>initialValue</code> is null,
     * @throws IllegalArgumentException if
     *         {@link LogicalValue#getSize() initialValue.getSize()} returns a
     *         negative number
     */
    ...
    /**
     * Removes an {@link Allocation} that was previously created in this
     * memory with {@link #allocate(LogicalValue)}.
     *
     * @param allocation an {@link Allocation allocation} that was previously
     *          created in this memory
     *
     * @throws NullPointerException if <code>allocation</code> is null
     * @throws IllegalArgumentException if <code>allocation</code> is
     *           unknown by this memory
     */
    ...
    /**
     * Gets the {@link Allocation allocations} that are currently defined
     * in this memory.
     *
     * @return the collection of {@link Allocation allocations} that have
     *         been created (but not deleted) in this memory
     */
    ...
    /**
     * Gets the {@link LValue LValues} for which accesses to one or
     * more {@link Location Locations} in this memory are currently
     * recorded.
     *
     * @return a collection of {@link LValue lvalues}, one for each
     *         that currently accesses a {@link Location location} in
     *         this memory
     */
    public Collection getLValues ()
    {
        return this.accessors.getLValues();
    }

    /**
     * Gets the {@link Location locations} in this memory that are
     * currently recorded as being accessed by a given {@link LValue}.
     *
     * @param lvalue an {@link LValue lvalue} expression
     * @return the collection of {@link Location Locations} that are
     *         currently recorded as being accessed by <code>lvalue</code>;
     *         this collection may be empty if <code>lvalue</code> does not
     *         access any {@link Location locations} in this memory
     * @throws NullPointerException if <code>lvalue</code> is null
     */
    public Collection getAccesses (LValue lvalue)
    {
        if (lvalue == null)
        {
            throw new NullPointerException("null lvalue");
        ...
        final Accessor_ accessor = (Accessor_)this.accessors.getForLValue(lvalue);
        return accessor == null ? Collections.EMPTY_LIST : accessor.getTargets();
    }

    /**
     * Remove the given LocationConstant as a reference to this
     * memory. 
     */
    ...
    /**
     * Remove the given Pointer as a reference to this
     * memory. 
     */
    ...
    /**
     * Removes the given LValue as an identified accessor of this
     * memory. 
     */
    ...
    /**
     * Gets the {@link LocationConstant LocationConstants} which have
     * been added to this memory.
     *
     * @return a set of {@link LocationConstant}
     */
    public Collection getLocationConstants ()
    {
     ...
     *
     * @param locationConstant the constant to be added; it represents
     *          a pointer to a {@link Location} in this memory
     *
     * @return true if <code>locationConstant</code> was added, false if
     *         was already added to this memory previously
     *
     * @throws IllegalArgumentException if <code>locationConstant</code> is
     *           null or if its target {@link Location} does not refer to
     *           this memory
     */
...
//         return locationConstants.add(locationConstant);
     ...
     *
     * @param locationConstant a previously added constant to be removed
     *
     * @return true if <code>locationConstant</code> was removed, false if
     *         it was not found
     */
    public boolean removeLocationConstant (LocationConstant locationConstant)
    {
        //System.out.println("Removing " + locationConstant + " from " + locationConstant.showOwners() + " " + this);
    ...
    /**
     * Returns an unmodifiable collection of all the Pointer objects
     * whose target Location is contained in this memory.
     *
     ...
     *
     * @return the number of bytes needed to represent the current set
     *         of {@link Allocation allocations} without taking into
     *         account extra padding that may be introduced when
     *         converting to a physical memory layout.
     */
    public int getAddressableSize ()
    {
        int totalAddresses = 0;

        for (Iterator allocIter = this.allocations.iterator(); allocIter.hasNext();)
        {
    ...
    /**
     * Returns the size of this memory in bytes, however this is an
     * imprecise calculation as it multiplies the addresses by stride
     * div 8
     */
    ...
    /**
     * Returns the {@link AddressStridePolicy} for this memory after
     * checking all allocations for consistent policies.
     *
     * @throws IllegalAddressingStateException if the address stride
     * is inconsistent for this memory.
     */
    public AddressStridePolicy getAddressStridePolicy ()
    {
        if (this.allocations.isEmpty())
            throw new IllegalAddressingStateException("Cannot determine address stride policy for empty memory");
        
        final AddressStridePolicy policy = ((Allocation)this.allocations.iterator().next()).getInitialValue().getAddressStridePolicy();
        for (Iterator iter = this.allocations.iterator(); iter.hasNext();)
        {
            final Allocation alloc = (Allocation)iter.next();
            if (!alloc.getInitialValue().getAddressStridePolicy().equals(policy))
            {
                throw new IllegalAddressingStateException("Inconsistent addressing schemes in memory " + policy.toString() + " " + alloc.getInitialValue().getAddressStridePolicy().toString());
            }
        }
    ...
    /**
     * Gets the physical byte address of a given {@link Location} in
     * this memory.
     *
     * @param location a {@link Location location} which references
     *          all or part of an {@link Allocation allocation} in this memory
     * @return the byte address denoted by <code>location</code> in this
     *         memory's physical implementation
     *
     * @throws NullPointerException if <code>location</code> is null
     * @throws IllegalArgumentException if <code>location</code> does not
     *           denote an address within one of the {@link Allocation allocations}
     *           in this memory
     */
    public int getAddress (Location location)
    {
        ...
        {
            System.err.println("LogicalMemory.baseAddressMap=" + this.baseAddressMap
                               + " is unchanged=" + (this.baseAddressMap == Collections.EMPTY_MAP));
    ...
    /**
     * Assigns a Map of Location to Integer identifying the numeric
     * value of the base address of that Location in this memory, as
     * implemented.
     *
     * @param baseMap a 'Map' of {@link Location} to Integer.
     */
    public void setBaseAddressMap (Map baseMap)
    {
     ...
     *
     * @param logicalMemoryPort a port of this memory
     * @return the (possible empty) list of {@link LValue LValues} that read
     *         from the specified port
     * @throws NullPointerException if <code>logicalMemoryPort</code> is null
     * @throws IllegalArgumentException if <code>logicalMemoryPort</code> is
     *           not a port of this memory
     */
    List getReadLValues (LogicalMemoryPort logicalMemoryPort)
    {
     ...
     *
     * @param logicalMemoryPort a port of this memory
     * @return the (possible empty) list of {@link LValue LValues} that write
     *         to the specified port
     * @throws NullPointerException if <code>logicalMemoryPort</code> is null
     * @throws IllegalArgumentException if <code>logicalMemoryPort</code> is
     *           not a port of this memory
     */
    List getWriteLValues (LogicalMemoryPort logicalMemoryPort)
    {
     ...
     *
     * @param logicalMemoryPort a port of this memory
     * @param isWrite true if the list of writing accesses are to be returned, false
     ...
     *           not a port of this memory
     */
    private List getLValues (LogicalMemoryPort logicalMemoryPort, boolean isWrite)
    {
        if (logicalMemoryPort == null)
        {
        ...
        final List lvalues = new LinkedList();
        for (Iterator iter = this.accessors.getLValues().iterator(); iter.hasNext();)
        {
            final LValue lvalue = (LValue)iter.next();
            final Accessor_ accessor = (Accessor_)this.accessors.getForLValue(lvalue);
    ...
    /**
     * Records an access in this memory.  An access consists of an
     ...
     *           to this memory or if <code>logicalMemoryPort</code> does not belong
     *           to this memory
     */
    void addAccess (LValue lvalue, Location location, LogicalMemoryPort logicalMemoryPort)
    {
    ...
    {
        if (!(from instanceof MemoryAccess)) throw new IllegalArgumentException("Wrong context for determining spacing.");
        if (!(to instanceof MemoryAccess)) throw new IllegalArgumentException("Wrong context for determining spacing.");
        ...
        boolean isCombinational = ((MemoryAccess)from).getLatency().getMinClocks() == 0;
...
//         System.out.println("read first " + isReadFirst);
        ...
        if (fromIsWrite)
        {
            if (isCombinational) // likely never true
                ...
                return 0; // done to go spacing.  Cycle offset handled by latency of from
        }
     ...
     * characteristics (eg dual port, read-first, and memory access
     * types).
     */
    ...
    private class Accessor_
    {
        ...
        protected Collection locations = new HashSet();
        ...
        void setLogicalMemoryPort (LogicalMemoryPort lmp)
        {
            ...
            if (this.memPort != lmp)
            {
                throw new IllegalArgumentException("conflicting LogicalMemoryPort");
            }
        }
        ...
        public void debug ()
        {
            System.out.println("Accessor@" + Integer.toHexString(this.hashCode()));
            System.out.println("\t" + getAccessor() + " " + getLogicalMemoryPort());
    ...
    private class SingleAccessor extends Accessor_
    {
        ...
        void addTarget (Location target)
        {
            ...
            {
                throw new IllegalArgumentException("Changing target of single target accessor");
            }
    ...
    private class AccessMap
    {
        ...
        public void remove (LValue lvalue)
        {
            ...
            final Accessor_ accessor = (Accessor_)this.lvalueAccesses.get(lvalue);
            if (accessor == null)
            {
        ...
        public void remove (Pointer pointer)
        {
            ...
            final Accessor_ accessor = (Accessor_)this.pointerAccesses.get(pointer);
            if (accessor == null)
            {
        ...
        public void remove (LocationConstant constant)
        {
            ...
            final Accessor_ accessor = (Accessor_)this.constantAccesses.get(constant);
            if (accessor == null)
            {
        ...
        public Collection getLValues ()
        {
            return Collections.unmodifiableCollection(this.lvalueAccesses.keySet());
        }

        public Collection getLocationConstants ()
        {
            return Collections.unmodifiableCollection(this.constantAccesses.keySet());
        }
        
        public Collection getPointers ()
        {
            return Collections.unmodifiableCollection(this.pointerAccesses.keySet());
        }

        public void debug ()
        {
            for (Iterator iter = lvalueAccesses.values().iterator(); iter.hasNext();)
            {
                ((Accessor_)iter.next()).debug();
            }
            for (Iterator iter = pointerAccesses.values().iterator(); iter.hasNext();)
            {
                ((Accessor_)iter.next()).debug();
            }
            for (Iterator iter = constantAccesses.values().iterator(); iter.hasNext();)
            {
                ((Accessor_)iter.next()).debug();
            }
        }
