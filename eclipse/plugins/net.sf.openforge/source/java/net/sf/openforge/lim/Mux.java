...
/**
 * A Mux accepts paired GO/Data signals, using each GO to select its related Data
 * to be provided on the Result Bus.
 *
...
public class Mux extends Primitive
{
     ...
     *
     * @param goCount  The number of mux entry pairs
     */
    ...
    */
    ...
    /**
    * Gets a Set view of all the mux entries, where each element is a Map.Entry pairing
    * of the Go/Data. The Set reflects the same ordering as the Go and Data Lists.
    */
    ...
    /**
     * Makes a new mux entry. The Go/Data pair are stored
     * in a Map with the Go as the key and the Data as the value.
     *
     * @return   the Go port
     */
    ...
    /**
     * Creates a copy of this Mux object with the same number of
     * inputs/outputs.  All ports and buses have the same attributes
     * as the original Mux.
     *
     * @return a Mux.
    ...
    /**
     * Result size is max data port width.  Bit states are dont care
     * if ALL data ports are dont care, constant if all data ports are
     * same constant and pass through if all data ports are same pass
     * through bit.  Also ensures that the result bus has had it's
     * 'bits' set via the {@link Bus#setBits} method since Muxes get
     * added during scheduling.
     */
    public boolean pushValuesForward ()
    {
        ...
        for (int i = 0; i < newValue.getSize(); i++)
        {
            ...
            for (Iterator goPortIter = getGoPorts().iterator(); goPortIter.hasNext();)
            {
                Port goPort = (Port)goPortIter.next();
                Port dataPort = getDataPort(goPort);
                Value drivingValue = dataPort.getValue();
                samePositionBits.setBit(index++,(i < drivingValue.getSize() ? drivingValue.getBit(i) : Bit.ZERO));
            }
        ...
        if (getResultBus().getValue() != null)
        {
            ...
            for (Iterator goPortIter = getGoPorts().iterator(); goPortIter.hasNext();)
            {
                Port goPort = (Port)goPortIter.next();
                Port dataPort = getDataPort(goPort);
                compactedSize = Math.min(newValue.getSize(), Math.max(compactedSize, dataPort.getValue().getCompactedSize()));
            }
    ...
    /**
     * Any constant or dont care bits on the result propagate
     * backwards to be dont care bits on all data inputs.
     */
    public boolean pushValuesBackward ()
    {
        ...
        for (Iterator iter = getGoPorts().iterator(); iter.hasNext();)
        {
            ...
            Value newValue = new Value(dataPortValue.getSize(), dataPortValue.isSigned());
    ...
    /**
     * Creates a new value to be pushed forward during partial const propagation.
     *
     ...
     *
     * @return the maximum number of inputs at any bit position which have either
     *         a non-constant select or a CARE data input
     */
     ...
     *
     * @return the number of inputs at the bit position which have either
     *         a non-constant select or a CARE data input
     */
