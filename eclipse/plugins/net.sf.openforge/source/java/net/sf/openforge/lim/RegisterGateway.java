...
/**
 * Acts as a gateway for register read/writes into and out of a
 * procedure.
 *
 * <p>The gateway has pairs of data and enable ports for each
 * RegisterWrite that exists on the local side of the gateway, and one
 * corresponding data and enable bus on the global side.  
 *
 * <p>There is also a pair of global side ports - data and enable -
 * that are wired through to the local side RegisterReads.  In the
 * static case these are straight wires, in cases where there is a
 * delay (memory), there may be delays or muxes in this area.
 * <p>ABK -- these have been removed since RegisterRead doesn't use a
 * GO signal and all reads get wired directly to the sideband data.
 * <P>
 *<pre>
...
public class RegisterGateway extends Gateway
{
    ...
    public RegisterGateway (int size, Register resource)
    {
        ...
        addComponent(mux);
        addComponent(or);
        Exit exit = makeExit(1); //one for the local->global data (ABK: reduced to 1)
        ...
        dataPort.getPeer().setSize(resource.getInitWidth(), resource.isSigned());
    }
    ...
    {
        visitor.visit(this); 
        // ABK -- well, RegisterGateways get visited for now so that they can be graphed
        // throw new UnsupportedOperationException("RegisterGateway should not be visited - see Steve for details");
    }
    ...
    /**
     * Throws a CloneNotSupportedExcpetion or asserts false since
    ...
    public Object clone () throws CloneNotSupportedException
    {
        ...
        throw new CloneNotSupportedException("Clone not implemented for RegisterGateway yet");
    }
