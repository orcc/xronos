 ...
 * each {@link Call} to be the clone of its current {@link Procedure}.  This ensures
 * that each {@link Call} has its own copy of the {@link Procedure}.
 *
...
public class Uniquifier
{
     ...
     *
     * @param design the design to be uniquified
        ...
        try
        {
            for (Iterator iter = design.getTasks().iterator(); iter.hasNext();)
            {
                ...
                if (!oldCall.getEntries().isEmpty())
                    throw new IllegalStateException("Top call has dependencies too early");
                for (Iterator it = oldCall.getPorts().iterator(); it.hasNext();)
                {
                    if (((Port)it.next()).isConnected())
                        throw new IllegalStateException("Top call port connected too early");
                }
                for (Iterator it = oldCall.getBuses().iterator(); it.hasNext();)
                {
                    Bus bus = (Bus)it.next();
                    if (!bus.getLogicalDependents().isEmpty() || bus.isConnected())
                    {
                        throw new IllegalStateException("Top call bus connected too early");
                    }
                }
...
class ReferenceCleaner extends DefaultVisitor
{
    ...
    /**
     * Unlinks all {@link Reference References} found in a hierarchy
     * of {@link Call Calls} by removing the reference from
     * its {@link Referent}.
     *
     * @param calls a collection of dead top-level {@link Call Calls}
     */
    ...
    /*
     * Pin accesses are dynamic, so they never refer directly to their referents.
     */
