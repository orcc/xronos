...
public class AddOp extends BinaryOp implements Emulatable
{
    ...
    /**
     * returns false, an indication that this add op has more than 2
     * inputs.
     */
     ...
     * this AddOp according to this rule:
     *
    ...
    public boolean pushValuesForward ()
    {
        ...
        for(int i = 0; i < newSize; i++)
        {
            ...
            else
            {
                ...
                {
                    int numOnes = 0;
                    if (bit0.isOn()) numOnes++;
                    if (bit1.isOn()) numOnes++;
                    if (carry)       numOnes++;

                    newValue.setBit(i, ((numOnes & 0x1) != 0) ? Bit.ONE:Bit.ZERO);
                    carry = numOnes > 1;
                }
                else
                {
        ...
        if (getResultBus().getValue() != null)
        {
            ...
            {
                int compactedSize = Math.min(newSize, Math.max(in0.getCompactedSize(), in1.getCompactedSize()) + 1);
    ...
    /**
     * Reverse constant prop on an AddOp simply propagates the
