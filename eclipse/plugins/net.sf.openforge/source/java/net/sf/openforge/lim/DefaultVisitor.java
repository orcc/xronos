...
/**
 * DefaultVisitor provides an implementation of the
 * Visitor interface that just traverses a LIM containment
 * hierarchy.  Subclasses that implement {@link Visitor} may
 * extend from this class and override just those methods for
 * the component types of interest; in those methods, a call
 * to <code>super.visit()</code> will continue the traversal.
 * <P>
 * If <code> _lim.db</code> is set, then a simply formatted
 * report report of the traversal is printed to the debug
 * output stream.
 *
...
public class DefaultVisitor implements Visitor
{
    ...
    /** Set to true to allow the traversal to enter {@link Composable}
        components */
    ...
    /**
     * Used to set whether this Visitor is to traverse into
     * {@link Composable} Components.
     */
    ...
    {
        if (_lim.db) writeln("Design: " + design.toString());
        ...
        {
            if (_lim.db) writer.inc();
            Task task = (Task)it.next();
            task.accept(this);
            if (_lim.db) writer.decrease();
        }
    }

    public void visit (Task task)
    {
        if (_lim.db) writeln("Task: " + task.toString());
    ...
    {
        if (_lim.db) writeln("Call: " + call.toString());
    ...
    {
        if (_lim.db) writeln("Procedure: " + procedure.toString());
        if (_lim.db)writer.inc();
    ...
    {
        if (_lim.db) writeln("Block: " + block.toString());
        genericBlockVisit(block);
    }

    public void visit (HeapRead heapRead)
    {
        if (_lim.db) writeln("HeapRead: " + heapRead.toString());
        genericBlockVisit(heapRead);
    }

    public void visit (HeapWrite heapWrite)
    {
        if (_lim.db) writeln("HeapWrite: " + heapWrite.toString());
    ...
    {
        if (_lim.db) writeln("ArrayWrite: " + arrayWrite.toString());
    ...
    {
        if (_lim.db) writeln("AbsoluteMemoryWrite: " + write.toString());
    ...
    {
        if (_lim.db) writeln("Loop: " + loop.toString());
        if (_lim.db) writer.inc();
        ...
        if (_lim.db) writer.dec();
    }

    public void visit (WhileBody whileBody)
    {
        if (_lim.db) writeln("While Body");
    ...
    {
        if (_lim.db) writeln("Decision: "+decision.toString());
    ...
    {
        if (_lim.db) writeln("Branch: "+branch.toString());
        if (_lim.db) writer.inc();
        if (_lim.db) writer.inc();
        ...
        if (_lim.db) writer.decrease();
        if (_lim.db) writeln("True: ");
        if (_lim.db) writer.inc();

        branch.getTrueBranch().accept(this);

        if (_lim.db) writer.decrease();
        if (_lim.db) writeln("False: ");
        if (_lim.db) writer.inc();
        ...
        {
            if (_lim.db) writeln("null");
        }
        
        
        if (_lim.db) writer.decrease();
        if (_lim.db) writeln("Selector: ");
        if (_lim.db) writer.inc();
        
        if (_lim.db) writer.decrease();
        if (_lim.db) writeln("Others: ");
        if (_lim.db) writer.inc();
    ...
    {
        if (_lim.db) writeln("Operation: " + add.toString());
    }
    
    public void visit (AndOp and)
    {
        if (_lim.db) writeln("Operation: " + and.toString());
    }
    
    public void visit (NumericPromotionOp numericPromotion)
    {
        if (_lim.db) writeln("Operation: " + numericPromotion.toString());
    }
    
    public void visit (CastOp cast)
    {
        if (_lim.db) writeln("Operation: " + cast.toString());
    }
    
    public void visit (ComplementOp complement)
    {
        if (_lim.db) writeln("Operation: " + complement.toString());
    }
    
    public void visit (ConditionalAndOp conditionalAnd)
    {
        if (_lim.db) writeln("Operation: " + conditionalAnd.toString());
    }
    
    public void visit (ConditionalOrOp conditionalOr)
    {
        if (_lim.db) writeln("Operation: " + conditionalOr.toString());
    }

    public void visit (Constant constant)
    {
        if (_lim.db) writeln("Operation: " + constant.toString());
    }
    
    public void visit (DivideOp divide)
    {
        if (_lim.db) writeln("Operation: " + divide.toString());
    }
    
    public void visit (EqualsOp equals)
    {
        if (_lim.db) writeln("Operation: " + equals.toString());
    }
    
    public void visit (GreaterThanEqualToOp greaterThanEqualTo)
    {
        if (_lim.db) writeln("Operation: " + greaterThanEqualTo.toString());
    }

    public void visit (GreaterThanOp greaterThan)
    {
        if (_lim.db) writeln("Operation: " + greaterThan.toString());  
    }

    public void visit (LeftShiftOp leftShift)
    {
        if (_lim.db) writeln("Operation: " + leftShift.toString());
    }
    
    public void visit (LessThanEqualToOp lessThanEqualTo)
    {
        if (_lim.db) writeln("Operation: " + lessThanEqualTo.toString());
    }
    
    public void visit (LessThanOp lessThan)
    {
        if (_lim.db) writeln("Operation: " + lessThan.toString());
    }
    
    public void visit (LocationConstant loc)
    {
        if (_lim.db) writeln("Location Constant Operation: " + loc.toString());
        visit((Constant) loc);
    }
    
    public void visit (MinusOp minus)
    {
        if (_lim.db) writeln("Operation: " + minus.toString());
    }
    
    public void visit (ModuloOp modulo)
    {
        if (_lim.db) writeln("Operation: " + modulo.toString());
    }
    
    public void visit (MultiplyOp multiply)
    {
        if (_lim.db) writeln("Operation: " + multiply.toString());
    }
    
    public void visit (NotEqualsOp notEquals)
    {
        if (_lim.db) writeln("Operation: " + notEquals.toString());
    }
    
    public void visit (NotOp not)
    {
        if (_lim.db) writeln("Operation: " + not.toString());
    }
    
    public void visit (OrOp or)
    {
        if (_lim.db) writeln("Operation: " + or.toString());  
    }
    
    public void visit (PlusOp plus)
    {
        if (_lim.db) writeln("Operation: " + plus.toString());
    }

    public void visit (ReductionOrOp reductionOr)
    {
        if (_lim.db) writeln("Operation: " + reductionOr.toString());
    }
    
    public void visit (RightShiftOp rightShift)
    {
        if (_lim.db) writeln("Operation: " + rightShift.toString());
    }
    
    public void visit (RightShiftUnsignedOp rightShiftUnsigned)
    {
        if (_lim.db) writeln("Operation: " + rightShiftUnsigned.toString());
    }

    public void visit (ShortcutIfElseOp shortcutIfElse)
    {
        if (_lim.db) writeln("Operation: " + shortcutIfElse.toString());
    }
    
    public void visit (SubtractOp subtract)
    {
        if (_lim.db) writeln("Operation: " + subtract.toString());
    }
    
    public void visit (XorOp xor)
    {
        if (_lim.db) writeln("Operation: " + xor.toString());
    }
    
    public void visit (InBuf ib)
    {
        if (_lim.db) writeln("InBuf: " + ib);
    }
    
    public void visit (OutBuf ob)
    {
        if (_lim.db) writeln("OutBuf: " + ob);
    }
    
    public void visit (Reg reg)
    {
        if (_lim.db) writeln("Reg: " + reg);
    }

    public void visit (SRL16 srl16)
    {
        if (_lim.db) writeln("SRL16: " + srl16);
    }

    public void visit (Mux m)
    {
        if (_lim.db) writeln("Mux: " + m);
    }

    public void visit (EncodedMux m)
    {
        if (_lim.db) writeln("EnabledMux: " + m);
    }
    
    public void visit (PriorityMux pmux)
    {
        if (_lim.db) writeln("PriorityMux: " + pmux);
        if (_lim.db) writer.inc();
    ...
    {
        if (_lim.db) writeln("And: " + a);
    }
    
    public void visit (Not n)
    {
        if (_lim.db) writeln("Not: " + n);
    }
    
    public void visit (Or o)
    {
        if (_lim.db) writeln("Or: " + o);
    }

    public void visit (TriBuf tbuf)
    {
        if (_lim.db) writeln("TriBuf: " + tbuf);
    }

    public void visit (Scoreboard scoreboard)
    {
        if (_lim.db) writeln("Scoreboard: " + scoreboard);
    ...
    {
        if (_lim.db) writeln("Latch: " + latch);
        if (traverseComposable)
        {
    ...
    {
        if (_lim.db) writeln("Kicker: " + kicker);
        if (traverseComposable)
        {
    ...
    {
        if (_lim.db) writeln("NoOp: " + nop);
    }

    public void visit (TimingOp timingOp)
    {
        if (_lim.db) writeln("TimingOp: " + timingOp);
    }

    public void visit (RegisterRead regRead)
    {
        if (_lim.db) writeln("RegisterRead: " + regRead);
    }
    
    public void visit (RegisterWrite regWrite)
    {
        if (_lim.db) writeln("RegisterWrite: " + regWrite);
    }
    
    public void visit (RegisterGateway gw)
    {
        if (_lim.db) writeln("RegisterGateway: "+gw.toString());
    ...
    {
        if (_lim.db) writeln("RegisterReferee: " + regReferee);
...
//         visit(regReferee.getOutBufs());
    ...
    {
        if (_lim.db) writeln("MemoryReferee: " + memReferee);
...
//         regReferee.getPriorityMux().accept(this);
    ...
    {
        if (_lim.db) writeln("MemoryBank: " + memBank);
    }
    ...
    {
        if (_lim.db) writeln("MemoryRead: " + memRead);
    ...
    {
        if (_lim.db) writeln("Memorywrite: " + memWrite);
    ...
    {
        if (_lim.db) writeln("MemoryGateway: "+mg.toString());
    ...
    {
        if (_lim.db) writeln("PinRead: " + pinRead);
    ...
    {
        if (_lim.db) writeln("PinWrite: " + pinWrite);
    ...
    {
        if (_lim.db) writeln("PinStateChange: " + pinChange);
    ...
    {
        if (_lim.db) writeln("PinReferee: " + pinReferee);
    ...
    {
        if (_lim.db) writeln("SimplePin: " + comp);
    }
    
    public void visit (TaskCall mod)
    {
        if (_lim.db) writeln("TaskCall: " + mod);
        visitGenericModule(mod);
    }
    
    public void visit (SimplePinAccess mod)
    {
        if (_lim.db) writeln("SimplePinAccess: " + mod);
        visitGenericModule(mod);
    }
    
    public void visit (SimplePinRead comp)
    {
        if (_lim.db) writeln("SimplePinRead: " + comp);
    }
    
    public void visit (SimplePinWrite comp)
    {
        if (_lim.db) writeln("SimplePinWrite: " + comp);
    }

    public void visit (FifoAccess mod)
    {
        if (_lim.db) writeln("FifoAccess: " + mod);
        visitGenericModule(mod);
    }
    
    public void visit (FifoRead mod)
    {
        if (_lim.db) writeln("FifoRead: " + mod);
        visitGenericModule(mod);
    }
    
    public void visit (FifoWrite mod)
    {
        if (_lim.db) writeln("FifoWrite: " + mod);
        visitGenericModule(mod);
    }
    
    public void visit (EndianSwapper mod)
    {
        if (_lim.db) writeln("EndianSwapper: " + mod);
        visitGenericModule(mod);
    }
