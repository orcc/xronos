...
public class Xflow
{
    ...
    /** this is the prefix passed when running with wine (XIL_LINUX_WINE env is 1) */
    ...
    public Xflow(Design design, File forge_synth_file, File destination_dir, String speed_target_override)
    {
           ...
           (this.destination_dir.getAbsolutePath().indexOf(' ') >= 0))
        {
            ...
            logger.error("       " + quote(this.forge_synth_file.getAbsolutePath()));
            logger.error("       " + quote(this.destination_dir.getAbsolutePath()));
        ...
        String xilinx_env_raw = environmentProperties.getProperty("XILINX","").trim();
        String xil_linux_wine_raw = environmentProperties.getProperty("XIL_LINUX_WINE","").trim();
        ...
        while((xilinx_env.length() > 0) && xilinx_env.endsWith(new String(File.separator)))
        {
        ...
        if (!linuxWine)
        {
            ...
            if(env_valid && !x_bin.isDirectory())
            {
                ...
                if(!x_bin.exists())
                    logger.error("       Couldn't find: " + x_bin.getAbsolutePath());
                else
                    logger.error("       " + x_bin.getAbsolutePath() + " wasn't a directory");
            }
            
            File x_verilog = new File(xilinx_env,"verilog");
            File x_unisim  = new File(xilinx_env,("verilog" +
                                                  File.separator + "src" +
                                                  File.separator + "iSE" +
                                                  File.separator +
                                                  "unisim_comp.v"));
        ...
        if(env_valid)
        {
            ...
            {
                String execedString=" ";
                for (Iterator iter=command.iterator(); iter.hasNext();)
                {
                    execedString+=iter.next()+" ";
                }
                // Couldn't find xflow!
                logger.error("");
        ...
        if(env_valid && (exitValue != 0))
        {
            ...
            logger.error("    "+execedString+"completed with error status: " + exitValue + ", your environment");
        ...
        if(partstring.length() > 0)
            partstring = (new XilinxDevice(partstring)).getFullDeviceNameNoTemp();
        ...
        // speed preference.
        if(env_valid && (speed_target_override == null) && (targetspeedstring.length() == 0))
        {
            logger.warn("");
            ...
            if(outfile != null)
            {
                ...
                catch(Throwable t)
                {
                    ...
                    logger.error("Couldn't append BitGen command line to file: " +
                                 outfile.getAbsolutePath() + " because: " +
                                 t.getMessage());
                }
            }            
    ...
    {
        return(forge_synth_file.getName()).substring(0,((forge_synth_file.getName()).lastIndexOf(".v")));
    }
    ...
    {    	
        if(isEnvironmentValid())
        {
            ...
            // write out ucf file in the working directory.  Might need to create the directory
            ...
            Collection ucf_list = ((OptionMultiFile)op).getValueAsList(CodeLabel.UNSCOPED);
            
            if((targetspeedstring.length() == 0) &&
               (ucfFile.exists()) && (ucf_list.size() == 0))
            {
            ...
            else
            {
                ...
                {
                    logger.error("");
                    logger.error("    During generation of ucf file: " + ioe.getMessage());
                    run_ok = false;
                }
                ...
                // OK, create our new UCF file
                try
                {
                    FileOutputStream fos = new FileOutputStream(ucfFile);
                ...
                {
                    logger.error("");
                    logger.error("    During generation of ucf file: " + fnfe.getMessage());
                    run_ok = false;
                }
                */
            }
            ...
            String ucfFilePref = op.getValue(CodeLabel.UNSCOPED).toString().trim();
            ...
            boolean isUcfOnly = ((OptionBoolean)op).getValueAsBoolean(CodeLabel.UNSCOPED);
            ...
            try
            {
                ...
                Process p;
                p = Runtime.getRuntime().exec(getCommandArray(command),null,this.destination_dir);
            ...
            catch(IOException ioe)
            {
                ...
                logger.error("");
                logger.error("    Cannot exec "+execedString+": " + ioe.getMessage());
            ...
            String[] extensions = {".bgn",".rbt",".nky",".ll",".msk",".drc",".bit"};
            
            for(int i=0; i<extensions.length; i++)
            {
                File todel = new File(destination_dir,(subFileName + extensions[i]));
                todel.delete();
            }
            ...
            if(!run_script && !xflow_error & shouldRunBitgen())
            {
                ...
                try
                {
                    ...
                    Process p = Runtime.getRuntime().exec(getCommandArray(command),null,this.destination_dir);
                    int exitValue = 0;
    ...
    {
    	Option op = EngineThread.getGenericJob().getOption(OptionRegistry.BITGEN_OPTIONS);
    ...
    /**
     * build an String array from the ArrayList
     * Side effect of rebuilding field commandLine for getCommandLine() method
     */
    private String[] getCommandArray (ArrayList command)
    {
     ...
     */
    private ArrayList getCommandLineForExec (ArrayList list)
    {
        list.add("-p");
        list.add(partstring);
        if(run_synth)
        {    
            list.add("-synth");
            list.add(synthtypestring);
        }
        
        if(run_implement)
        {
            list.add("-implement");
            list.add(implementstring);
        }
        ...
        list.add("-wd");
        list.add(this.destination_dir.getAbsolutePath());
        list.add(forge_synth_file.getAbsolutePath());
    ...
    public void setPinMap (HashMap hm)
    {
        this.pin_map = hm;
    }
