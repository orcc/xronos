...
public class MemoryReferee extends Referee
{
     ...
     * @param memoryPort what the referee is controlling access to
     * @param readList list of reads - null if task doesn't read
     * @param writeList list of writes - null if task doesn't writes
     */
    //public MemoryReferee (LogicalMemoryPort memoryPort, List readList, List writeList)
    public MemoryReferee (Arbitratable resource, List readList, List writeList)
    {
        ...
        // else there are multiple tasks... so build the full, arbitrated referee...
        ...
        for (int i=0; i<taskCount; i++)
        {
            ...
            TaskCapture tc=new TaskCapture(stateWidth, addressWidth, memoryWidth, getResetPort().getPeer(),df.getResultBus(),
                                           globalSlot.getReadDataPort().getPeer(), (TaskSlot) getTaskSlots().get(i),i,combinationalMemoryReads);
            taskCaptures.add(tc);
    ...
    public void connectImplementation (StructuralMemory.StructuralMemoryPort port)
    {
        ...
        {
            /*
             * The reason a Constant object is created here is because
             * the read data port needs to be tied to something which is 
             * outside of MemoryReferee; Otherwise, it will cause the
             * const prop looping infinitely.
             */
    ...
    class TaskCapture
    {
         ...
         */
        TaskCapture (int stateWidth, int depth, int width, Bus resetBus, Bus doneBus,
                     Bus dataOutBus, TaskSlot ts, int index, boolean combinationalRead)
        {
            boolean readUsed=ts.getGoRPort() != null;
            boolean writeUsed=ts.getGoWPort() != null;
            ...
            if (!readUsed || !writeUsed) // only 2 inputs if we only read or only write
            {
                readOr=new Or(2);
            }
            else
            ...
            Or addrOr=null;
            if (readUsed && writeUsed)// only necessary if both reading and writing
            {
                addrOr=new Or(2);
                addComponent(addrOr);
    ...
    class StateMachine
    {
        ...
        void setTaskCaptures (List taskCaptures)
        {
            ...
            // stateAnd gets inverted global reset, stateOr and last mux (done below)
            ...
            /*
             * stateAnd only produces 1 bit, so it has to be extended to
             * the state width.  Make it signed so the bit will be replicated.
             * Then cast it to be unsigned before plugging it into the stateAndOp.
             */
            ...
            for (int i=0; i<size; i++)
            {
                ...
                //first mux port is the output of the previous mux, or the stateAnd
                ...
                else // this is the last mux, its output feeds the 0th port of stateMux
                {
