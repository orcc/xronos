...
/**
 * ScalarConstant is a MemoryConstant whose value is a simple scalar
 * value, created as an ordered sequence of bytes.  The value of this
 * constant is immutable and fixed at creation.
 *
 * <p>Created: Wed Apr 21 09:33:21 2004
 *
...
public class ScalarConstant extends MemoryConstant 
{
     ...
     *
     * @param bytes an array of bytes ordered from least significant
     * address to most significant that defines the numerical value of
     * this constant.
     * @param width the width, in bits, of this constant
     * @param isSigned true if this is a signed constant, false
     * otherwise. 
     */
    //public ScalarConstant (byte[] bytes, int width, boolean isSigned)
    public ScalarConstant (AddressableUnit[] bytes, int width, boolean isSigned, AddressStridePolicy policy)
    {
        ...
        if (topBits == 0) topBits = policy.getStride();
        BigInteger mask = BigInteger.ZERO;
        for (int i=0; i < topBits; i++)
            mask = mask.shiftLeft(1).or(BigInteger.ONE);
        aurep[aurep.length-1] = new AddressableUnit(bytes[aurep.length-1].getValue().and(mask).intValue(), true);
    ...
    {
        return Collections.unmodifiableSet(Collections.singleton((Constant)this));
    }
    
    /**
     * Returns an array of ByteRep objects which specifies the exact
     * byte layout from memory.  All bytes are specified as fully
     * locked. 
     */
    @Override
