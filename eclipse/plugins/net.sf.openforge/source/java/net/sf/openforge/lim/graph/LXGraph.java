...
public class LXGraph extends DefaultVisitor
{
     ...
     * @param topModule the top level module to be graphed
     */
    public LXGraph (String name, Visitable topModule, int fontSize, int layersToGraph)
    {
        super();
        ...
        if (topModule instanceof StructuralMemory || topModule instanceof Register.Physical)
        {
...
//         ModuleGraph subgraph = new ModuleGraph(block, nodeCount++,fontSize);
        BlackBoxGraph subgraph = new BlackBoxGraph(ComponentNode.getShortClassName(block)
            + " @" + Integer.toHexString(block.hashCode())+": "+ID.showLogical(block), nodeCount++,fontSize);
    ...
    {
        _graph.ln("Visit: "+block);
        String id = ComponentNode.getShortClassName(block) + " @" + Integer.toHexString(block.hashCode());
        id += ": " + block.showIDLogical();
        BlackBoxGraph subgraph = new BlackBoxGraph(id, nodeCount++,fontSize);

        for(Iterator it=block.getComponents().iterator();it.hasNext();)
        {
            Component c=(Component)it.next();
            subgraph.addComponent(c);
        }
    ...
    public void visit (MemoryWrite mw)
    {
        ...
        {
            ModuleGraph subgraph = new ModuleGraph(mw.getPhysicalComponent(), nodeCount++,fontSize);
            pushSubgraph(subgraph);
            nodeCount = subgraph.getNodeCount();
        }
    }
    ...
    public void visit (MemoryRead mr)
    {
        ...
        if(mr.getPhysicalComponent()!=null)
        {
            ModuleGraph subgraph = new ModuleGraph(mr.getPhysicalComponent(), nodeCount++,fontSize);
            pushSubgraph(subgraph);
            nodeCount = subgraph.getNodeCount();
        }
    }
...
//         visit(block.getMemoryWrite());
        ...
        if (mw.getPhysicalComponent() != null)
        {
            ...
            for (Iterator iter = mw.getPhysicalComponent().getDataPorts().iterator(); iter.hasNext();)
            {
    ...
    public void visit (PinWrite pwrite)
    {
        ...
        {
            ModuleGraph subgraph = new ModuleGraph(pwrite.getPhysicalComponent(), nodeCount++,fontSize);
            pushSubgraph(subgraph);
            nodeCount = subgraph.getNodeCount();
        }
        super.visit(pwrite);
    ...
    public void visit (PinRead pread)
    {
        ...
        {
            ModuleGraph subgraph = new ModuleGraph(pread.getPhysicalComponent(), nodeCount++,fontSize);
            pushSubgraph(subgraph);
            nodeCount = subgraph.getNodeCount();
        }
        super.visit(pread);
    ...
    public void visit (EndianSwapper endianSwapper)
    {
        ...
        ModuleGraph subgraph = new ModuleGraph(endianSwapper, nodeCount++,fontSize);
        pushSubgraph(subgraph);
        nodeCount = subgraph.getNodeCount();
    ...
    public void visit (Design design)
    {
        ...
        BlackBoxGraph designModuleGraph = new BlackBoxGraph(ID.showLogical(design)+"_designModule", designMod, nodeCount++, fontSize);
        HashSet moduleComps = new HashSet();
        for (Iterator iter = designMod.getComponents().iterator(); iter.hasNext();)
        {
        ...
        for (Iterator iter = moduleComps.iterator(); iter.hasNext();)
        {
    ...
    {
        BlackBoxGraph physGraph = new BlackBoxGraph(ID.showLogical(mod), mod, nodeCount++, fontSize);
        for (Iterator iter = mod.getComponents().iterator(); iter.hasNext();)
        {
            physGraph.addComponent((Component)iter.next());
            nodeCount++;
        }
    ...
    /**
     *
     * Adds the given graph as a subgraph of the current graph, but
     * allows for a hierarchical limit.  The trigger is set to true on
     * the first pass through this method (when we've graphed one
     * module) which has the effect of hiding any sub-modules
     */
