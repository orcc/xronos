 ...
 * @author gandhij
 *
 * Helper class to emulate a loopBody. Create a class of this type
 * and reapeatedly call the iterate method to do all the iterations
...
public class LoopBodyEmulator extends ModuleEmulator {
	 ...
	 * @param loop the loop whose body needs to be emulated
	 */
	...
	public Map emulate(Map inputValues) throws UnEmulatableLoopException{
		// System.out.println("\nEMULATING LOOPBODY -- " + module + " with inputValues = "+ inputValues);
		...
		while(iter.hasNext()){
            ...
            if(component == loop.getBody().getDecision()){
            	LoopDecisionEmulator de = new LoopDecisionEmulator(loop, (Module)component);
