...
/**
 * Bit represents the value of a single bit in a LIM graph.  There are two
 * kinds of Bits.  The first kind are global, and are defined to be the
 * static variables {@link ZERO} (a constant 0 bit), {@link ONE} (a constant
 * 1 bit), {@link CARE} (a care bit of unknown value), and {@link DONT_CARE}
 * (a don't-care bit of unknown value).
 * <P>
 * There are also non-global bus bits.  A sequence of these Bits is
 * used to represent the components of a {@link Bus}.  Each bus Bit is
 * said to be owned by its {@link Bus}.
 * <P>
 * Bits are immutable.  In particular, a bus Bit will always belong to
 * the same {@link Bus}.  Bits from any source, global or bus, may be
 * arbitrarily combined in a {@link Value} to represent a logical or
 * numerical datum.
 *
...
public class Bit
{
    ...
    /**
     * This is the position of this bit in the list of its owner Bus. If
     * there is no owner, it is set to ILLEGAL_POSITION.
     */
    private final int position;


    /**
     * Constructs a new non-global Bit.  This Bit will be a component
     * of a given {@link Bus}.
     *
     * @param owner the bus to which this bit will belong
     ...
     *
     * @return the owner bus if this is a non-global bit, otherwise a
    ...
    /**
     * Gets the position of this bit in its owner {@link Bus Bus's} ordered
     * list of component bits.
     *
     * @return the position of this bit
     * @throws NullPointerException if this is a global bit
     */
    public int getPosition () throws NullPointerException
    {
        ...
        {
            throw (NullPointerException)new NullPointerException("not a bus bit: " + this).initCause(eNull);
        }
    }
     ...
     *
     * @param bit the inverted bit
     */
    public void setInvertedBit (Bit bit)
    {
        if (isGlobal())
        {
            throw new UnsupportedOperationException("setInvertedBit of a global");
        }
