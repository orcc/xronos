...
/**
 * A hack of {@link LGraph} with reduced complexity.  Single {@link Entry Entries} and
 * {@link Exit Exits} are drawn as part of the main node.  {@link NoOp NoOps} with no
...
public class LDependencyGraph extends DefaultVisitor
{
    ...
    public static void graphTo (Module module, String filename)
    {
        try
        {
            LDependencyGraph graph = new LDependencyGraph(module.toString(), module);
    ...
    /**
     * Constructs a new LDependencyGraph that will not draw the insides of pass-through
     ...
     * 
     * if we are graphing physical connections, then draw the ports at the top of the 
     * component, else draw just the component
     */
    private void graph (Component component)
    {
        ...
        {
            final Entry entry = (Entry)component.getEntries().iterator().next();
            ...
            addToNodeMap(EntryPort.get(entry, entry.getGoPort()), goPort, DEPENDENCY);
            ...
            if (drawCR)
            {
                ...
                addToNodeMap(EntryPort.get(entry, entry.getClockPort()), clockPort, DEPENDENCY);
                ...
                addToNodeMap(EntryPort.get(entry, entry.getResetPort()), resetPort, DEPENDENCY);
            }
            ...
            for (int i = 0; i < component.getDataPorts().size(); i++)
            {
                ...
                addToNodeMap(EntryPort.get(entry, dp),
                    dataPort, DEPENDENCY);
            }
        }
    ...
    private void graphSingleExit (InBuf inbuf, net.sf.openforge.util.graphviz.Record node)
    {
    ...
    private void connectEntries (Module module, Component target, boolean feedback)
    {
     ...
     *
     * @param component the component whose entries are to be attached
     * @param target the component to which the entries are attached
     */
    private void connectEntries (Component component, Component target, boolean feedback)
    {
        ...
        for (Iterator iter = component.getEntries().iterator(); iter.hasNext();)
        {
            ...
            /*
            if((graphDExits)&&(entry.getDrivingExit()!=null))
            {
                edge = new Edge(WT_ENTRY);
                edge.setLabel("dexit" + index++);
                edge.setDirection(Edge.DIR_BACK);
                edge.setColor("orange");
                Exit ex=entry.getDrivingExit();
                Node exitNode = (Node)nodeMapDeps.get(ex);
    ...
    private void graphOutside (Component component,boolean feedback)
    {
        ...
        // don't need to do physical "entry" (ports) because they are drawn as a part of 
    ...
    private void graphEntry (Entry entry, boolean feedback)
    {
        ...
        net.sf.openforge.util.graphviz.Record record = new net.sf.openforge.util.graphviz.Record("entry" + nodeCount++);
        ...
        if (drawCR)
        {
            net.sf.openforge.util.graphviz.Record.Port clock = record.getPort("clk");
            clock.setLabel("c");
            clock.setSeparated(true);
            addToNodeMap(EntryPort.get(entry, entry.getClockPort()), clock,
                         DEPENDENCY);
            
            net.sf.openforge.util.graphviz.Record.Port reset = record.getPort("rst");
            reset.setLabel("r");
    ...
    private void graphDependencies (Component component, Entry entry, boolean feedback)
    {
        for (Iterator iter = entry.getPorts().iterator(); iter.hasNext();)
        {
            Port port = (Port)iter.next();
            if (!drawCR && ( (port == component.getClockPort()) ||
    ...
    private void addToNodeMap(Object key, Object value, int type)
    {
        ...
        if (unresolvedNodes.containsKey(key))
        {
            ...
            {
                topGraph.connect(target, (Node) sources.get(i), (Edge) edges.get(i));
            }
...
//     }
    ...
    public void visit (Scoreboard scbd) { graph(scbd); }
    ...
    public void visit (MemoryRead op) { graph(op); }

    public void visit (MemoryWrite op) { graph(op); }
    ...
    public void visit (PinRead op) { graph(op); }

    public void visit (PinWrite op) { graph(op); }
    ...
    public void visit (SimplePinRead op) { graph(op); }
    public void visit (SimplePinWrite op) { graph(op); }
}
