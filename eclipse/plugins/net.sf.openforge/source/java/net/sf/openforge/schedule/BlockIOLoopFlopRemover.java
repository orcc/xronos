...
/**
 * The idea of this visitor is to find any loop in the top level
 * application which contains exactly one memory access and one fifo
 * access.  If the memory access or the fifo access has a latency of
 * more than 0 cycles then the loop flop is removable.  This is
 * because we know that in the structure of the block IO loops the
 * memory access and the fifo access are dependent on one another, and
 * that they will not execute in the same cycle.  This means that for
 * block IO input the fifo read may be 0 cycles, but the memory write
 * will take 1 cycle and will thus not occur in both the first and
 * last cycles (which the default analysis currently shows is
 * 'possible').  Similarly for block IO output, the memory read may be
 * 0 cycles, but the fifo write will take at least one cycle.  This
 * means that the memory access cannot occur in both the first and
 * last cycle.
 * <p>This visitor will not descend into any calls thus it only
 * analyzes the wrapper level.
 *
 * <p>Created: Thu Apr  1 10:57:09 2004
 *
...
public class BlockIOLoopFlopRemover extends DefaultVisitor 
{
    ...
    /**
     * A new tracker is created for each loop entered.  Its job is to
     * simply keep track of whether we have found a fifo access and a
     * memory access.
     */
    private Tracker tracker = null;
    
    /**
     * Traverses the block IO module wrapper looking for any loops,
     * and for each found loop marks that loop with whether or not the
     * loop flop can be removed.
     *
     * @param design a non null Design
     */
    public static void analyze (Design design)
    {
        if (!EngineThread.getGenericJob().getUnscopedBooleanOptionValue(OptionRegistry.NO_BLOCK_IO)) //If Do BlockIO
        {
            try
            {
            ...
            {
                EngineThread.getGenericJob().warn("Unexpected internal structure in block IO loop.  Recovered normally. " + e);
            }
        }
    }
    
    /**
     * Made private so that nobody can enter this analysis except via
     * a Design.
     */
    ...
    /**
     * Jumps straight to the procedure so that we can override Call to
     * do nothing.  This gets us the top level call (ie the block IO
     * wrapper and nothing else to analyze)
     */
    public void visit (Task vis)
    {
        if (vis.getCall() != null &&
            vis.getCall().getProcedure() != null)
        {
    ...
    /**
     * Creates a new Tracker, then begins processing the loops.  After
     * processing the loop the tracker is analyzed to determine if the
     * loop flop can be removed.
     */
    public void visit (Loop vis)
    {
        ...
        {
            // throw new UnexpectedStructureException("Nested loop in block IO loop");
        ...
        if (this.tracker.isRemovable())
        {
            vis.getBody().setLoopFlopNeeded(false);
            if (debug) System.out.println("Marked loop " + vis + " " + vis.getBody().getBody().getComponents());
        }
    ...
    /*
     * These are the things that we are looking for so we mark them in
     * the tracker for analysis later.
     */
    /*
     * Ignore memory read/write.  They are always contained INSIDE a
     * MemoryAccess 
    public void visit (MemoryRead vis){ fail(vis); }
    public void visit (MemoryWrite vis){ fail(vis); }
    */
    public void visit (AbsoluteMemoryRead vis){ markMem(vis); }
    public void visit (AbsoluteMemoryWrite vis){ markMem(vis); }
    public void visit (ArrayRead vis){ markMem(vis); }
    public void visit (ArrayWrite vis){ markMem(vis); }
    public void visit (HeapWrite vis){ markMem(vis); }
    public void visit (HeapRead vis){ markMem(vis); }

    public void visit (FifoAccess vis) { markFifo(vis); }
    public void visit (FifoRead vis) { markFifo(vis); }
    public void visit (FifoWrite vis) { markFifo(vis); }
     ...
     * happen for the write to memory of any result value from the
     * entry method.
     */
    private void markMem (MemoryAccessBlock mab)
    {
        if (this.tracker != null)
            this.tracker.mark(mab);
    }
    private void markFifo (FifoAccess fa)
    {
        if (this.tracker != null)
            this.tracker.mark(fa);
    }
    ...
    */
     ...
     * components that are expected in block IO loops.  It also
     * provides the analysis which determines if a loop conforms to
     * the 'removable loop flop' criteria.
     */
    class Tracker
    {
        ...
        {
//             if (this.memAcc != null)
//                 throw new UnexpectedStructureException("Multiple memory accesses in block IO loop");
//             this.memAcc = acc;
...
//                 throw new UnexpectedStructureException("Multiple fifo accesses in block IO loop");
//             this.fifoAcc = acc;
        ...
        /**
         * The loop flop is removable if we found both a fifo access
         * and a memory access AND the memory access or the fifo
         * access takes at least one cycle to complete.
         */
        public boolean isRemovable ()
        {
            if (this.memAcc == null || this.fifoAcc == null)
            {
                if (BlockIOLoopFlopRemover.debug) System.out.println("Wrong signature, not removable. memAcc " + this.memAcc + " fifoAcc " + this.fifoAcc);
                return false;
            }

            if (this.fifoAcc.getLatency().isGT(Latency.ZERO) ||
                this.memAcc.getMemoryAccess().getLatency().isGT(Latency.ZERO))
            {
                return true;
            }
            
            if (BlockIOLoopFlopRemover.debug) System.out.println("memAcc latency not GT zero " + this.memAcc.getMemoryAccess().getLatency());
            return false;
        }
    }
