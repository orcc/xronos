...
/**
 * This library contains code that can be used with the Forge
 * Operation Replacement feature to provide implementations for all
 * operations that work on the Java <code>double</code> primitive
 * type.  These operations will be replaced with a call to one of the
 * public methods contained in this library according to the defined
 * naming conventions.  The implementation of each operations is to
 * supply values to an IPCore that has been defined for each type of
 * operation.  The return value is retrieved from an output of that
 * IPCore.  The core's implementation is defined by the associated
 * <code>HDLWriter</code> and is simply an empty module with the black
 * box attribute set.  This leaves the actual implementation of the
 * functionality of each core to the user.
 * <p>The cores defined in this library are all defined in non-static
 * fields of this library class.  This means that a <b>unique</b>
 * instance of the core will be created for <b>each</b> operation that
 * is replaced.  The alternative is to store the core and associated
 * pins in static fields of this class, in which case only 1 core will
 * be allocated for each type of operation, and all replacements will
 * target that one core (which will be correctly arbitrated by the
 * Forge scheduling algorithms).
 */
public class DEFAULT_DOUBLE
{
    ...
    {
        public List writeVerilog (IPCore core, java.io.PrintWriter pw)
        {
            pw.println("// synthesis attribute BOX_TYPE of "+name()+" is \"BLACK_BOX\"");
        ...
        int inWidth () {  return 64; }
        int outWidth () { return 64; }
    }
