 ...
 * <P>
 * A LoopBody designates one of its {@link Exit Exits} as a "feedback
 * exit."  When this exit is asserted, it indicates that another
 * iteration of the loop should begin.  The Buses of this exit are
 * connected to the feedback inputs of data and control
 * {@link Reg Registers}.  If the body will never iterate (for example,
 * if it ends with a break) then this exit may be null.
 * <P>
 * The {@link Decision} may also be null if it is known that it will
 * never be reached (for instance, a do-loop that ends with a break).
 * <P>
 * In addition, a LoopBody identifies the input Port that provides
 * the initial value for each feedback data flow.
 *
...
public abstract class LoopBody extends Module
{
    ...
    public static final Exit.Tag FEEDBACK_TAG = Exit.getTag(Exit.DONE, "#feedback#");
    ...
    /** This boolean tracks whether or not the loop flop related to
     * this loop body has been deemed necessary or whether it can be
     * removed.  The default behavior is to assume that it is always
     * needed.  The LoopFlopAnalysis visitor in the scheduling package
     * may change the state of this variable */
    ...
    {
        super();
        if (_lim.db) _lim.ln(_lim.LOOPS,"Creating 2 exits on Loop Body");
    }

    /**
     * Gets the exit which signals that this block has finished,
     * and another iteration should begin.
     *
    ...
    /**
     * Gets the exit which signals that this block has finisished, and
     * there are no more iterations to process.
     */
    ...
    /**
     * Tests whether this loop body will ever iterate.  If not, the
     * feedback {@link Exit} will be null.
     */
    public boolean isIterative ()
    {
        return (getFeedbackExit() != null
            && !getFeedbackExit().getPeer().getEntries().isEmpty());
    }
     ...
     *
     * @return the update component, a block which the update expression resides,
     * in this loop
     */
    ...
    /**
     * Returns true if this loop body must have a feedback register
     * (the control loop flop) to seperate iterations, if false the
     * loop flop may be removed.
     *
     * @return a boolean
     */
    ...
    /**
     * Used to set whether the loop control feedback register (loop
     * flop) is necessary for correct functioning of the loopbody.
     *
     * @param value a boolean, set to false if the loop flop can be
     * safely removed.
     */
