...
/**
 * AbsoluteMemoryWrite is a fixed access to a
 ...
 * offset address.  This module is populated with a {@link MemoryWrite}
 * and 2 constants.  The first identifies the address being writen and
...
public class AbsoluteMemoryWrite extends MemoryAccessBlock 
{
     ...
     *
     * @param access a value of type 'MemoryWrite'
     * @param target a value of type 'Allocation'
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     */
    public AbsoluteMemoryWrite (Location target, int maxAddressWidth, boolean isSigned)
    {
        this(target, target.getAddressableSize(), maxAddressWidth, isSigned);
    }
    
    /**
     * Constructs a write access which retrieves the specified number
     * of bytes from the given Allocation.
     *
     * @param memoryWrite a value of type 'MemoryWrite'
     * @param target a value of type 'Allocation'
     * @param addressableLocations a value of type 'int', the number
     * of addresses accessed atomically by this operation.
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     */
    public AbsoluteMemoryWrite (Location target, int addressableLocations, int maxAddressWidth, boolean isSigned)
    {
        ...
        this(new MemoryWrite(false, (addressableLocations * target.getAbsoluteBase().getInitialValue().getAddressStridePolicy().getStride()), isSigned),
     ...
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     */
    private AbsoluteMemoryWrite (MemoryWrite memoryWrite, Location target, int addressableLocations,
        int maxAddressWidth)
    {
        ...
        this.addrConst = new LocationConstant(target, maxAddressWidth, target.getAbsoluteBase().getLogicalMemory().getAddressStridePolicy());
...
//         valueBus.setSize(addressableLocations * 8, false);
        ...
        writeEntry.addDependency(memoryWrite.getSizePort(), new DataDependency(sizeConst.getValueBus()));
        writeEntry.addDependency(memoryWrite.getAddressPort(), new DataDependency(addrConst.getValueBus()));
    }
     ...
     *
     * @param visitor the instance to be visited with {@link Visitor#visit(AbsoluteMemoryWrite)}
     */
    ...
    /**
     * Clones this object.  A new AbsoluteMemoryWrite is created using a copy of the
     * {@link MemoryWrite}, including its {@link LogicalMemoryPort}, and the {@link Location}
    ...
    public Object clone () throws CloneNotSupportedException
    {
        ...
        final AbsoluteMemoryWrite clone = new AbsoluteMemoryWrite((MemoryWrite)getMemoryWrite().clone(),
