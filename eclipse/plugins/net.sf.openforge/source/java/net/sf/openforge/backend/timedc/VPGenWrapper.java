...
/**
 * VPGenWrapper generates a header file and C++ source file that makes
 * our cycle C model Virtual Platform compliant.  The header file
 * defines the instance of the pcore class and the publicly available
 * member data and functions.
 *
 * <p>Created: Sat Apr 16 13:32:09 2005
 *
...
public class VPGenWrapper
{
    ...
    /** The ioHandler that will take care of naming for our models
     * ports. */
     ...
     *
     * @param io a value of type 'IOHandler'
     * @param fxnName a value of type 'String'
     * @param simHeaderFile a value of type 'File'
     */
    public VPGenWrapper (IOHandler io, String fxnName, File simHeaderFile)
    {
        ...
        this.headerFile = EngineThread.getGenericJob().getFileHandler().getFile(CycleCTranslateEngine.VPGEN_HEADER);
        this.sourceFile = EngineThread.getGenericJob().getFileHandler().getFile(CycleCTranslateEngine.VPGEN_WRAPPER);
        
        this.fifoIFs = new ArrayList(this.ioHandle.getInputs());
    ...
    private VarInits writeHeaderFile () throws IOException
    {
        ...
        final PrintStream ps = new PrintStream(new FileOutputStream(this.headerFile), true);
        ...
        for (FifoIF fifo : this.ioHandle.getInputs())
        {
            ...
            for (SimplePin pin : fifo.getPins())
            {
                varInits.structMemberInits.add(name + "." + this.ioHandle.getMemberName(pin) + " = 0;");
            }
        ...
        ps.println("    " + ioHandle.getInputType() + " *"+I_PTR+"["+varInits.inputPointers.size()+"];");
        ps.println("    " + ioHandle.getOutputType() + " *"+O_PTR+"["+varInits.outputPointers.size()+"];");
     ...
     *
     * @param cModel a value of type 'File'
     * @exception IOException if an error occurs
     */
    private void writeSourceFile (File cModel, VarInits varInits) throws IOException
    {
        final PrintStream ps = new PrintStream(new FileOutputStream(this.sourceFile), true);
        ...
        for (FifoIF fifo : this.fifoIFs)
        {
            ...
            for (SimplePin pin : fifo.getPins())
            {
                ...
                ps.println("    " + base + "." + member + " = *ports->" + getVPName(pin) + ";");
            }
        }
        ...
        for (FifoIF fifo : this.fifoIFs)
        {
            ...
            {
                compareTerms.add("(" + base + "." + this.ioHandle.getMemberName(outPin) + " != *ports->" + getVPName(outPin) + ")");
                copyTerms.add("*ports->" + getVPName(outPin) + " = " + base + "." + this.ioHandle.getMemberName(outPin) + ";");
            }
        }
        ...
        ps.println("    }");
        /* I dont think that this needs to be done here.  I believe
           that this function is the clock edge and EvalAsyncOutputs
           is the combinational phase.
        ps.println("    // Now that the clock has advanced, do the combinational stuff too");
        ps.println("    "+this.fxnName+"_update("+I_PTR+", "+O_PTR+");");        
        */
        ps.println("}");
    ...
    private void writeVarInits (VarInits varInits, PrintStream ps)
    {
        ...
        {
            ps.println(tab + "" + I_PTR+"["+i+"] = &" + varInits.inputPointers.get(i) + ";");
        }
        for (int i=0; i < varInits.outputPointers.size(); i++)
        {
            ps.println(tab + "" + O_PTR+"["+i+"] = &" + varInits.outputPointers.get(i) + ";");
        }
        
        for (Iterator iter = varInits.structMemberInits.iterator(); iter.hasNext();)
        {
    ...
    /**
     * Generates/caches a name for the given fifo interface.  This is
     * the name used for our private variable which interacts with our
     * simulation model.
     *
     * @param pin a value of type 'SimplePin'
     * @return a value of type 'String'
    ...
    private String getVPName (FifoIF fifo)
    {
        ...
        String prefix = ((SimplePin)fifo.getPins().iterator().next()).getName();
