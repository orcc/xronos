 ...
 *
 * @author  Conor Wu
 * @version $Id: RightShiftOp.java 2 2005-06-09 20:00:48Z imiller $
 */
public class RightShiftOp extends ShiftOp implements Emulatable
{
    private static final String _RCS_ = "$Rev: 2 $";

    /**
     * Constructs a logical right shift operation.
     *
     * @param maxStages the maximum number of stages (shifts by 2^n)
     * necessary to implement this shift
     */
    public RightShiftOp (int maxStages)
    {
        super(maxStages);
    }
    ...
    public Map emulate (Map portValues)
    {
        ...
        return Collections.singletonMap(getResultBus(), lval.shiftRight(rval));
    }
     ...
     * this RightShiftOp according to these rules:
     * 
     * If both inputs are care (non-constant):
     * Result size is data input size.  All result bits are care.
     * If shift magnitude is constant value:
     *  1. if we shift beyond our inputs size, then our output
     *     is simply the sign bit of the input.    
     *  2. if we shift within our inputs size, then the MSB bits are all
     *     signed bit of data input value, and the Next 'y' bits are all
     *     passthrough of the top 'y' bits of data input value. 
     * Result is signed if data value is signed.
     * 
    ...
    public boolean pushValuesForward ()
    {
        ...
        if(in1.isConstant())
        {
            ...
            if (newValue.getSize() < shiftMag)
            {
                ...
                {
                    newValue.setBit(i, in0.getBit(in0.getSize()-1));
                }
            }
            else
            {
                ...
                Bit extendedBit = newValue.getBit(position -1);
                for (; position < newValue.getSize(); position++)
                {
                    if (in0.isSigned())
                    {
                        newValue.setBit(position, extendedBit);
                    }
                    else
                    {
    ...
    /**
     * Reverse constant prop on a RightShiftOp applies only if the
     * shift magnitude is a constant value. Constant/don't
     * care/passthrough bits are taken from result to data port based
     * on shift magnitude offset.
     *
     * @return a value of type 'boolean'
     */
    public boolean pushValuesBackward ()
    {
        ...
        if (inValue1.isConstant())
        {
            ...
            {
                for (int i = 0; i < newValue0.getSize() -1; i++)
                {
