...
public class OutputPin extends Pin
{
    ...
    /**
     * Constructs a new OutputPin based on a Bus,
     * and connects the OutputPin's Port to the Bus.
     * <p>
     * XXX FIXME.  The OutputPin structure isn't quite correct.  For
     * Character data types the Pin will report it's width as 16 bits
     * but the Port attached to it will be 17 bits.  We rely on the
     * fact that Verilog allows a smaller wire to be connected to a
     * larger port on the instantiation of the entry method to resolve
     * this discrepancy which is a bad idea!
     *
     * @param bus the Bus upon which to base the OutputPin
     */
    ...
    public int getWidth ()
    {
        ...
        {
            return Math.min(super.getWidth(), getPort().getValue().getCompactedSize());
        }
        else
        {
    ...
    /**
     * Looks at the Value on the input Port and replaces the all the MSB constants
     * with don't cares (except for a leading sign bit, if necessary).
     */
     ...
     *
     * @return true if this pin was created with {@link OutputPin#OutputPin(Bus)},
     *         false otherwise
     */
