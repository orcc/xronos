...
/**
 * A Scalar is an implementation of {@link LogicalValue} for scalar
 * numeric values that represent pure numbers, i.e. not pointers.  The
 * internal representation is in endian correct order, meaning that
 * the getRep() method returns values in address order.
 *
...
public class Scalar implements LogicalValue, MemoryVisitable
{
    ...
    /**
     * Constructs a new Scalar object which has only a single
     * addressable unit and the given addressing policy.
     * @param rep the single {@link AddressableUnit} making up the
     * value of this Scalar.
     * @param policy the AddressStridePolicy for this LogicalValue
     */
    ...
    /**
     * Constructs a new Scalar LogicalValue whose value is indicated
     * by the array of {@link AddressableUnit} objects and whose
     * addressing is governed by the the given address policy.
     *
     * @param rep the array of bytes, high to low, containing the bitwise
     *          representation of this value
     * @param policy the AddressStridePolicy for this LogicalValue
     * @throws NullPointerException if <code>rep</code> is null
     */
    public Scalar (AddressableUnit[] rep, AddressStridePolicy policy)
    {
     ...
     *
     * @param rep the array of initial values, index 0 is the least
     * significant bits of the values, index N-1 is the most
     * significant. 
     * @param adjust for endianness - if false do not adjust
     * @param policy the AddressStridePolicy for this LogicalValue
     * @throws NullPointerException if <code>rep</code> is null
     */
    private Scalar (AddressableUnit[] rep, boolean adjust, AddressStridePolicy policy)
    {
        ...
        this.isLittleEndian = EngineThread.getGenericJob().getUnscopedBooleanOptionValue(OptionRegistry.LITTLE_ENDIAN);
     ...
     *
     * @return the number of addresses needed to represent this value;
     * this number is at least 1 for a scalar value
     */
    public int getSize ()
    {
        return rep.length;
    ...
    /**
     * Returns the size of this logical value in addressable units (as
     * a Scalar has only one constituent member, itself)
     *
     ...
     * the value of an empty data item, such as a struct with no
     * fields 
     */
    ...
    /**
     * Returns a new LogicalValue object (Scalar) which has the same
     * numerical representation as this Scalar object.
     *
     * @return a new LogicalValue with the same structure and
     * initial values as this LogicalValue.
     */
     ...
     * AddressableUnits specified by min and max (both inclusive) has
    ...
    public LogicalValue removeRange (int min, int max) throws NonRemovableRangeException
    {
        ...
        if (min < 0 || max < 0)
        {
            throw new NonRemovableRangeException("Illegal range: requires positive range.");
        }
        else if (min > max)
        {
            throw new NonRemovableRangeException("Illegal range: min is greater than max.");
        }
        else if ((min > rep.length - 1) || (max > rep.length - 1))
        {
            throw new NonRemovableRangeException("Illegal range: range out of bound.");
        }
    ...
    public MemoryConstant toConstant()
    {
        AddressableUnit[] rep = getRep();
        int size = getBitSize();
        return new ScalarConstant(rep, getBitSize(), false, getAddressStridePolicy());
    }
    ...
    public static final Scalar buildScalar (byte[] rep, AddressStridePolicy policy)
    {
...
//             units[i] = new AddressableUnit(BigInteger.valueOf(rep[i]));
    ...
    public static final Scalar buildByteScalar (BigInteger value, boolean isSigned, int byteLength, AddressStridePolicy policy)
    {
        ...
        /*
         * Copy the bits from the value to the rep, up to but
        ...
        /*
         * If the type is signed and the value is negative,
         * perform sign extension to the end of the rep.
         */
