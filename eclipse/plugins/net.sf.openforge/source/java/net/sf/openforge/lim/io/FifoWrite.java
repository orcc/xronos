...
/**
 * FifoWrite is an atomic access to a given {@link FifoIF} which
 * sends a single element of data to that interface.  It is,
 ...
 * <P>FifoWrite components are inherently unsigned due to the fact that
 * the {@link SimplePinRead} and {@link SimplePinWrite} on which they
 * are based are inherently unsigned.  If you need a FifoWrite to
 * consume a signed value you must cast the data port to the proper
 * signedness.
 *
...
//         data.setUsed(true);
         ...
         * pending = (GO || GO');
         * fifo_WR = pending & !fifo_FF
         * write_done (bus) <= fifo_WR; // NOTE THE REGISTER!
         * GO' <= pending & fifo_ff
         */
        ...
        final CastOp doutCast = new CastOp(targetInterface.getDataPin().getWidth(), false);
        final SimplePinWrite dout = new SimplePinWrite(targetInterface.getDataPin());
        final SimplePinWrite write = new SimplePinWrite(targetInterface.getSendPin());
        final SimplePinRead full = new SimplePinRead(targetInterface.getAckPin());
    ...
    /**
     * Needs a GO to generate the write signal, overrides Module
    ...
    protected void cloneNotify (Module clone, Map cloneMap)
    {
        ...
        //((FifoWrite)clone).feedbackPoints = Collections.singleton(cloneMap.get(this.feedbackPoints.iterator().next()));
    }
