...
/**
 * MemAccessVar maintains the various global variables for handling
 * all types of memory accesses (read or write).  The variables it
 * maintains are for the access pending state of the access, which is
 * essentially just a flop.  Additional methods are provided for
 * assembling the correct syntax for reading the memory and updating
 * the state bearing variables.
 *
 *
...
public class MemAccessVar extends OpHandle implements StateVar
{
     ...
     *
     * @param acc a non-null MemoryAccess
     * @param memVar a non-null MemoryVar
     * @param cache a CNameCache used to uniquify naming
     */
    ...
    /**
     * Returns a correctly formatted and name read access to the
     * backing memory for this memory access, taking into account any
     * necessary translation of the address based on access size.
     *
    ...
    public String getMemoryRead ()
    {
        ...
        int divisor = 0;
        if (width <= 8) divisor = 1;
        else if (width <= 16) divisor = 2;
        else if (width <= 32) divisor = 4;
        else if (width <= 64) divisor = 8;
    ...
    {
        return getBusName(this.acc.getExit(Exit.DONE).getDoneBus()) + "_current";
    }
