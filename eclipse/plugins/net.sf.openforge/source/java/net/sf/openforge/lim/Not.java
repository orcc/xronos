...
/**
 * An Not accepts a single 1-bit input and generates a 1-bit result which is
 * the inverse of the input.
 * The go port and done bus of an Not are unused.
 *
...
public class Not extends Primitive
{
     ...
     * <P>
     * In particular, a constant {@link Bit} will be propragated
     * as its inversion.  If the input {@link Bit} is non-global,
     * then the output will be the local {@link Bit} of the result
     * {@link Bus}, and the input {@link Bit} will be set as its
     * inversion using {@link Bit#setInvertedBit(Bit)}.
     *
    ...
    public boolean pushValuesForward ()
    {
        ...
        assert (inValue.getSize() == 1) : ("invalid not input size: " + inValue.getSize());
        ...
        if (!inBit.isGlobal() && !getResultBus().getValue().getBit(0).isGlobal())
        {
            /*
