...
public class EncodedMux extends Primitive
{
    ...
    */
    ...
    /**
     * Bit states are dont care if ALL data ports are dont care,
     * constant if all data ports are same constant and pass through
     * if all data ports are same pass through bit.
     */
    public boolean pushValuesForward ()
    {
        /*
         * The pushed value must be big enough to accommodate the
         * largest input.
         */
        int maxSize = 0;
        ...
        {
            /*
             * If the select value is constant, that means the
             * same input is always chosen, so push that one through.
             */
            final int selection = (int)(selectValue.getValueMask() & selectValue.getConstantMask());
            assert (selection >= 0) && (selection < getDataPorts().size()) : "select out of bounds: " + selection + " size: " + getDataPorts().size();
        ...
        {
            /*
             * Otherwise the input values have to be combined to produce
            ...
            for (int i=0; i < pushedValue.getSize(); i++)
            {
                ...
                do
                {
                    ...
                    final Bit padBit = portValue.isSigned() ? portValue.getBit(portValue.getSize()-1) : Bit.ZERO;
                    final Bit nextBit = i < portValue.getSize() ? portValue.getBit(i) : padBit;
                    if (bit == null) // initial condition
                    {
                        bit = nextBit;
                    }
                    if (bit != nextBit) // different bits. Default to CARE and jump out of loop
                    {
                        bit = Bit.CARE;
                    }
    ...
    /**
     * Clones this encoded mux to produce a copy with the same
     * characteristics.  
     *
     ...
     *
     * @return true if the output of this component can be reduced to
     * a constant or the value of a single input, based upon the
     * current {@link Value} of each data {@link Port}
     */
