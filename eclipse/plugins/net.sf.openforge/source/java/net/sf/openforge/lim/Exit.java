...
/**
 * An Exit is a group of Buses that represent an exit condition
 * from an Executable.  This includes the control signal as
 * well as all the data values that are output at that point.
 * A given {@link Component} may have multiple Exits.
 * <P>
 * Each Exit has an identifier called a {@link Exit.Tag Tag}.  A
 * {@link Exit.Tag Tag} consists of an Exit type (one of {@link Exit#DONE},
 * {@link Exit#BREAK}, {@link Exit#RETURN}, {@link Exit#CONTINUE}, or {@link Exit#EXCEPTION})
 * and an optional label String.  A {@link Component} may have only
 * one Exit with a given {@link Exit.Tag Tag}.  When added, a new {@link Exit}
 * will replace any existing {@link Exit} with the same {@link Exit.Tag Tag};
 * if assertions are turned on, this condition will also thrown an
 * {@link AssertionError}.
 *
...
public class Exit extends ID implements LatencyKey
{
    ...
    /** A Set of Entry objects identifying which entries list this
        Exit as their 'drivingExit' */
    private Set drivenEntries;
    ...
    /**
     * The type of Exit that represents the normal completion of a module,
     * for example, falling off the end of a sequence
     */
    ...
    public static class Tag
    {
        ...
        public boolean equals (Object obj)
        {
            ...
            {
                Tag tag = (Tag)obj;
                return (tag.getType() == getType()) && tag.getLabel().equals(getLabel());
            }
            return false;
        }
     ...
     *
     * @param type the exit type: {@link Exit#DONE}, {@link Exit#BREAK}, {@link Exit#RETURN},
     *          {@link Exit#CONTINUE}, {@link Exit#SIDEBAND}, or {@link Exit#EXCEPTION}
     ...
     *
     * @return the data Buses which are valid when the done Bus
     *         is asserted
     */
    ...
    /**
     * Gets the latency from the start of the Executable until
     * the activation of this exit.
     */
    ...
    /**
     * Sets the latency from the start of the Executable until
     * the activation of this exit.
     *
    ...
    /**
     * Gets the owner of this exit's done bus, which is considered
     * to be the owner of this exit.
     */
    ...
    /**
     * Gets the Component inside this Exit's owner which connects
     * it internally.
     *
    ...
    /**
     * Makes and returns a new data bus; if there is an internal
     * peer component, it receives a corresponding data port.
     *
    ...
    /** 
     * makes and returns a new data bus, and tags the bus (and port if there is an internal peer)
     *
     * @param tag type of bus
     */
    ...
    /**
     * Specifies that the given entry is one which lists this exit as
     * it's 'drivingExit'.
     *
    ...
    public void disconnect()
    {
         ...
         */
        //assert this.drivenEntries.size() == 0 : "Disconnecting an exit that is still listed as the driving exit of something. " + this + " drives " + drivenEntries;

        for (Iterator entryIter = (new LinkedList(this.drivenEntries)).iterator(); entryIter.hasNext();)
        {
            ((Entry)entryIter.next()).setDrivingExit(null);
        }
    }

    public Set getDrivenEntries()
    {
    ...
    /**
     * Constructs a new Exit with a done bus that is marked as not
     * used.
     *
     * @param owner the owner of the exit
     * @param dataCount the number of data buses to create
     * @param type the type of exit
     * @param label the label of the exit
     */
    ...
    /**
     * Constructs a new {@link Module} exit.  A peer {@link OutBuf} will also
     * be created inside the {@link Module}.
     *
     * @param owner the owner of the exit
     * @param dataCount the number of data buses to create
     * @param type the type of exit
    ...
    /**
     * Constructs a new {@link Exit#DONE} {@link Module} exit.  A peer {@link OutBuf} will also
     * be created inside the {@link Module}.
     *
     * @param owner the owner of the exit
     * @param dataCount the number of data buses to create
    ...
    /**
     * Copies the primitive attributes of a given Exit and
     * its {@link Bus Buses} to this Exit and its {@link Bus Buses}.
     */
