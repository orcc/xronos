...
/**
 * The SysgenSimApi is an output engine capable of creating the
 * necessary files for dropping a generated core into a SysGen
 * simulation.  The generated files include:
 * <ul>
 * <li><i>design</i>.cxx - this file is the configuration API C++
 * class for the design.  It contains all the functionality to
 * correctly configure the number and types (rate, data type, etc) of
 * ports on the core.  Additionally this file configures the relevant
 * simulation characteristics and synthesis file inclusion.
 * <li><i>design</i>_Model.cxx - this file is the simulation API C++
 * class for the design.  This file translates the System Generator
 * &tm; simulation API to the API used by our cycle accurate C
 * models.
 * <li><i>design</i>_createBlock.m - this file is an MCode
 * function/script which generates the library block in System
 * Generator &tm;
 * <li><i>design</i>_gui.xml - specifies the look and functionality of
 * the block configuration GUI.
 * <li><i>design</i>_declarations.xml - specifies any non-standard
 * variables that are passed between the System Generator &tm;
 * environment and the configuration API file <i>design</i>.cxx via
 * the 'mask'.
 * </ul>
 *
 * <p>Created: Wed Mar 22 13:05:00 2006
 *
...
{
    private static final ForgeFileKey SYSGEN_DIR = new ForgeFileKey("SysGen directory");
    public static final ForgeFileKey API_CXX = new ForgeFileKey("SysGen C++ class for interfaceing to SysGen");
    public static final ForgeFileKey MODEL_CXX = new ForgeFileKey("SysGen C++ class for tying our model to SysGen");
    public static final ForgeFileKey MCODE_BLOCK = new ForgeFileKey("SysGen createBlock mcode script");
    public static final ForgeFileKey GUI_XML = new ForgeFileKey("SysGen XML file for definition of the config panel GUI");
    public static final ForgeFileKey DECL_XML = new ForgeFileKey("SysGen XML file for declaration of vars");
    ...
    public void initEnvironment ()
    {
        ...
        this.fileHandler.registerFile(MCODE_BLOCK, base, this.fileHandler.buildName("_createBlock","m"));
        this.fileHandler.registerFile(GUI_XML, base, this.fileHandler.buildName("_gui","xml"));
        this.fileHandler.registerFile(DECL_XML, base, this.fileHandler.buildName("_declarations","xml"));
        this.fileHandler.registerFile(API_CXX, base, this.fileHandler.buildName("","cxx"));
        this.fileHandler.registerFile(MODEL_CXX, base, this.fileHandler.buildName("_Model","cxx"));
    ...
    public void translate (Design design) throws IOException
    {
        ...
        final String funcName = net.sf.openforge.backend.timedc.CNameCache.getLegalIdentifier(ID.showLogical(design));
        if (!baseDir.exists())
        {
            baseDir.mkdirs();
        }
        
        List<IOHandler> ioHandlers = new ArrayList();
        for (Object fifoIF : design.getFifoInterfaces())
        {
            ioHandlers.add(new IOHandler((FifoIF)fifoIF));
        }
        
        PrintStream mcodePS = new PrintStream(new FileOutputStream(this.fileHandler.getFile(MCODE_BLOCK)));
        writeCreateBlock(mcodePS, ioHandlers.size());
        mcodePS.close();
        
        PrintStream guiPS = new PrintStream(new FileOutputStream(this.fileHandler.getFile(GUI_XML)));
        writeGUI(guiPS);
        guiPS.close();

        PrintStream declPS = new PrintStream(new FileOutputStream(this.fileHandler.getFile(DECL_XML)));
        writeDeclarations(declPS);
        declPS.close();
        
        PrintStream apiPS = new PrintStream(new FileOutputStream(this.fileHandler.getFile(API_CXX)));
        writeAPI(apiPS, ioHandlers);
        apiPS.close();
        
        PrintStream modelPS = new PrintStream(new FileOutputStream(this.fileHandler.getFile(MODEL_CXX)));
     ...
     *
     * @param ps a <code>PrintStream</code> value
     * @param numPorts an <code>int</code> value, the number of ports
     * on the block (ports NOT pins)
     */
    private void writeCreateBlock (PrintStream ps, int numPorts)
    {
        ...
        ps.println("");
        ps.println("  Blk.gui_xml = '"+this.fileHandler.getFile(GUI_XML).getName()+"';");
        ps.println("  Blk.declarations_xml = '"+this.fileHandler.getFile(DECL_XML).getName()+"';");
        ps.println("");
    ...
    private void writeGUI (PrintStream ps)
    {
        ...
        ps.println("    <editbox name=\"infoedit\" read_only=\"true\" multi_line=\"true\" default=\"This block is the System Generator&tm; model of a core designed in the Actor/Dataflow network  methodology. &lt;P&gt;Design "+label+"&lt;P&gt;Generated on: "+runDate+" .\" evaluate=\"false\"/>");
        ...
        ps.println("  <hiddenvar name=\"block_config\" default=\"myDLL:"+this.configName+"\" evaluate=\"false\"/>");
        ...
        ps.println("  <hiddenvar name=\"block_type\" default=\""+this.baseName+"\" evaluate=\"false\"/>");
...
//         ps.println("	yack->setRate(y->getRate());");
        ...
        ps.println("        mask.set(\"simulation_model\", \""+this.baseName+":"+this.modelName+"\");");
        ps.println("    }");
        ps.println("    else {");
        ...
        ps.println("{");
        final File synFile = this.fileHandler.getFile(VerilogTranslateEngine.SYNPRIMINCL);
        final File hdlFile = this.fileHandler.getFile(VerilogTranslateEngine.VERILOG);
        ps.println("    addFile(\""+synFile.getName()+"\"); // copies the specified file INTO the generated HDL 'in place'");
        ps.println("    addFile(\""+hdlFile.getName()+"\"); // copies the specified file INTO the generated HDL 'in place'");
        ...
        ps.println("");
        ps.println("");
        ps.println("// The "+this.baseName+"_config symbol comes from the created block, via ");
        ps.println("// the .mdl file (which is created with the "+this.fileHandler.getFile(MCODE_BLOCK).getName()+" mcode");
        ps.println("EXPORT_SYSGEN_FACTORY ("+this.configName+", Sysgen::"+this.baseName+");");
    }

    private void writeModelAPI (PrintStream ps, List<IOHandler> ioHandlers, String funcName)
    {
        //final String simHeader = this.fileHandler.getFile(CycleCTranslateEngine.HEADER).getName();
        final String simHeader = this.fileHandler.getFile(CycleCTranslateEngine.SOURCE).getName();
        ...
        ps.println("class "+modelClassName+" : public Sysgen::ExternalSimulationModel<double> ");
        ps.println("{");
        ps.println("public:");
        ...
        ps.println("");
        ps.println("");
        ps.println(""+modelClassName+"::"+modelClassName+"(Sysgen::BlockDescriptor &bd) :");
        ...
        ps.println("    void* ("+this.modelName+")(Sysgen::BlockDescriptor &bd) ");
    ...
    private static class IOHandler
    {
        ...
        private String getDataName () { return base + (this.input ? "_din":"_dout"); }
        private String getSendName () { return base + "_snd"; }
        private String getAckName () { return base + "_ack"; }
        private String getHandleName () { return base + "Handle"; }
        ...
        void writeHandleDecl (PrintStream ps, String inType, String outType)
        {
            ...
            ps.println("    "+in+" *"+getDataName()+", *"+getSendName()+";");
        ...
        void writePortDecl (PrintStream ps)
        {
            if (this.input)
            {
                ps.println("    "+getDataName()+" = addInport(\""+getDataName()+"\", \""+getDataName()+"\");");
                ps.println("    "+getSendName()+" = addInport(\""+getSendName()+"\", \""+getSendName()+"\");");
                ps.println("    "+getAckName()+" = addOutport(\""+getAckName()+"\", \""+getAckName()+"\");");
            }
            else
            {
                ps.println("    "+getDataName()+" = addOutport(\""+getDataName()+"\", \""+getDataName()+"\");");
                ps.println("    "+getSendName()+" = addOutport(\""+getSendName()+"\", \""+getSendName()+"\");");
                ps.println("    "+getAckName()+" = addInport(\""+getAckName()+"\", \""+getAckName()+"\");");
            }
        }
        ...
        void writeRateAndType (PrintStream ps)
        {
            ...
            if (this.input)
            {
                ps.println("    "+getAckName()+"->setType(Sysgen::SysgenType(Sysgen::SysgenType::xlBOOL, 1, 0));");
                // Set the ACK rate to equal the data rate
                ps.println("    "+getAckName()+"->setRate("+getDataName()+"->getRate());");
            }
            else
            {
                ps.println("    "+getDataName()+"->setType(Sysgen::SysgenType(Sysgen::SysgenType::xlFIX, "+this.width+", 0));");
                ps.println("    "+getDataName()+"->setRate("+SysgenSimApi.CORE_OUTPUT_RATE+");");
                ps.println("    "+getSendName()+"->setType(Sysgen::SysgenType(Sysgen::SysgenType::xlBOOL, 1, 0));");
                ps.println("    "+getSendName()+"->setRate("+SysgenSimApi.CORE_OUTPUT_RATE+");");
            }
        }
        ...
        void writeHandleInit (PrintStream ps)
        {
            if (this.input)
            {
                ps.println("  "+getDataName()+"  = getInputPtr(\""+getDataName()+"\");");
                ps.println("  "+getSendName()+" = getInputPtr(\""+getSendName()+"\");");
                ps.println("  "+getAckName()+" = getOutputPtr(\""+getAckName()+"\");");
                ps.println("  "+getHandleName()+" = getInterfaceID(\""+this.base+"\");");
            }
            else
            {
                ps.println("  "+getDataName()+" = getOutputPtr(\""+getDataName()+"\");");
                ps.println("  "+getAckName()+" = getInputPtr(\""+getAckName()+"\");");
                ps.println("  "+getSendName()+" = getOutputPtr(\""+getSendName()+"\");");
                ps.println("  "+getHandleName()+" = getInterfaceID(\""+this.base+"\");");
            }
        }
        ...
        void writeOutputUpdate (PrintStream ps)
        {
            ...
            if (this.input)
            {
                ps.println("    *"+getAckName()+" = (unsigned)isAcking("+getHandleName()+");");
            }
            else
            {
                ps.println("    *"+getDataName()+" = (unsigned)getDataValue("+getHandleName()+", (int)*"+getAckName()+");");
                ps.println("    *"+getSendName()+" = (unsigned)isSending("+getHandleName()+");");
            }
        }
        ...
        void writeInputUpdate (PrintStream ps)
        {
            ...
            if (this.input)
            {
                ps.println("    setDataValue("+getHandleName()+", (int)*"+getDataName()+", (int)*"+getSendName()+");");
            }
            else
            {
                ps.println("    getDataValue("+getHandleName()+", (int)*"+getAckName()+");  // just to set the ack");
            }
        }
