...
public class Value
{
    private static final String rcs_id = "RCS_REVISION: $Rev: 41 $";
     ...
     *
     * isCare is TRUE if all bits are care, FALSE if all bits are not care, and ARRAY_VALID if
     *    isCareArray is valid.  isCareArray (if valid) is an array of 0..(size-1)
     *    where the index corresponds to bit position, and the value is true if
     *    the bit is constant, else false.
     *
     *                   current state  next state
     * setCare(i,careState)
     *                    const[i]=any    const[i]=NOT_CONSTANT
     *                    care[i]=any     care[i]=careState
     *                    owner[i]=any    owner[i] unchanged
     */

    /**
     * bits 1-2 of state
     *
     * similar to isCare, but describes constant bits.  if isConstant=NOT_CONSTANT there
     *    are no constant bits, if ONE then all constant bits, if 2 then some
     *    constant bits.  isConstantArray is defined for isConstant > 0,
     *    isConstantArray contains ZERO or ONE or NOT_CONSTANT for non-constant bits
     *
     *                   current state  next state
     * setConstant(i,constState)
     *                    care[i]=any     care[i]=true
     *                    const[i]=any    const[i]=constState
     *                    owner[i]=any    owner[i]=null
     */

    /**
     * bit 3 of state
     *
     * similar to isCare, but describes global bits - derived from owner/ownerArray
     */

    /**
     * holds the bus that "owns" this value, if null then consult ownerArray.
     *    if ownerArray is null then there is no owner for any bit; if owner
     *    array is not null, then each element corresponds to the owner of a
     *    particular position.  ownerArray is only defined if owner==null
     *
     *                   current state  next state
     * setOwner(i,bus)
     *                    const[i]=any    const[i]=NOT_CONSTANT
     *                    care[i]=any     care[i] unchanged
     *                    owner[i]=any    owner[i]=bus
     *
    ...
    /**
     * The array of bits that comprise this value - used only if bits need to
     * be generated - it is expected that this will be deprecated
     */
     ...
     *
     * @param size the number of bits in this value
     * @param isSigned true if this is a signed value, false if unsigned
     */
    public Value (int size, boolean isSigned)
    {
        ...
        {
            state[i]=CARE_MASK|GLOBAL_MASK|CONSTANT_NOT|(i<<4)|(i<<10);
        }
        this.size=size;
     ...
     *
     * @param bus the bus that owns all bits
    ...
    public Value (Value value, boolean isSigned)
    {
        ...
        if (value.ownerInvArray != null)
        {
            ...
            System.arraycopy(value.ownerInvArray, 0, this.ownerInvArray, 0, size);
        }
        this.isSigned=isSigned;
    }
     ...
     *
     * @param position the index of the bit, 0 <= position < {@link #getSize()}
     * @param care true if care, false if don't care
     */
    public void setCare (int position, boolean care) throws IndexOutOfBoundsException
    {
        /* RT-72
        setConstant(position, NOT_CONSTANT);//so a constant to care works
        */
     ...
     *
     * @param position the index of the bit, 0 <= position < {@link #getSize()}
     * @param constant ZERO or 1 if constant, NOT_CONSTANT if not constant
     */
    public void setConstant (int position, int constant) throws IndexOutOfBoundsException, IllegalArgumentException
    {
        assert constant >= ZERO && constant <= NOT_CONSTANT:"Illegal argument "+constant+" to setConstant";
        ...
        switch (constant)
        {
            case ONE: state[position] |= CONSTANT_ONE; break;
            case NOT_CONSTANT: state[position] |= CONSTANT_NOT; break;
        }

        //owner to be set to null
        state[position] |= GLOBAL_MASK;
        if (owner != null) //if owner not null then all bits owned by the same bus
        {
        ...
        else if (ownerArray != null)
        {
            ownerArray[position]=null;
            updateOwner();
        }  // else both owner and ownerArray are null and the bit was already global
    }

    /**
     * sets the owner of a specific bit
     * @param position bit to modify
     * @param owner of the bit
     */
    public void setOwner (int position, Bus owner) throws IndexOutOfBoundsException
    {

        // nothing for care bits

/*      RT-72
        // set constant to NOT_CONSTANT if owner != null
        if (owner != null)
        {
            state[position] |= NOT_CONSTANT;
        }
*/
     ...
     * @param position in this Value
     * @param owner source of the bit
     * @param ownerPosition offset in the owner
     */
     ...
     * @param i index of the bit in this Value
     */
     ...
     * @param position bit to modify
     * @param owner of the bit
     * @param ownerPosition position in owner of the bit
     */
    public void setInvertedOwner(int position, Bus owner, int ownerPosition) throws IndexOutOfBoundsException
    {
        ...
        // TBD: do we need to maintain care/constant/global status for inverted bits?
    ...
    /**
     * assumes ownerArray != null, are all entries the same?
     * also updates isGlobal/isGlobalArray
     */
    ...
    {
        assert position < size : "Attempting to get a bit which is too large for this value " + position + " " + size;

        if (isGlobal(position))
        {
            /* RT-72
               Since value is not made NOT_CONSTANT by modifying care, a value can both be ON/OFF and don't care.
               Hence check for don't care before checking for ON/OFF
            */
            if(!isCare(position))
            {
    ...
    public void setBit (int position, Bit bit)
    {
        ...
        /* RT-72
           Call setCare() after setConstant() as setConstant always sets the care bit to true
        */
        ...
        if (inverted != null)
        {
            setInvertedOwner(position, inverted.getOwner(), inverted.getPosition());
        }
    }
     ...
     * @param valuePosition offset in source value
     */
    public void setBit (int position, Value value, int valuePosition)
    {
        ...
        setOwner(position, value.getOwner(valuePosition));//, value.getPosition(valuePosition));
    }
    ...
    /**
     * Gets the current vector of constant/non-constant states as
     * a mask.
     */
    ...
    /**
     * Returns true if all Bits in this Value are ONE, ZERO, or
     * DONT_CARE.
     *
    ...
    /**
     * Tests whether all the care bits are of the same type
     * (constants or non-constants from the same source Bus).
     * Also considers any non-contiguous series of care bits
     * to be mixed.  Also considers any sign extended value to be
     * mixed.
     *
     * @return false if all the Bits are the same type; otherwise true
     *
     * @throws IllegalStateException if the value is not constant, but
     *           none of the Bits has an owner Bus
     */
    public boolean isMixed ()
    {
        ...
        {
            /*
             * It's mixed if we find any constant bits, or if there are
             * Bits from more than one owner Bus.
             */
            Bus bitOwner = null;
    ...
    public String debugState ()
    {
        ...
        for (int i=0; i<size; i++)
        {
            result+="\t"+Integer.toHexString(state[i])+" "+getInvertedOffset(i)+" "+getPosition(i)+" "+isGlobal(i)+" "+getConstant(i)+" "+isCare(i)+"\n";
        }
        return result;
    }
...
//         for (int i=0; i<size; i++)
        ...
        for (int i=0; i < size; i++)
        {
            ...
            if (isGlobal(i))
            {
                ...
                while (isCare(i)==care && getConstant(i)==constant && i < size && isGlobal(i))
                {
                    if(!care)
                        group.add("x");
                    else if(constant == 0)
            ...
            else
            {
                ...
                while (getOwner(i) == currentSource &&
                       getPosition(i) == (startOffset + count) &&
                       i < size)
                {
                    owner=currentSource;
                    if (start == -1)
    ...
    /**
     * Returns true if all the bits are equivalent which means that
     * they are the SAME bit unless one is {@link Bit#CARE} in which
     * case it is equivalent to any other non-constant non-dontcare
     * bit.
     *
     * @param test the Value to test against this one
    ...
    public boolean equivalent (Value test)
    {
        ...
        for (int i=0; i<size; i++)
        {
            ...
            else if (testIsGlobal && testIsCare && testConstant==NOT_CONSTANT)
            {
                if (localConstant==ONE || localConstant==ZERO || !localIsCare)
                {
                    return false;
                }
     ...
     * @param position the bit of this value to compare
     * @param test the value to compare with
     * @param testPosition the bit of the test value
     */
    public boolean bitEquals (int position, Value test, int testPosition)
    {
        ...
        if (isGlobal(position) && getConstant(position) != test.getConstant(testPosition))
        {
            return false;
        }
        ...
        if (getInvertedOwner(position) != null &&
            getInvertedOffset(position) != test.getInvertedOffset(testPosition))
        {
            return false;
        }
        return true;
     ...
     * @param position the bit of this value to compare
     * @param test the Bit to compare with
     */
    ...
    /**
     * Joins the values by following these rules for each bit
     * position:
     * <p><ul>
     * <li> if either bit is a care, result bit is a care
     * <li> if either bit is non constant, result bit is not constant
     * <li> if both bits are constant AND same, result bit is that
     * constant
     * <li> if either value is signed, then the joined value is signed
     * </ul>
     *
     * @param value the Value to union with this Value.
     * @return a new Value representing the state achieved after
     * merging this Value with the specified one.
     */
    public Value union (Value value)
    {
        assert value.getSize() == size : "size mismatch: " + value.getSize() + ", " + size;
        assert value.isSigned() == isSigned : "sign mismatch: " + value.isSigned() + ", " + isSigned;
    ...
    /**
     * Returns the minimum number of care bits of which is significant
    ...
    {
        System.err.println("bit "+i+" isGlobal "+isGlobal(i)+" isCare "+isCare(i)+" getConstant "+getConstant(i)+" isGenericCare "+isGenericCare(i)+" owner "+getOwner(i)+" offset "+getPosition(i));
    }

    /**
     * Gets the {@link SizedInteger} representation of this value.  Any
     * <code>DONT_CARE</code> bits will be represented as 0.  This value
     * must not contain non-global bus bits.
     *
     * @return a <code>SizedInteger</code> representation
     * @throws IllegalStateException if this value contains any non-global
     *           bits
     */
     ...
     *
     *
     * @param token a string consisting of a sequence of '1', '0', 'c', or 'x', one
     *          for each bit; the beginning of the string corresponds to the most
     *          significant bit
     * @return a new value with the corresponding bits
     * @exception IllegalArgumentException if an invalid character appears in
     *              the sequence
     */
    public static Value parseValue (String token) throws IllegalArgumentException
    {
        ...
        for (int i = sequence.length - 1; i >= 0; i--)
        {
            ...
            switch (sequence[i])
            {
                ...
                case 'x':
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("invalid bit specifier: " + token.charAt(i));
            }
    ...
    /**
     * Utility for obtaining a Value object whose state is all
     * constant according to the parameter value.
     */
    public static Value getConstantValue (long constant, int minSize, boolean isSigned)
    {
        ...
        if (size < minSize)
        {
            final int padBit = (isSigned && (size > 0)) ? value.getConstant(size - 1) : 0;
     ...
     *
     * @param value the Value to genericize.
