...
/**
 * ActorScalarOutput is a specialized fifo output interface which
 ...
 *
 *
 * <p>Created: Fri Aug 26 15:14:55 2005
 *
...
public class ActorScalarOutput extends FifoOutput implements ActorPort
{
    ...
    /**
     * ActorScalarOutput ports have no special naming requirements,
    ...
    /**
     * Returns a {@link FifoWrite} object that is used to obtain data
     * from this FifoIF.
     *
     ...
     * from this FifoIF.
     *
     * @param blocking if set true returns a blocking fifo write
     * otherwise a non-blocking access.
    ...
    /** Returns the input ready pin, indicating that the queue is
     * ready to accept at least one token. */
    public SimplePin getReadyPin () { return this.rdy; }
    ...
    {
        throw new UnsupportedOperationException("Output channels do not have token count facility");
    }

    public Component getPeekAccess ()
    {
        throw new UnsupportedOperationException("Peeking at output interface not yet supported");
        //return new ActionTokenPeek(this);
    }

    public Component getStatusAccess ()
    {
        //throw new UnsupportedOperationException("Status of output interface not yet supported");
    ...
    private class ActorScalarOutputWrite extends FifoWrite
    {
        private ActorScalarOutputWrite (ActorScalarOutput aso)
        {
             ...
             * dataLatch = writeData(port) : en GO
             * fifo_data = writeData (port);
             * pending = (GO || GO');
             * send = pending
             * GO' <= pending & !ack
             * write_done = pending & ack
             */
            // Needs RESET b/c it is in the control path
            final Reg flop = Reg.getConfigurableReg(Reg.REGR, "fifoWritePending");
            final Or pending = new Or(2);
            final And done_and = new And(2);
            final And flop_and = new And(2);
            final Not not = new Not();
            final CastOp doutCast = new CastOp(aso.getDataPin().getWidth(), false);
            ...
            final Constant index1 = new SimpleConstant(1, aso.tokenCount.getWidth());
            ...
            countWrite.getGoPort().setBus(index1_1.getValueBus()); // ALWAYS enabled
        }
    }
