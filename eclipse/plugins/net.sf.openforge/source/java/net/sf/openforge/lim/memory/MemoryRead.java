...
public class MemoryRead extends MemoryAccess implements StateAccessor
{
    ...
    /**
     * This accessor forces contention on the {@link Referenceable}
     * target so it may not execute in parallel with other accesses.
     */
    ...
    /**
     * Returns a copy of this MemoryRead by creating a new memory read
     * off of the {@link MemoryPort} associated with this node.  We
     * create a new access instead of cloning because of the way that
     * the MemoryPort stores references (not in Referent).  Creating a
    ...
    public Object clone ()
    {
        ...
        final MemoryRead clone = new MemoryRead(isVolatile(), getWidth(), this.isSigned());
    ...
    public class Physical extends PhysicalImplementationModule
    {
         ...
         * port-bus connections of the MemoryRead.
         */
        public Physical ()
        {
            super(0); 
            ...
            Bus memReadData = (Bus)MemoryRead.this.getExit(Exit.DONE).getDataBuses().get(0);
            ...
            for (Iterator dataConsumers = memReadData.getPorts().iterator(); dataConsumers.hasNext();)
            {
                Port consumer = (Port)dataConsumers.next();
                consumer.setBus(dataOut);
            }
            
            // appropriate the done bus
            Bus done = physicalExit.getDoneBus();
            Bus memReadDone = (Bus)MemoryRead.this.getExit(Exit.DONE).getDoneBus();
            done.setUsed(memReadDone.isUsed());
            done.setIDLogical(ID.showLogical(memReadDone));
            for (Iterator doneConsumers = memReadDone.getPorts().iterator(); doneConsumers.hasNext();)
            {
                Port consumer = (Port)doneConsumers.next();
            ...
            sideAddress.setSize(memReadAddress.getBus().getSize(), memReadAddress.getBus().getValue().isSigned());
            ...
            sideSize.setSize(MemoryRead.this.getSizePort().getBus().getSize(),
                MemoryRead.this.getSizePort().getBus().getValue().isSigned());
            sideSize.getPeer().setBus(sizeIn.getPeer());
            
            /*
             * Cast the data returned by the memory to the requested
             * size.  This is because the side data bus connected to the memory
             * is initially sized for the largest possible data value.
             */
            final CastOp castOp = new CastOp(dataOut.getValue().getSize(), dataOut.getValue().isSigned());
            addComponent(castOp);
    ...
    /**
     * This class implements the logic and connectivity for the
     * 'done caching' logic in the read.  It is basically a register
     * whose set and reset pins are manipulated (Q feeds back to D).
     * The incoming GO set's the register.  The incoming done (from
     * the memory) is ANDed with the register output to produce the
     * done for this read.  The output of the AND is also used to
    ...
    public class DoneCache
    {
        ...
        public DoneCache (Module owner)
        {
            ...
            ((Port)resetOr.getDataPorts().get(0)).setBus(doneAnd.getResultBus());
            
            /*
             * XXX: It seems the internal reset is what's intended, but
             * just to be safe, connect the regular reset as well.
             */
     ...
     * whose set and reset pins are manipulated (Q feeds back to D).
     * The incoming GO set's the register.  The incoming done (from
     * the memory) is ANDed with the logical OR of the register output
     * and the incoming GO in order to produce the done for this read.
     * The output of the AND is also used to clear the register by
     * being ORed with the global RESET.  This class differs in that
     * there is the OR gate beween the register output and the AND
     * which includes the incoming GO so that combinational accesses
    ...
    public class ZeroPlusDoneCache extends DoneCache
    {
        ...
        public ZeroPlusDoneCache (Module owner)
        {
            ...
            ((Port)doneAnd.getDataPorts().get(0)).setBus(combOr.getResultBus());
            
            ((Port)combOr.getDataPorts().get(0)).setBus(goRegister.getResultBus());
        }
    ...
    /**
     * This class implements the done caching logic necessary for read
     * accesses that complete combinationally.  All it does is
     * logically AND the read's GO with the memory's DONE.
     *
