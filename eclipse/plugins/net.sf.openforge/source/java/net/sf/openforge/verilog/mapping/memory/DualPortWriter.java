...
/**
 * DualPortWriter is the super class for all Dual Ported memory
 * writers and allocates ports for each input/output, is responsible
 * for instantiation of the module, and contains utility code for
 * cloning out the necessary RAM primitives and assigning initial
 * values to each.
 *
...
{
    private static final String _RCS_ = "$Rev: 493 $";
    ...
    public DualPortWriter (MemoryBank memBank)
    {
        ...
        mpA = new MemoryPortDef(pA, "A", 0);
        mpB = new MemoryPortDef(pB, "B", 1);
        
        //this.moduleName = "forge_dualport_memory_" + getDepth() + "x" + getDataWidth() + "_"+ memory_module_id++;
        //this.moduleName = "forge_dualport_memory_" + getDepth() + "x" + getDataWidth() + "_"+ memory_module_id++;
        //this.moduleName = memBank.showIDLogical() + "_"+ memory_instance_id++;
    ...
    private MemoryBank.BankPort getPortA (MemoryBank bank)
    {
        ...
        MemoryBank.BankPort pA = (MemoryBank.BankPort)bank.getBankPorts().get(0);
        MemoryBank.BankPort pB = (MemoryBank.BankPort)bank.getBankPorts().get(1);
    ...
    {
        MemoryInstance memoryInstance = new MemoryInstance(getName(), getName() + "_instance" + memory_instance_id++);
...
//         Net doneOutWireA = new BusWire(implementationA.getExit(com.xilinx.hllc.lim.Exit.DONE).getDoneBus());
...
//         instance.addProducedNet(doneOutWireA);
    ...
    /**
     * Utility method for copying out the rams and populating each
     * with the appropriate initial bits.
     *
     * @param type the {@link DualPortRam} that will be cloned to
     * populate the implementation.
     * @param initVals a 'BitSet' of the initial values for the
     * impelmentation
     * @param portA the {@link MemoryPortDef} for port A.
     * @param portB the {@link MemoryPortDef} for port B.
     * @return a 'RamImplementation' containing the array of
     * implementation Rams and any extra bit wires.
     */
    protected RamImplementation getImplementationRams (DualPortRam type,
        MemoryPortDef portA, MemoryPortDef portB)
    {
        int resultWidth = (int)java.lang.Math.ceil((double)getDataWidth()/(double)type.getWidth());
        int resultDepth = (int)java.lang.Math.ceil((double)getDepth()/(double)type.getDepth());
        ...
        for (int w = 0; w < resultWidth; w++)
        {
            for( int d = 0; d < resultDepth; d++)
            {
                ...
                // them all to it.
                new_element.setAddr(portA.getADRName(), getAddrWidth(), 0, getAddrWidth() == 1, portA.whichPort());
                new_element.setAddr(portB.getADRName(), getAddrWidth(), 0, getAddrWidth() == 1, portB.whichPort());

                // We don't give the memory a signal if we aren't
                // given one, then it will write out zeros for us
                if (portA.writes())
                {
                    new_element.setDataInName(portA.getDINName(), portA.whichPort());
                }
                if (portB.writes())
                {
                    new_element.setDataInName(portB.getDINName(), portB.whichPort());
                }
                if (portA.reads())
                {
                    new_element.setDataOutName("pre_douta_" + d, portA.whichPort());
                    new_element.setDataOutExtraName("extrasa_" + d, portA.whichPort());
                }
                if (portB.reads())
                {
                    new_element.setDataOutName("pre_doutb_" + d, portB.whichPort());
                    new_element.setDataOutExtraName("extrasb_" + d, portB.whichPort());
                }
                
                new_element.setDataInStartBit(w * new_element.getWidth(), portA.whichPort());
                new_element.setDataInStartBit(w * new_element.getWidth(), portB.whichPort());
                new_element.setDataOutStartBit(w * new_element.getWidth(), portA.whichPort());
                new_element.setDataOutStartBit(w * new_element.getWidth(), portB.whichPort());
                
                if (((w + 1) * new_element.getWidth()) <= getDataWidth())
                {
                    new_element.setDataAttachWidth(new_element.getWidth(), portA.whichPort());
                    new_element.setDataAttachWidth(new_element.getWidth(), portB.whichPort());
                }
                else
                {
                    ...
                    new_element.setDataAttachWidth(new_element.getWidth() - spareBits, portA.whichPort());
                    new_element.setDataAttachWidth(new_element.getWidth() - spareBits, portB.whichPort());

                    // We have spare bits, store the extra name to
                    // declare later.  Not used???
                     if (portA.reads())
                     {
                         extra_douta_wires[d] = new Wire("extrasa_" + d, spareBits);
                     }
                     if (portB.reads())
                     {
                         extra_doutb_wires[d] = new Wire("extrasb_" + d, spareBits);
                     }
                }
                
                new_element.setDataScalar(getDataWidth() == 1, portA.whichPort());
                new_element.setDataScalar(getDataWidth() == 1, portB.whichPort());
            }
        }
        ...
        {            
            for (int ed = 0; ed < ram_array[0][0].getDepth(); ed++)
            {
                for (int w = 0; w <resultWidth; w++)
                {
                    ...
                    {
                        if ((w * element.getWidth() + b) < getDataWidth() && (d * element.getDepth() + ed) < getDepth())
                        {
                            element.setNextInitBit(initVals.get(currentBit) ? 1:0);
                            currentBit++;
                        }
                        else
                        {
        ...
        return new RamImplementation(ram_array, extra_douta_wires, extra_doutb_wires);
    }
    ...
    class MemoryPortDef
    {
        ...
        public MemoryPortDef (MemoryBank.BankPort mp, String id, int which)
        {
            ...
            adr = new Input(VerilogMemory.adr + id, DualPortWriter.this.getAddrWidth());
            din = new Input(VerilogMemory.din + id, DualPortWriter.this.getDataWidth());
            dout = new Output(VerilogMemory.dout + id, DualPortWriter.this.getDataWidth());
