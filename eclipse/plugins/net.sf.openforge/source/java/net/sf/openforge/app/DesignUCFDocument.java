...
import net.sf.openforge.backend.hdl.ucf.*;
...
public class DesignUCFDocument extends UCFDocument implements OutputEngine
{
    ...
    public static final ForgeFileKey UCF_FILE = new ForgeFileKey("Design UCF File");
    ...
    public void initEnvironment ()
    {
        ...
        final String filename = gj.getOption(OptionRegistry.UCF_FILE).getValue(CodeLabel.UNSCOPED).toString();
        final File testFile = new File(filename);
        if (testFile.isAbsolute())
            gj.getFileHandler().registerFile(UCF_FILE, testFile.getParentFile(), testFile.getName());
        else
        {
            // Follow the HDL source
            File hdlTarget = gj.getFileHandler().getFile(VerilogTranslateEngine.VERILOG);
            gj.getFileHandler().registerFile(UCF_FILE, hdlTarget.getParentFile(), filename);
        }
    }
    
    public void translate (Design design) throws IOException
    {
        File ucfFile = EngineThread.getGenericJob().getFileHandler().getFile(UCF_FILE);
     ...
     */
    public String getOutputPhaseId () { return "UCF Document"; }
    
    /**
     *
     * @param genJob the GenericJob from which the design was derived
    ...
    private void includeUserUCF()
    {
        ...
        Option op = EngineThread.getGenericJob().getOption(OptionRegistry.UCF_INCLUDES);
        ucf_list.addAll(((OptionMultiFile)op).getValueAsList(CodeLabel.UNSCOPED));
        ...
        for(Iterator it = ucf_list.iterator(); it.hasNext(); )
        {
            ...
            try
            {
                ...
                comment("Including user specified ucf file: " + userUcfFile.getAbsolutePath());
    ...
    private void addApiPin(Buffer apiPin, int size, String netName)
    {
        if (size == 1)
        {
            if ((apiPin != null) && (apiPin.getUCFAttributes().size() > 0))
            {
                ...
                {
                    state(new UCFArbitraryStatement(
                              "NET \"" + netName + "\"" +
                              "\t" +
                              attrs.next().toString()));
                }
            ...
            {
                comment(new UCFArbitraryStatement(
                            "NET \"" + netName + "\"" +
                            "\t" +
                            "IOSTANDARD=LVTTL"));
                comment(new UCFArbitraryStatement(
        ...
        else
        {
            if ((apiPin != null) && (apiPin.getUCFAttributes().size() > 0))
            {
                for (Iterator attrs = apiPin.getUCFAttributes().iterator(); attrs.hasNext();)
                {
