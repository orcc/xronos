 ...
 * {@link Component Component}s.  At most one source {@link Bus Bus}es
 * may be connected to a Port.
 * <P>
    ...
    extends ID
{
    ...
    /**
     * The peer bus of this port, if any, that continues it across
     * a Module boundary
     */
     ...
     * @param isSigned true if this port represents a signed value,
     *          false if unsigned
     ...
     * @throws NullPointerException if {@link #setSize(int,boolean)} has not
     *           yet been called
     */
     ...
     *         or null if {@link #setSize(int,boolean)} has not yet been called
     */
    ...
    /**
     * Updates this port's {@link Value} with its incoming values
     * during forward constant propagation.
     * <P>
     * If this port is connected, then there is only a single incoming
     * value, that of the connected {@link Bus}.  This value us used
     * as the incoming value.
     * <P>
     * Otherwise, if this port is not connected, then the incoming
     * value must be determined by looking at the {@link Bus Buses}
     * on which this port depends.  A copy of each bus's value is
     * made and any bus bits are replaced with {@link Bit#CARE} bits
     * (because scheduling has not yet run, so the bus bits are not
     * yet valid).  If there is only one dependency value, that is used
     * as the incoming value.  Multiple values must instead be combined.
     * This combining process is performed on a {@link Bit}-wise basis.
     * For each bit position, the union is defined as follows for the set
     * of bits at that position:
     * <P>
     * <ol>
     * <li>If all of the bits are {@link Bit#DONT_CARE}, then the union of
     *       those bits is {@link Bit#DONT_CARE}; otherwise, discard the
     *       {@link Bit#DONT_CARE} bits and compute the union on those that
     *       remain
     * <li>If all the remaining bits are the same bit object, then the union
     *       is that bit; otherwise, the union is {@link Bit#CARE}
     * </ol>
     * <P>
     * Whether derived from a bus connection or dependencies, the single
     * incoming value is then made the current value of this port by
     * copying its {@link Bit Bits} to this port's {@link Value}, BUT
     * ONLY IF THE CURRENT {@link Bit} IS NOT {@link Bit#DONT_CARE}.  If
     * this port does not yet have a value, one is first created by calling
     * {@link #setSize(int,boolean} with the incoming value's size and
     * sign-edness.
     *
     * @return true if this port's value has changed as a result of this
     *           method, false if it remains unchanged
     */
    public boolean pushValueForward ()
    {
        ...
        else
        {
            ...
            for (Iterator entryIter = getOwner().getEntries().iterator(); entryIter.hasNext();)
            {
                Entry entry = (Entry)entryIter.next();
                for (Iterator depIter = entry.getDependencies(this).iterator(); depIter.hasNext();)
                {
                    ...
                    {
                        /*
                         * We have to allow for inputs that might not have values
                         * yet, i.e., those providing feedback signals.
                         */
                        vlist.add(v);
                    }
                    else
            ...
            for (int i = 0; i < derived.getSize(); i++)
            {
                ...
                if((testBucket.remove(Bit.DONT_CARE))&&(testBucket.size()==0))
                {
        ...
        {
            /*
             * Allocate a value even if not all the inputs are
             * available yet.  At least we know the size.
             */
            setSize(derived.getSize(), derived.isSigned());
            isModified = true;
        }
        if (derived.getSize()!=getValue().getSize())
            throw new SizeMismatchException("derived: "+derived.getSize()+" current: "+getValue().getSize());
        //assert derived.getSize()==getValue().getSize(): "derived: "+derived.getSize()+" current: "+getValue().getSize();

        /*
         * If not all inputs have values yet, as can be the
         * case when there is feedback, exit now.
         */
        if (!isInputComplete)
        {
            return isModified;
        ...
        for (int i = 0; i < getSize(); i++)
        {
             ...
             * the incoming Bit, then replace the current Bit with the incoming Bit.
             */
            if (!getValue().bitEquals(i, derived, i) && currentBit != Bit.DONT_CARE && !currentBit.isConstant())
            {
                isModified = true;
     ...
     * during reverse constant propagation.  Only don't-care bits are
     * propagated backwards.  Therefore, for each {@link Bit} of a
     * given update value, if the bit is {@link Bit#DONT_CARE}, then the
     * bit at the same position of this port's value is also set to
     * {@link Bit#DONT_CARE}.  Otherwise no modifications are made to
     * this port's value.
     *
     * @param value the value whose don't-care bits are copied to
     *          this port's value
     *
     * @return true if this port's value was modified, false otherwise
     * @throws IllegalStateException if {@link #forceValue(Value)} has been
     *           previously called on this port
     * @throws NullPointerException if <code>v</code> is null
     */
    ...
    /**
     * Forces the current value of this port to a given value.
     ...
     * before translation.
     *
     * @param value the value to be forced into this port
     * @throws NullPointerException if the given value is null
     * @throws IllegalStateException if this port does not currently
     *           have a value
     */
     ...
     *
     * @param owner the component to which this port belongs
     */
    ...
    /**
     * Gets the peer Bus, if any, that connects this Port across
     * a module boundary.
     *
    ...
    /**
     * Sets the Bus to which this Port is connected and reconciles the
     * Value on the Bus with the Value on the Port. If this port has
     * a peer, set the peer to contain the bus's information.
     *
     * @param bus the Bus to connect, or null if not connected
     */
    ...
    /**
     * Copies the primitive attributes of a given Port to this one.
     * These attributes include {@link Port#isUsed()},
     * {@link Port#getTag()}, and {@link Port#getValue()}.  Used during cloning.
     */
    ...
    /**
     * Sets the peer Bus, if any, that connects this Port across
     * a module boundary.
     *
     * @param peer the peer bus, or null if there is none
     */
     ...
     * particularly useful when they are attached to the body of a
     * procedure and represent the input args.
     *
