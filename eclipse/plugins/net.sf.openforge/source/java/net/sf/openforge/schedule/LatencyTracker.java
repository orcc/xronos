...
/**
 * LatencyTracker records control information for various points in a
 * LIM graph.  It allows a control Bus to be specified for the
 * entry to each {@link Component} and for every {@link Exit} of a
 * {@link Component}.  It can also fetch new scheduling hardware
 * components from its internal {@link OpCache}.  The control Buses
 * of these components are updated automatically when they are fetched.
 *
...
public class LatencyTracker implements LatencyCache
{
    ...
    /**
     * Creates a new LatencyTracker with the same OpCache as the
     * 'basis' LatencyTracker.
     *
     * @param basis a value of type 'LatencyTracker'
     */
     ...
     *
     * @param bus the bus that indicates the flow of control
     * @param latency the latency of the bus
     */
    ...
    /**
     * Gets the result of inserting a Reg to a given data {@link Bus}.  A control Bus
     * is calculated for the Reg's Exit.
     *
     * @param dataBus the data bus to Reg
     * @param module the module to which the Reg will be added
     * @return the Reg
     */
    Reg getEnableReg (Bus dataBus, Module module)
    {
        ...
        if (enableRegs.contains(busOwner) && (getControlBus(busOwner) == enableBus))
        {
            /*
    ...
    /**
     * Gets the result of latching a given data {@link Bus}.  A control Bus
     * is calculated for the Latch's Exit.
     *
     * @param dataBus the data bus to latch
     * @param module the module to which the latch will be added
     * @return the latch
     */
    Latch getLatch (Bus dataBus, Module module)
    {
        final Bus enableBus = getControlBus(dataBus);
        final Component busOwner = dataBus.getOwner().getOwner();
        if (latches.contains(busOwner) && (getControlBus(busOwner) == enableBus))
        {
            /*
             * Instead of latching the output of a latch with the same enable
             * bus, just return the original latch.
    ...
    /**
     * Gets the result of scoreboarding a collection of control {@link Bus Buses}.
     * A control Bus is calculated for the Scoreboard's Exit.
     *
     * @param controlBuses a collection of control {@link Bus Buses}
     * @param module the module to which the scoreboard will be added
    ...
    Scoreboard getScoreboard (Collection controlBuses, Module module)
    {
        ...
        defineControlBus(scoreboard.getResultBus(), Latency.and(latencies, exit));
        return scoreboard;
    }
    ...
    Stallboard getStallboard (Collection controlBuses, Module module)
    {
        ...
        /*
         * The latency of a stall board is always open because the
         * stall input is feedback and cannot be compared to any other
         * latency. 
         */
        defineControlBus(stallboard.getResultBus(), Latency.and(latencies, exit).open(exit));
        return stallboard;
    }
     ...
     *
     * @param inputBus the data bus to be registered
     * @param module the module to which the new register (and control flop register)
     *          will be added
     */
    Reg getDataReg (Bus inputBus, Module module)
    {
        ...
        /*
         * We can only set the size of a bus once, so check to see if it
         * was done already, since we might be fetching a register that was
         * already cached.
         */
        if (reg.getResultBus().getValue() == null)
        {
            final Value inputValue = inputBus.getValue();
            reg.getResultBus().setSize(inputValue.getSize(), inputValue.isSigned());
        }
    ...
    /**
     * Gets the {@link Reg} that results from delaying a given control
     * input {@link Bus} by a single clock.  The output Bus of the
     * Reg will be defined to be a control Bus.
     *
     * @param inputBus the control bus to be delayed by one clock
     * @param module the module to which the register will be added
    ...
    /*
     * Gets the {@link Bus} that results from delaying a given control
     * {@link Bus} by a specified number of clocks.
     *
    ...
    /*
     * Gets the {@link Bus} that results from delaying a given data
     * {@link Bus} by a specified number of clocks.
     *
     * @param inputBus the control bus to be delayed
     * @param module the module to which new registers will be added
    ...
    /**
     * Gets the result of ORing a collection of control {@link Bus Buses}.
     * A control Bus for the Exit of the Or will also be calculated.
     *
     * @param controlBuses a collection of control {@link Bus Buses}
     * @param module the module to which the Or will be added
    ...
    /**
     * Gets the result of ANDing a collection of control {@link Bus Buses}.
     * A control Bus for the Exit of the And will also be calculated.
     *
     * @param controlBuses a collection of control {@link Bus Buses}
     * @param module the module to which the And will be added
    ...
    /**
     * Gets the result of MUXing a collection of data {@link Bus Buses}.
     * A control Bus will also be calculated for the Exit of the Mux.
     *
     * @param selectBuses a list of control {@link Bus Buses}, one for each data bus
     * @param dataBuses a list of data {@link Bus Buses}, one for each select bus
     * @param module the module to which the Mux will be added
    ...
    /**
     * Calculates a control Bus for each Exit of a given Component
     * based upon its entry control Bus and the relative latencies
     * of its Exits.
     */
    void updateExitStates (Component component)
    {
        ...
        for (Iterator iter = component.getExits().iterator(); iter.hasNext();)
        {
            ...
            if (debug) System.out.println("updateExitState " + component + " " + exit + " exitLat " + exitLatency + " entryLat " + entryLatency + " cumLat " + cumulativeLatency);
            ...
            {
                if (component.isDoneSynchronous() || isPlaceholder(entryControlBus))
                {
                    if (debug) System.out.println("\town done and isSync");
                    /*
                     * If the component produces a clocked done or if the entry bus
                     * doesn't supply a control signal, then use the exit's done bus.
                     */
                ...
                {
                    if (debug) System.out.println("\town done and NOT Sync, generating AND");
                    /*
                     * Otherwise AND the component's done bus with the entry's control bus
                     * to get a valid done bus for the exit.
                     */
            ...
            {
                if (debug) System.out.println("\tno done, creating flop chaing");
                /*
                 * If the component does not produce a done, the Exit must
                 * have a fixed latency.  A chain of zero or more flops is
                 * created in parallel with the component to represent the
                 * flow of control.  This chain may end up getting removed if
                 * it is not used further on in the control flow.
                 */
                assert exitLatency.isFixed() : "Exit has no done, but Latency = " + exitLatency + " on " + component;
                for (int i = 0; i < exitLatency.getMinClocks(); i++)
                {
                    final Reg flop = getControlReg(exitControlBus, component.getOwner());
    ...
    /**
     * Tests whether a given ControlState is just a placeholder.  That is,
     * its {@link Bus} is not an active control bus; it is just there to
     * record the {@link Latency} at that point.  Currently, the only
     * placeholder state that is created is for {@link InBuf InBufs} that
     * do not have an active done {@link Bus}.
     *
