 ...
 * {@link Component Component}s.  One or more destination
 * {@link Port Port}s may be connected to the Bus; these represent
 * the receivers of the Bus's data.
 * <P>
 * Each Bus is owned by a single {@link Component Component}.  In
 * addition to Port connections, a Bus may have multiple logical
 * dependent Ports, repesented by {@link Dependency Dependencies}.
 * It is from these that the final Port connections are ultimately
 * derived.
 * <P>
 * Each Bus has an underlying vector of {@link Bit Bits} which comprise
 * it.  The number of bits in the vector is determined when the width
 * of the Bus is established via the method
 * {@link Bus#setBits(int)}). Each of these {@link Bit Bits} also
 * designates the Bus as its owner; they can also be referenced in any
 * {@link Value}, but their number and identity within the Bus can never
 * be changed once they are created.
 * <P>
 * The Bus also has a {@link Value}.  An initial {@link Value} is created
 * along with the {@link Bit} vector; this initial value simply contains
 * the bits of the vector.  Other value may be set on the Bus containing
 * Bits that are owne by other Buses.
 *
...
{
    /** Version info */
    private static final String _RCS_ = "$Rev: 202 $";

    /** The owner of this bus */
    private Exit owner;

    /**
     * The peer port, if any, that continues this bus across
     * a module boundary.
     */
    ...
    /*
     * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     * Begin new constant prop implementation.
     */
     ...
     *
     * @param size the number of bits that comprise this bus, > 0
     * @param isSigned true if this bus represents a signed value,
     *          false if unsigned
     ...
     *
     * @param position a value of type 'int'
     ...
     *
     * @return the value object that was created with {@link #setSize(int,boolean)}
     */
    ...
    /**
     * Updates this bus's {@link Value} from within its {@link Component}
     * during forward constant propagation.
     * <P>
     * Firstly, if this bus does not yet have a value, then
     * {@link #setSize(int,boolean)} is called using the size and
     * sign-edness of the input value.  Then the update occurs as
     * described below.
     * <P>
     * For each {@link Bit} of the incoming value,
     * this bus's value bit at the same position is updated
     * as follows:
     * <ol>
     * <li>If either bit is {@link Bit#DONT_CARE}, no update occurs;
     *     otherwise:
     * <li>If the incoming bit is a constant, it becomes the current
     *     bit; otherwise:
     * <li>If the incoming bit is {@link Bit#CARE}, the current bit
     *       becomes this bus's bit from the same position; otherwise:
     * <li>The incoming bit (another bus's bit) becomes the current bit.
     * </ol>
     *
     * @param value the incoming value to be pushed onto this bus
     *
    ...
    public boolean pushValueForward (Value v)
    {
        ...
        /*
         * Firstly, if this bus does not yet have a value, then
         * then initialize it with a "generic" care value that has
         * the same size and sign as the incoming value.
         */
        if (value == null)
        {
        ...
        assert (v.getSize() == getSize()) : ("size mismatch: " + v.getSize() + " vs. " + getSize());
        assert (v.isSigned() == value.isSigned()) : "sign mismatch";

        /*
         * Compare each incoming Bit with the current Bit at the
         * same index.
         */
        for (int i = 0; i < value.getSize(); i++)
        {
            ...
            if (!v.bitEquals(i, value, i))
            {
                ...
                {
                    /*
                     * Otherwise, if the incoming bit is a constant, it
                     * becomes the current bit...
                     */
                    value.setBit(i, v, i);
                    mod=true;
                }
                ...
                {
                    /*
                     * Otherwise, if the incoming bit is global
                     * CARE (no owner), the current bit is set to this
                     * bus's bit from the same position
                     * (if not already)...
                     */
     ...
     * {@link Port Ports} during reverse constant propagation by
     * propagating don't care bits.
     * <P>
     * If this bus is connected, then the target ports are those connected
     * to this bus.  Otherwise, the target ports are those which have a
     * dependency on this bus.
     * <P>
     * For each {@link Bit} of this bus's value, if all of the bits at
     * the same position in the target values are {@link Bit#DONT_CARE},
     * then this bus's bit is set to {@link Bit#DONT_CARE}.  Otherwise
     * no modification to this bus's value is made.
     *
     * @return true if this bus's value was modified, false otherwise
     */
    public boolean pushValueBackward ()
    {
        ...
        else
        {
            ...
            // for all logicals
            for (Iterator it = getLogicalDependents().iterator(); it.hasNext();)
            {
        ...
        for (int i = 0; i < getSize(); i++)
        {
            ...
            for (Iterator it = plist.iterator(); (it.hasNext()) && (!foundCare);)
            {
                Port p = (Port)it.next();
    ...
    /**
     * Forces the current value of this bus to a given value.
     * This method should only be used after all constant propagation
     * has completed in order to shave don't-care bits from the
     * upper end of the value.  It is intended to be called directly
     * before translation.
     *
     * @param value the value to be forced into this bus
     * @throws NullPointerException if the given value is null
     * @throws IllegalStateException if this bus does not currently
     *           have a value
     */
    public void forceValue (Value v)
    ...
    /**
     * Constructs a new Bus, but defers the creation of the vector
     * of {@link Bit Bits} that comprise this Bus.
     *
     * @param owner the exit to which this bus belongs
     */
     ...
     * Gets the peer Port.
     *
     * @return the peer port which continues this bus across a
     *         module boundary; null if there is none
     */
    public Port getPeer ()
    {
        return peer;
     ...
     *
     * @return a collection of Dependency objects, one for each
     ...
     *
     * @param isUsed true if used, false if ignorable
     */
    ...
    /**
     * Gets the cumulative latency at this bus.  This value is
     * relative to the start of the scheduling context (usually
     * the current module).
     */
    ...
    /**
     * Disconnects all Ports that are physically connected to
     * this bus.
     */
    public void disconnect ()
    {
        for (Iterator iter = new ArrayList(getPorts()).iterator(); iter.hasNext();)
        {
            ((Port)iter.next()).setBus(null);
        }
    }

    /**
     * Removes this bus as a dependency from all logical dependents,
     * and deletes the dependency which stored the relationship on the
     * target entry.
     */
    public void clearLogicalDependents ()
    {
        for (Iterator iter = new ArrayList(getLogicalDependents()).iterator(); iter.hasNext();)
        {
            Dependency logical = (Dependency)iter.next();
    ...
    /**
     * Removes this bus as a dependency from all dependents, both
     ...
     * Sets the peer Port.
     *
     * @param peer the peer port which continues this bus across a
     *         module boundary; null if there is none
     */
     ...
     * process.  This includes the isUsed flag, the tag, and the {@link ID}.
     * In addition, if the given Bus already has a vector of {@link Bit Bits},
     * them same number of {@link Bit Bits} will be created for this Bus.
     *
     * @param bus the bus from which to copy the attributes
     */
    ...
    /**
     * Returns true if the implementation of this bus is
     * floating point.
     */
