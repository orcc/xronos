...
public class HexBits {
	...
	/**
	 * A catch all utility method for converting a generic Number into a
	 * collection of binary bits needed to represent that number. For all
	 * integer type numbers (Byte, Short, Integer, Long) this is a simple call
	 * to 'longValue', for floating point representations we use the 'toRawBits'
	 * methods in Float and Double to get a hex representation of the value in
	...
	public static long getBits(Number number) {
		...
		if (number instanceof Float) {
			...
			retValue = ibits & 0x00000000FFFFFFFFL;
		} else if (number instanceof Double) {
			retValue = Double.doubleToRawLongBits(((Double) number)
					.doubleValue());
		} else {
	...
	public static void test(long x) {
		System.err.println("for input: " + x + "(0x" + Long.toHexString(x)
				+ ") size is: " + getSize(x));
	}
