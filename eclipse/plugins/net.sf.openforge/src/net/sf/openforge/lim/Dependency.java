...
/**
 * A Dependency describes the relationship between a dependent {@link Port Port}
 * and the {@link Bus Bus} on which it depends. There are multiple types of
 * dependency and the type of dependency affects how it is handled during
 * scheduling. DataDependencies indicate that the port will recieve a connection
 * to the logical bus that is depended on, though it may be delayed, latched,
 * etc. ControlDependencies will cause the scheduler to find the control signal
 * which qualifies the dependent bus and will use that controlling signal to
...
public abstract class Dependency implements Cloneable {
	 ...
	 * 
	 * @param logicalBus
	 *            the bus on which the port will logically depend; this bus must
	 *            be owned by an Executable
	 */
	...
	/**
	 * Gets the number of clock edges that must exist between the logical bus
	 * contained in this dependency and the enabling of the entry to which this
	 * dependency is attached.
	 * 
	 * @return the number of clocks that the port input should be delayed
	 *         following the fulfillment of this dependency
	 */
	...
	/**
	 * Removes this dependency from it's containing entry and sets the logical
	 * bus to null (which in turn causes the bus to lose any references back to
	 * this dependency).
	 * 
	 */
	public void zap() {
	...
	/**
	 * Tests the given Object for equality with this Dependency by checking
	 * <b>only</b> the port and logical bus.
	 * 
	 * @param obj
	...
	public boolean equals(Object obj) {
		if ((obj != null) && (obj instanceof Dependency)) {
			...
			return (dep.getPort().equals(getPort()) && dep.getLogicalBus() == getLogicalBus());
	...
	/**
	 * Generates a hashcode based <b>only</b> on the port, logical bus, and
