...
/**
 * Directive is the parent class for any Verilog directive. It is a fully
 * qualified statement starting with a backquote followed by a directive token.
 * 
 * <p>
...
public abstract class Directive implements Statement {
	...
	public static class Define extends Directive {
		 ...
		 * 
		 * @param macro
		 *            the name which will be replaced
		 * @param substitution
		 *            the text which will replace the macro name
		 */
	...
	public static class UnDefine extends Directive {
		 ...
		 * 
		 * @param macro
		 *            the name of the macro which is being undefined
		 */
	...
	public static class IfDef extends Directive {
		 ...
		 * 
		 * @param macro
		 *            the name of the macro to be tested
		 */
	...
	public static class EndIf extends Directive {
		/**
		 * Constructs a new EndIf directive, which indicates the end of an IfDef
		 * section.
		 */
		public EndIf() {
			super(Keyword.ENDIF);
	...
	public static class DefaultNetType extends Directive {
		...
		/**
		 * Constructs a new DefaultNetType directive specifying the default net
		 * type for implicit net declarations. The Keyword used to specify the
		 * net type must pass Keyword.isWireword().
		 * 
		 * @param nettype
		 *            the net type
		 */
		public DefaultNetType(Keyword nettype) {
			...
			assert (Keyword.isWireword(nettype)) : "defaultNettype directive must use a net type, not a "
					+ nettype.toString();
	...
	public static class TimeScale extends Directive {
		...
		/**
		 * Constructs a new TimeScale directive specifying the relative scale of
		 * time. Both the unit and precision paramters should be made up of
		 * values from 1, 10, and 100 and units from s, ms, us, ns, ps, and fs.
		 * 
		 * @param unitValue
		 *            the number of units
		 * @param unitUnit
		 *            the units of the unit value
		 * @param precisionValue
		 *            the precision of time
		 * @param precisionUnit
		 *            the units of the precision value
		 */
		public TimeScale(int unitValue, Keyword unitUnits, int precisionValue,
				Keyword precisionUnits) {
			this(new Constant(unitValue), unitUnits, new Constant(
					precisionValue), precisionUnits);
		}
	...
	public static class UnconnectedPullUp extends Directive {
		...
		/**
		 * Constructs a new UnconnectedDrive directive, specifying that any
		 * unconnected input ports should be pulled up.
		 */
