 ...
 */
public abstract class Assign implements Statement {
	...
	public Assign(Symbol assign, NetLValue net, Expression expression,
		...
		/**
		 * It is the expression's responsibility to correctly report the width
		 * of the result it produces. Assignments should be exactly matched, so
		 * any problems should be corrected by re-considering the width reported
		 * by the expression.
		 */
		if (checkBalance) {
	...
	public static class UnbalancedAssignmentException extends
		...
		public UnbalancedAssignmentException(NetLValue net,
				Expression expression) {
			super(
					new String(
							"continuous assignment has mismatched bit sizes: "
