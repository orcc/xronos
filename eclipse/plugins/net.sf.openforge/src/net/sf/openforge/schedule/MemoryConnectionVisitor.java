...
//             InputPin reset = null;
            ...
            for (Iterator memports = memory.getLogicalMemoryPorts().iterator(); memports.hasNext();)
            {
                ...
                /*
                 * query the memory port for clock and reset.  If different
                 * ports have different clock or reset then fatally err out.
                 */
//                 InputPin newClock = memport.getClockPin();
//                 InputPin newReset = memport.getResetPin();

//                 if ((clock != null && newClock != clock) || (reset != null && newReset != reset))
//                 {
...
//         referee.getResetPort().setBus(memport.getResetPin().getBus());
    ...
    private void exitModule (Module module)
    {
        ...
        for (Iterator iter = frame.getResources().iterator(); iter.hasNext();)
        {
            ...
            MemoryGateway gateway = new MemoryGateway(memoryPort,
                reads.size(), writes.size(), memoryPort.getMaxAddressWidth());
    ...
    private static abstract class MemAccess
    {
        ...
        public Resource getTarget () { return this.target; }
...
//                 addr.setSize(32, false); // THIS SHOULD BE BASED ON THE BACKING MEMORY!
            }
...
//                 data.getPeer().setSize(65, false); // THIS SHOULD BE BASED ON THE BACKING MEMORY!
            }
...
//                 data.setSize(65, false); // THIS SHOULD BE BASED ON THE BACKING MEMORY!
            }
            Bus size = (Bus)duplicate.get(gateway.getMemorySizeBus());
            if (size == null)
            {
