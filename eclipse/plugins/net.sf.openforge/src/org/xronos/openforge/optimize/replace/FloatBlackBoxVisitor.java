...
import org.xronos.openforge.lim.PinAccess;
...
/**
 * FloatBlackBoxVisitor is responsible for finding all floating point operations
 * (minus pin accesses) left in the LIM after operation replacement has
 * completed. Any which are found will be replaced by a call to a default
 * implementation for that operation which may be simply a black box to be
 * implemented by the user in HDL.
 * 
 * <p>
 * Created: Wed Apr 2 12:04:09 2003
 * 
 * @author imiller, last modified by $Author: imiller $
 * @version $Id: FloatBlackBoxVisitor.java 2 2005-06-09 20:00:48Z imiller $
 */
public class FloatBlackBoxVisitor extends ReplacementVisitor implements
		Optimization {
	...
	/**
	 * Finds all floating point operations (ignoring PinAccess) and throws a
	 * fatal error. Eventually we will do a replacement here with a black box.
	 * 
	 * @param op
	...
	public void filter(Operation op) {
		...
		if (op instanceof Constant || op instanceof Access)
			return;

		if (op.isFloat() && !(op instanceof PinAccess)) {
			ReplacementCorrelation correlation = ReplacementCorrelation
					.getCorrelation(op);
			String type = (correlation != null) ? correlation
					.getReplacedOperationDescription() : "unknown type: " + op;
			EngineThread
					.getEngine()
					.fatalError(
							"Non replaced floating point operation found.  "
									+ "All floating point operations must be replaced: "
									+ type);
			// Job.error("Non replaced floating point operation found.  All floating point operations must be replaced: "
			// + op);
			...
			// else
			// {
			// Job.fatalError("Illegal state.  Non replaceable floating point operation "
			// + op);
			// }
		}
	}
