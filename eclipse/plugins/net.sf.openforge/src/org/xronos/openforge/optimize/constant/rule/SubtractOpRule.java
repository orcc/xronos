...
public class SubtractOpRule {
	...
	public static boolean halfConstant(SubtractOp op, Number[] consts) {
		...
		if (p2 != null && p2.longValue() == 0) {
			...
			portCorrelation.put(op.getDataPorts().get(1), mop.getDataPorts()
					.get(0));
			...
			/**
			 * set the unary Minus port and bus size with this SubtractOp port0
			 * size info
			 **/
		...
		// op + " due to (a - 0x80000000)");
		// // a - 0x80000000 = a ^ 0x80000000. Simply replace the component with
		// a bitwise xor
		// XorOp xor = new XorOp();
		// Module owner = op.getOwner();
		// assert owner != null :
		...
		// xor.getDataPorts().get(0));
		// portCorrelation.put(op.getDataPorts().get(1),
		// xor.getDataPorts().get(1));
		...
		// assert xor.getExits().size() == 1 :
		// "Only expecting one exit on MinusOp";
		// busCorrelation.put(exit.getDoneBus(),
		// ((Exit)xor.getExits().iterator().next()).getDoneBus());
		...
		// /** set the data ports and bus size with this SubtractOp data ports
		// and bus size info **/
