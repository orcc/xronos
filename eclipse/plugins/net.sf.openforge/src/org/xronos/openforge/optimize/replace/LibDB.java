...
/**
 * LibDB is a class which builds and stores the Design created for each Library
 * that has been searched. When an instance of this class is queried for a given
 * library, the map of libraries is searched to return the previously built
 * implementation if it exists. If it does not, then it is build from its
 * source. This class also ensures that the implementation is only ever merged
 * with another design one time.
 * 
 * <p>
 * Created: Wed Apr 9 12:29:18 2003
 * 
...
public class LibDB {
	...
	/**
	 * A map of String->Status for each library that has been built.
	 */
	private Map<String, Status> nameToStatus = new HashMap<String, Status>();

	/**
	 * The Status instance that we use for any library which could not be found.
	 */
	...
	/**
	 * Retrieves the {@link Design} which is the implementation of the named
	 * (fully qualified) library source.
	 * 
	 * @param libName
	 *            a fully qualified String name of the library.
	 * @return a {@link Design}
	 */
	public Design getDesign(String libName) {
		Status status = nameToStatus.get(libName);
		if (status == null) {
			...
			nameToStatus.put(libName, status);
		}
		return status.getDesign();
	}

	/**
	 * Merges the named library Design into the specified design, or does
	 * nothing if the library was already merged.
	 * 
	 * @param libName
	...
	public void mergeDesigns(String libName, Design top) {
		assert nameToStatus.containsKey(libName) : "Merge error: Library "
				+ libName + " not previously fetched from database.";
		assert nameToStatus.get(libName) != NULL_STATUS : "Merge error: Library "
				+ libName + " could not be instantiated.";
		Status status = nameToStatus.get(libName);
		if (!status.isMerged()) {
	...
	/**
	 * Builds a new and independent Design which represents the given library
	 * (source file).
	 * 
	 * @param library
	 *            a value of type 'String'
	...
	private Design buildDesignFor(String library) {
		String[] sourcePath = EngineThread.getGenericJob()
		...
		if (url != null) {
			EngineThread.getGenericJob().info(
					"Building library from source " + url);
			...
			for (Option opt : oldGenJob.optionsMap.values()) {
				option = opt;
				newOption = newGenJob.getOption(option.getOptionKey());
				newOption.replaceValue(CodeLabel.UNSCOPED,
						option.getValue(CodeLabel.UNSCOPED));
			}
			...
			option = newGenJob
					.getOption(OptionRegistry.OPERATOR_REPLACEMENT_MAX_LEVEL);
			...
			try {
				...
				oldGenJob
						.error("Library files must have the expected source language extension (ie: .c)");
				oldGenJob
						.error("Library file "
								+ url.getFile()
								+ " does not meet this requirement and can not be used.");
				return null;
			}
			...
			Engine libEngine = null; // says this thread no belongs to this new
										// job
			/*
			 * if (url.getFile().endsWith(".java")) { libJob = new JJob(proj); }
			 * else
			 */
			...
			// if you want to turn off logging in this build now, TEMPRORARILY
			// reset the log level to off
			...
			EngineThread.getGenericJob().warn(
					"Library source file " + library + " not found");
		}
		return design;
	}

	/**
	 * A convenience class for tracking the library design and whether it has
	 * been merged yet.
	 */
	private static class Status {
		...
		public void mergeWith(Design design) {
			...
			if (!isMerged()) {
				design.mergeResources(libDesign);
				addedToDesign = true;
			}
		}

		public boolean isMerged() {
			return addedToDesign;
		}
	}
