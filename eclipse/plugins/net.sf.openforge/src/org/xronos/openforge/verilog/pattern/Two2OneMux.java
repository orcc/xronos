...
import org.xronos.openforge.verilog.model.Conditional;
...
/**
 * An old favorite which uses a conditional expression to pick an input to
 * assign to an output Net.
 * <P>
 * Takes the form:
 * <P>
 * 
...
public class Two2OneMux implements Statement {
	...
	/**
	 * Constructs a Two2OneMux based on either of two selection Nets, which
	 * specify which input data Net will be assigned to the result Net.
	 * 
	 * @param result
	 ...
	 */
	public Two2OneMux(Net result, Expression sel, Expression dataA,
			Expression dataB) {
		/*
		...
		if (_pattern.db)
			_pattern.d.ln("Select: " + sel);
		two2one_statement = new Assign.Continuous(result, conditional);
	}

	/**
	 * Builds a VMux ignoring the selB input since the 'b' input will be
	 * selected when the selA is not active (true).
	 */
	private Two2OneMux(Net result, Expression selA, Expression selB,
			Expression dataA, Expression dataB) {
