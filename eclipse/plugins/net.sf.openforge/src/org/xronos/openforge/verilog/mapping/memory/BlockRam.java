...
public abstract class BlockRam extends Ram {
	...
	public StatementBlock initialize() {
		...
		initial_block
				.add(new InlineComment(
						" Initialization of Block ram now done through explicit parameter setting.",
						Comment.SHORT));
	...
	public ModuleInstance instantiate() {
		...
		moduleInstance
				.connect(new Wire("ADDR", getLibAddressWidth()), addr_exp);
		...
		if (nonpardatwidth > dataAttachWidth && dataOutName.length() > 0) {
			...
			do_exp = padded_do;
		} else if (nonpardatwidth <= dataAttachWidth
				&& dataOutName.length() > 0) {
			if (!datScalar) {
		...
		if (getParityWidth() != 0) {
			...
			if (getParityWidth() > pardatwidth && zerobits == getParityWidth()) {
				dop_exp = new Wire("parOut", getParityWidth());
					...
					&& dataOutName.length() > 0) {
				// padd with zeros
				Expression dox_exp = new Wire(dataOutExtraName, zerobits)
						.getFullRange();
				...
				if (!datScalar) {
					int msb = pardatwidth + dataOutStartBit + nonpardatwidth
							- 1;
