 ...
 * configurations which are possible in a Xilinx FPGA.  Specific
 * accessor methods are available to retrieve the data, enable, set,
 * reset, clear, and preset ports.  It's exit specifies that the
 * Latency of this component is Latency.ONE.
 *
 * A Reg object always has ports for sync Set, sync Reset, Enable
 * async Preset, async Clear.  Depending on the desired configuration
 * (immutably specified during construction) certain ports will be
 * marked used and the others will be marked unused. 
 *
 * <pre>
...
public class Reg extends Primitive implements Emulatable
{
    ...
    /* If set to false then the isGroupable method will always return
     * false. */
    ...
    /**
     * Constructs a new Reg of the specified type, where the type
     * defines what ports are marked as used.
     *
     * @param type an int, one of the specified types in {@link Reg} 
     * @param id a String to use for the id of this Reg, or null if
     * none specified.
     */
    public Reg (int type, String id)
    {
        ...
        if (((type & RESET) != 0) && ((type & CLEAR) != 0))
            throw new IllegalArgumentException("Cannot have a reg that is both RESET and CLEAR");
        if (((type & SET) != 0) && ((type & PRESET) != 0))
            throw new IllegalArgumentException("Cannot have a reg that is both SET and PRESET");
        
        // set the ports unused/used state based on the type
        for (int i=1; i<4; i++)
        {
            boolean state = false;
            if ((i == 1) && ((type & (PRESET|SET)) != 0)) state = true;
            else if ((i == 2) && ((type & (RESET|CLEAR)) != 0)) state = true;
            else if ((i == 3) && ((type & ENABLE) != 0)) state = true;
    ...
    /**
     * <code>updateResetType</code> is called by the translation
     * engine to cause this register to update its type of reset/set
     * based on its context and the configuration settings.  The
     * type of reset (async vs sync) is determined by the options
     * {@link OptionRegistry#SYNC_RESET}.  
     */
    public void updateResetType ()
    {
        ...
        final boolean isSync = ((OptionBoolean)this.getGenericJob().getOption(OptionRegistry.SYNC_RESET)).getValueAsBoolean(this.getSearchLabel());
    ...
    /**
     * Sets the initial value for this register, which is the value
     * that this register will contain after programming, and any time
     * the RESET port is asserted.  Default value for power up and/or
     * reset behavior is 0.
     *
    ...
    {
        if (value == null)
            throw new IllegalArgumentException("Illegal null initial value for reg");
     ...
     * {@link Port}.  Returns true if this Reg has a 'reset' value or
     * was constructed to consume reset. 
     */
    ...
    /**
     * Allows this register to be define whether it is done
     * synchronous, which it is if it is part of the control chain of
    ...
    /**
     * Overrides isDoneSynchronous in Component to return true if this
     * Reg is part of the control chain, in which case it is producing
     * a Done which is synchronous.
     */
    ...
    /**
     * Returns the value which is stored in this register after
     * programming of the device and subsequent to reset, ie the
     * initial value.  The SET port will return the register to the
     * bitwise inverted value.
     *
     ...
     * @return a constant describing the type of register (see
    ...
    /**
     * Sets whether or not this Reg can be considered for grouping
     * into an SRL16, but cannot force the reg to be grouped.
     *
    ...
    /**
     * Rules of testing whether this Reg can be qualified for grouping
     * into a SRL16.
     */
     ...
     *
     * @param portValues a map of {@link Port} to {@link net.sf.openforge.util.SizedInteger}
     *          input value
     * @return a map of {@link Bus} to {@link net.sf.openforge.util.SizedInteger} result value
     */
    ...
    /**
     * Bit states propagate through after being unioned with the reset
     * value (if any), except that pass through bits become care bits.
     */
    public boolean pushValuesForward ()
    {
        ...
        {
            int newSize = Math.max(this.initialValue.getSize(), drivenValue.getSize());
            newValue = new Value(newSize, drivenValue.isSigned() && this.initialValue.isSigned());
            Bit drivenExtendBit = drivenValue.isSigned() ? drivenValue.getBit(drivenValue.getSize()-1) : Bit.ZERO;
            Bit initExtendBit = this.initialValue.isSigned() ? this.initialValue.getBit(this.initialValue.getSize()-1) : Bit.ZERO;
            
            for (int i=0; i < newValue.getSize(); i++)
            {
                Bit b1 = ((i >= drivenValue.getSize()) ? drivenExtendBit : drivenValue.getBit(i));
                Bit b2 = (i >= this.initialValue.getSize() ? initExtendBit : this.initialValue.getBit(i));
                if (b1 == b2)
                { // otherwise, leave it as a care bit.
    ...
    /**
     * Any bit of the consumed output that is dont care
     * becomes a dont care on the inputs, all other
     * bits are care.
     */
    ...
    private void validate ()
    {
        ...
        // 'used' of the pins.
        if (!getDataPort().isUsed()) throw new IllegalRegisterConfiguration("Data Port must be used for Reg");
        if (getSetPort().isUsed() != ((regState & (PRESET|SET)) != 0)) throw new IllegalRegisterConfiguration("Inconsistent internal state for Set Port of Reg " + this.hashCode());
        if (getInternalResetPort().isUsed() != ((regState & (RESET|CLEAR)) != 0)) throw new IllegalRegisterConfiguration("Inconsistent internal state for Reset/Clear Port of Reg " + this.hashCode());
        if (getEnablePort().isUsed() != ((regState & ENABLE) != 0)) throw new IllegalRegisterConfiguration("Inconsistent internal state for Enable Port of Reg " + this.hashCode());
//         if (getPresetPort().isUsed() != ((regState & (PRESET|SET)) != 0)) throw new IllegalRegisterConfiguration("Inconsistent internal state for Set/Preset port of Reg " + this.hashCode());
//         if (getClearPort().isUsed() != ((regState & (RESET|CLEAR)) != 0)) throw new IllegalRegisterConfiguration("Inconsistent internal state for Clear Port of Reg " + this.hashCode());
        ...
        {
            throw new IllegalRegisterConfiguration("Inconsistent internal state Reg " + this.hashCode() + " it cannot be both synchronous and asynchronous");
        }
    }
