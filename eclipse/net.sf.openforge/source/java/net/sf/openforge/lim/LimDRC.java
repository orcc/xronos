...
public class LimDRC extends FilteredVisitor
{
    ...
    public void dumpFailures ()
    {
        if (!failures.isEmpty())
        {
            ...
            EngineThread.getGenericJob().warn("internal integrity check failure:");
    ...
    public void visit (Call call)
    {
        ...
        for (Iterator iter = call.getPorts().iterator(); iter.hasNext();)
        {
            ...
            //assert (port != null && procPort != null) : "DISAGREEMENT between call interface and procedure interface";
        }
        ...
        for (Iterator iter = call.getBuses().iterator(); iter.hasNext();)
        {
            ...
            // XXX FIXME.  Why is there disagreement????  I commented
            // this out for now, but it should probably be fixed anyway!!!
            //assert (bus != null && procBus != null) : "DISAGREEMENT between call interface and procedure interface";
        }
    }

    /**
     * Recursively calls filterAny on all components of the given
     * Module.  Meant to be used for Modules contained in the design
     * which are not defined in the Visitor interface.
     */
     ...
     * <ul>
     * <li>
     * All buses have a {@link Value}
     * </li>
     * </ul>
     * </p>
     *
     * @param c the {@link Component} to check.
     */
        ...
        for (Iterator it = buses.iterator(); it.hasNext();)
        {
            ...
            if (b.isUsed())
            {
                ...
                if (b.getValue() == null)
                {
                    failures.put(
                        b,
                        "Bus " + ID.showLogical(b) + " belonging to "
                        + ID.showLogical(c) + " Lacks Value");
                    if (_lim.db)
                    {
                        _lim.ln(
                            _lim.DRC, "Bus has no value: " + b + " " + c.show());
                    }
                }
            }
        }
        ...
        for (Iterator it = ports.iterator(); it.hasNext();)
        {
            ...
            if (port.isUsed())
            {
                ...
                if (!portHasValue && !isWeirdRegPort(port))
                {
                    failures.put(
                        port,
                        "Port " + ID.showLogical(port) + " belonging to "
                        + ID.showLogical(component) + " Lacks Value");
                    if (_lim.db)
                    {
                        _lim.ln(
                            _lim.DRC,
                            "Port has no value: " + port + " is used: "
                            + port.isUsed() + " " + component.show());
                    }
                }
                ...
                if (
                    (port.getBus() == null)
                    && !procedureBodies.contains(port.getOwner()))  // Procedure bodies have no connections on their ports
                {
                    if (!isWeirdRegPort(port))
                    {
                        failures.put(
                            port,
                            "Port " + ID.showLogical(port) + " belonging to "
                            + ID.showLogical(component) + " Has No Bus");
                        if (_lim.db)
                        {
                            _lim.ln(
                                _lim.DRC,
                                "Port has no bus: " + port + " "
                                + component.show());
                        }
                    }
                }

                /* The rule is actually that all 'care' bits in the port's
                 * value must come froma Bus and the position in that bus
                 * must be less than that bus' Value size
                 */
                /*
                 * Check that all bits sourcing a port (non constant) come
                 * from a bus and that the Value on that Bus has
                 * sufficient bits to cover the sourced bit.  InBufs and
                 * procedure blocks are special since they never get their
                 * ports hooked up.
                 */
                if (portHasValue)
                {
                    if (
                        !(component instanceof InBuf)
                        && !procedureBodies.contains(port.getOwner()))
                    {
                        for (int i = 0; i < port.getValue().getSize(); i++)
                        {
                            ...
                            // bit. All should come from real buses by this point.
                            if (bit == Bit.CARE)
                            {
                                failures.put(
                                    port,
                            ...
                            // basic bit test added CRSS, fixed for new Value: SGE
                            if (!bit.isGlobal())
                            {
                                int position = bit.getPosition();
                                if (
                                    position >= bit.getOwner().getValue().getSize())
                                {
                                    failures.put(
                                        port,
                                        "Bit " + i + " of Port "
                                    ...
                                    if (_lim.db)
                                    {
                                        _lim.ln(
                                            ...
                                            + position
                                            + " is greater than bus "
                                            + bit.getOwner() + " value size "
                                            + bit.getOwner().getValue().debug());
                                    }
                                }
                            }
                        }
