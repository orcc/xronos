...
/**
 * Decision is a refinement of {@link Module} for
 * components that produce a truth value.  It defines two {@link Exit Exits},
 * one whose done {@link Bus} signals <i>true</i> and one whose
 * done {@link Bus} signals <i>false</i>.
 * On a true condition, the true bus is asserted and the false
 * bus is deasserted; on a false condition, the true bus is
 * deasserted and the false bus is asserted.
 * <P>
 * The Decision is constructed with a {@link Component} representing the
 * boolean value to be tested.  This argument must produce a single data
 * value.
 * <P>
 * <b>NOTE: it is the responsibility of the caller (i.e., the scheduler) to AND the
 *    the {@link Bus} representing the boolean values the ready {@link Bus}
 *    before they exit the Decision if necessary; this will be necessary if
 *    the test component requires a control signal.</b>
 *
...
public class Decision extends Module 
{
    ...
    /**
     * The block containing the logic that is executed to produce the
     * boolean value
     */
    ...
    /**
     * Constructs a new Decision.  It is the responsibility of the caller
     * to add an appropriate {@link DataDependency} to the input of the
     * {@link Not} component; in this case, the dependency should be made
     * on the data output of the given {@link Component}.
     *
     * @param testBlock a component (possibly a module) which is executed
     *          within the Decision to obtain a boolean value
     * @param testComponent a {@link Component} within the testBlock which is the
     *          component which provides the boolean true/false for this decision.
     */
    ...
    /**
     * The low level {@link Component} of the test {@link Block}
     * that drives the boolean test value with its output {@link Bus}.
     */
    ...
    /**
     * Gets the data bus coming out of the testBlock that is used to
     * create the true and false buses.
     *
    ...
    public Bus getDecisionDataBus()
    {
        ...
        Bus decisionData = ((Dependency)notDeps.iterator().next()).getLogicalBus();
        return decisionData;
    }
    

    /**
     * Returns the 'done' bus from this decision which asserts when
     * the test block has completed (regardless of the true/false
     * value of the decision) and creates the 'no decision' exit on
    ...
    /**
     * Sets the internal dependencies for all clock, reset, and go {@link Port Ports}
     * of the children of this Decision.  Also creates the {@link Exit Exits} for this
     * Decision based upon the {@link Exit Exits} of its components.
     */
    private void setControlDependencies ()
    {
        /*
         * Set the dependencies so that everything is initially assumed to
        ...
        /*
         * Connect the component that produces the boolean value and make
         * its done the new go.
         */
        ...
        addDependencies(trueOutBuf.makeEntry(doneExit), clockBus, resetBus, trueAnd.getResultBus());
        ...
        addDependencies(falseOutBuf.makeEntry(doneExit), clockBus, resetBus, falseAnd.getResultBus());
    ...
    private void setDataDependencies ()
    {
        ...
        /*
         * Connect the true And; it uses the go bus and boolean data
         * bus as input data.
         */
        final And trueAnd = getTrueAnd();
        entry = trueAnd.getMainEntry();
        entry.addDependency((Port)trueAnd.getDataPorts().get(0), new DataDependency(testDoneBus));
        entry.addDependency((Port)trueAnd.getDataPorts().get(1), new DataDependency(testBlockBus));
        ...
        /*
         * Connect the false And, it uses the go bus and Not data bus
         * as input data.
         */
        final And falseAnd = getFalseAnd();
        entry = falseAnd.getMainEntry();
        entry.addDependency((Port)falseAnd.getDataPorts().get(0), new DataDependency(notDoneBus));
        entry.addDependency((Port)falseAnd.getDataPorts().get(1), new DataDependency(notBus));
    }
    ...
    protected void mergeExits (Map exitMap, Bus clockBus, Bus resetBus)
    {
        ...
        final Collection doneExits = (Collection)exitMap.remove(Exit.getTag(Exit.DONE));
        super.mergeExits(exitMap, clockBus, resetBus);
    }

    /**
     * Overrides method in {@link Module Module} to ensure that the
     * arguments to the decision are scheduled before the
     * {@link Not Not}.
     *
    ...
    {
        /*
         * XXX: Traversal should be done by the scheduler in the order that
         * it wants -- get rid of this.
         */
    ...
    {
        /*
         * Use a CloneListener to obtain the clone of testComponent, which
         * is hidden inside the testBlock.
         */
    ...
    protected void cloneNotify (Module moduleClone, Map cloneMap)
    {
        ...
        /*
         * Update the clone fields, except for testComponent which is handled
         * in clone().
         */
