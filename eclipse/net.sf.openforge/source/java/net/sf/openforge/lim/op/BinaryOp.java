...
/**
 * Base class of all operations, which require two operands and
 * generate one result.
 *
...
public abstract class BinaryOp extends Operation
{
     ...
     * By default, the Done of a BinaryOp is unused.
     */
    public BinaryOp ()
    {
        super(2);
    ...
    /**
     * Tests whether this is a signed operation. An operation is signed
     * if both the left and right port {@link Value values} are signed,
     * otherwise the operation is considered unsigned.
     * 
     * @return boolean
     */
    ...
    {
        return (getLeftDataPort().getValue().isConstant()
            || getRightDataPort().getValue().isConstant());
    }

    /**
     * Tests whether this is a passthrough operation based on a bitwise comparison
     * of the operands.  This method is used in subclasses for bitwise operations.
     * For each pair of {@link Bit Bits} from the same positions in each operand,
     * the method {@link #isSignificant()} is called until it returns true or the
     * number of bits has been exhausted.
     *
     * @return true if the output of this component can be reduced to a constant
     *         or the value of a single input, based upon the current {@link Value}
     *         of each data {@link Port}
    ...
    protected boolean isBitwisePassthrough ()
    {
        ...
        {
            Bit l = leftValue.getBit(i);
            Bit r = rightValue.getBit(i);
    ...
    /**
     * Tests whether the states of two bits from the same position of
     * each operand will cause work to be done in a bitwise binary operation.
     * Called as a helper method by {@link #isBitwisePassthrough()}.
     *
     * @param leftBit the left bit from a given position
     * @param rightBit the right bit from a given position
     * @return true if logic will be generated based upon these bits,
     *         false if the output is constant or a wire
     */
