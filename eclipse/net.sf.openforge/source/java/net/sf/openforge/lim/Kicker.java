...
/**
 * The Kicker uses the Reset port as input, and the done bus as output.
 * the equation is:
 *     reg1=(!RESET)
 *     reg2=(reg1)&(!RESET)
...
public class Kicker extends Module implements Composable
{
     ...
     *
     * @param size the initial size in bits of the Latch.
     */
    public Kicker ()
    {
        ...
        regFinal.getClockPort().setBus(getClockPort().getPeer()); // connect clock
    ...
    /**
     * <code>getRepeatPort</code> will return a non-null port if this
     * kicker has an input port for causing the kicker to repeat, or
     * null if no such port exists.
     *
    ...
    /**
     * Indicates that this module will be instantiated as its own
     * module in HDL, ie signal names cannot propagate across its
     * boundry. 
     *
    ...
    /**
     * A KickerPerpetual is a Module which takes in Clock and
     * generates a single cycle pulse on the done output shortly after
     * device power-up (de-assertion of GSR).  Additionally, there is
     * a 'repeat' port which is delayed by one cycle and then
     * logically ORed with this post-powerup signal.  When the repeat
     * port is connected to the DONE of an entry method and the output
     * of the kicker to the GO, this will cause the circuit (entry
     * method) to run forever as it will be continuously restarted
    ...
    public static class KickerPerpetual extends Kicker
    {
        ...
        public KickerPerpetual (boolean fbFlopRequired)
        {
            ...
            if (fbFlopRequired)
            {
                ...
                Reg delay = Reg.getConfigurableReg(Reg.REGR, ID.showLogical(this) + "_fb_delay");
                this.addComponent(delay);
                
                delay.getDataPort().setBus(repeatBus);
                delay.getClockPort().setBus(getClockPort().getPeer()); // connect clock
                delay.getResetPort().setBus(getResetPort().getPeer()); // connect reset
                delay.getInternalResetPort().setBus(getResetPort().getPeer()); // connect reset
        ...
        /**
         * Returns the port which when set high will cause the
         * output (done bus) of this KickerPerpetual to go high, ie it
         * causes the kicker to fire again.
         *
    ...
    /**
     * A KickerContinuous is a Module which takes in Clock and
     * generates a single cycle pulse on the done output shortly after
     * device power-up (de-assertion of GSR) and once asserted,
     * maintains that done signal in the true state until the device
    ...
    public static class KickerContinuous extends Kicker
    {
        ...
        /**
         * Returns the port which when set high will cause the
         * output (done bus) of this KickerContinuous to go high, ie it
         * causes the kicker to fire again.
         *
         * @return a non null Port
         */
