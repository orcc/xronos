...
/**
 * PriorityMux is a {@link Module} composed of a tree of 2-1
 * {@link EncodedMux EncodedMuxes}, which protects against any
 * problems resulting from multiple selects being asserted. There is
 * only one data bus, representing the selected data, and a Done bus
 * which reflects the assertion of any select signal.
 *
...
public class PriorityMux extends Module implements Cloneable
{
    ...
    /**
     * Creates and builds the internals of a priority based Mux, in
     * which the 0th data port is the <b>LEAST</b> priority and the
     * nth data port is the <b>HIGHEST</b> priority input.
     */
    public PriorityMux(int inputs)
    {
        ...
        for (int i=0; i < inputs; i++)
        {
            ...
            SelectDataPair sdp = new SelectDataPair(select_port.getPeer(), data_port.getPeer());
            queue.add(sdp);
        }
        ...
        while (queue.size() > 1)
        {
            ...
            while (queue.size() > 1)
            {
                ...
                subList.add(0, new SelectDataPair(or.getResultBus(), emux.getResultBus()));
            }
    ...
    /**
     * Retrieves the List of select {@link Port Ports} for this
     * encoded mux in increasing priority order.
     *
     * @return a 'List' of {@link Port Ports}
     */
    public List getSelectPorts()
    {
        return Collections.unmodifiableList(new ArrayList(selectToData.keySet()));
    }
    
    /**
     * Retrieves the data port that corresponds to a given select
     * port.
     */
    ...
    protected void cloneNotify (Module moduleClone, Map cloneMap)
    {
        ...
        for (Iterator iter = selectToData.entrySet().iterator(); iter.hasNext();)
        {
            ...
            final Port selectClone = getPortClone((Port)entry.getKey(), cloneMap);
            final Port dataClone = getPortClone((Port)entry.getValue(), cloneMap);
