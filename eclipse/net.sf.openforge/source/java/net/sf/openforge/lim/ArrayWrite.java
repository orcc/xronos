...
/**
 * ArrayWrite is an {@link OffsetMemoryWrite} which gets its offset
...
public class ArrayWrite extends OffsetMemoryWrite implements ArrayAccess
{
    ...
    /**
     * The number of housekeeping locations between the base address
     ...
     *
     * @param addressableLocations, the number of addressable
     * locations written. 
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     * @param skip the number of housekeeping locations between the
     * array base address and the first data element
     * @param policy, the address stride policy for this access
     */
    public ArrayWrite (int addressableLocations, int maxAddressWidth, boolean isSigned, AddressStridePolicy policy)
    {
     ...
     * Constructs an ArrayWrite with the specified memory write access
     *
     * @param addressableLocations, the number of locations written.
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     * @param skip the number of housekeeping locations between the
     * array base address and the first data element
     * @param policy, the address stride policy for this access
     */
    public ArrayWrite (int addressableLocations, int maxAddressWidth, boolean isSigned, int skip, AddressStridePolicy policy)
    {
        this(new MemoryWrite(false, (addressableLocations * policy.getStride()), isSigned),
     ...
     *          to the memory, contained in this ArrayWrite module.
     * @param addressableLocations, the number of addressable
     * locations written. 
     * @param maxAddressWidth the pre-optimized number of bits in the address bus
     * @param skip the number of housekeeping locations between the
     * array base address and the first data element
     */
    private ArrayWrite (MemoryWrite memoryWrite, int addressableLocations, int maxAddressWidth, int skip)
    {
    ...
    /**
     * Clones this object.  A new ArrayWrite is created using a copy
     * of the {@link MemoryWrite}, including its
    ...
    protected void cloneNotify (Module moduleClone, Map cloneMap)
    {
        ...
        ((ArrayWrite)moduleClone).skipConstant = (Constant)cloneMap.get(this.skipConstant);
        ((ArrayWrite)moduleClone).offsetPort = getPortClone(this.offsetPort, cloneMap);
    }
