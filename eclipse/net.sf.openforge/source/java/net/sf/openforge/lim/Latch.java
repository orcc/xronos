...
/**
 * Latch is so not a subclass of reg. Rather, latch is the composition of 
 * of an enabled reg with reset, and a 2:1 mux. Out out damn reg relation!
 *
...
public class Latch extends Module implements Composable, Emulatable
{
    ...
    /**
     * A latch does not require a connection to its GO, but it does
     * require a control signal to enable it.
     *
    ...
    public Latch ()
    {
        ...
        addComponent(this.reg = Reg.getConfigurableReg(Reg.REGE, makeName("_reg")));
        ...
        // finally, connect the resultbus from the mux to resultbus for the module
        resultBus.getPeer().setBus(mux.getResultBus());
        Entry outEntry = resultBus.getPeer().getOwner().makeEntry(mux.getResultBus().getOwner());
    ...
    /**
     * Get the data {@link Port Port} which is the 1'th port
     * in the list
     *
     * @return a value of type 'Port'
     */
    ...
    /**
     * Actually Latches shouldn't preclude balanced scheduling since
     * they are inserted during scheduling in response to a
     * non-balanced condition.  However, the only place they exist
     * pre-scheduling is in Loops and we account for this in the
     * {@link Loop#isBalanceable}
     *
     * @return false
     */
