...
/**
 * FifoIF is a class representing an atomic FIFO interface to a Forged
 * design.  The class ties together all the pins necessary to interact
 * with fifo interface as well as maintaining important
 * characteristics of the interface such as data path width.
 *
 *
 * <p>Created: Mon Dec 15 12:40:24 2003
 *
...
public abstract class FifoIF implements Referenceable, StateHolder
{
    ...
    /** The set of defined SimplePin objects for this interface.  We
     * use a linked hashset just as a convenience to the user so that
     * the interface will always be generated in the same order. */
    private Set pins = new LinkedHashSet();
    
    /**
     * Constructs a new FifoIF (fifo interface) with the specified
     * byte width for the data path.
     *
     * @param width a positive int, the byte width of the data path.
     * @throws IllegalArgumentException if width is not greater than
     * zero 
     */
    protected FifoIF (int width)
    {
        if (width <= 0)
        {
            throw new IllegalArgumentException("Illegal byte width for fifo IF " + width);
        }
    ...
    /**
     * Returns a value which is one of the types represented by the
     * type fields of this class.
     */
    public abstract int getType ();

    /**
     * Returns a positive integer value indicating the BIT width of
     * the data path for this FIFO interface in bytes.
     *
    ...
    /**
     * Returns a Collection of {@link SimplePin} objects representing
     * the complete set of pins necessary to implement this fifo
     * inteface.
     *
    ...
    /**
     * Returns a {@link FifoAccess} object that is used to either
     * obtain data from this FifoIF (if this is an input FIFO
     * interface) or to write data to this FifiIF (if this is an
     * output FIFO interface)
     */
    public abstract FifoAccess getAccess ();

    /**
     * Returns true if the data port of this fifo interface is an
     * input to the generated core.
     */
    public abstract boolean isInput ();

    /**
     * Returns a String, based upon the supplied port name, that is to
     * be used as the base name for all pins comprising this fifo
     * interface. 
     */
