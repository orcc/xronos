...
/**
 * AggregateConstant is a {@link Constant} that represents a concatentation
 * of other Constants, typically derived from a {@link Record}.
 *
...
public class AggregateConstant extends MemoryConstant
{
    ...
    /**
     * Creates a new AggregateConstant from a List of Constant
     * objects. 
     * 
     * @param constants the list of Constants
     * @param width the total bitwidth of the constants
     */
    ...
    /**
     * Gets the list of Constants which comprise this
     * AggregateConstant.
     * 
    ...
    /**
     * Returns the ordered and flattened list of constants that make
     * up the value generating constants for this aggregate.
     *
     * @return a List of Constants
     */
    @Override
     ...
     * the contents of every constant being aggregated.
     *
    ...
    /**
     * Returns a bundle of AURep objects that define the numerical
     * value (and locked state) of each unit in this constant as those
     * units were laid out in memory.  Thus this representation
     * follows the endianness of the compilation.
     *
    ...
    public AURepBundle getRepBundle (boolean swapEndian)
    {
        ...
        for (Constant memConst : getConstituents())
        {
            ...
            if (size < 0) size = repBundle.getBitsPerUnit();
            if (repBundle.getBitsPerUnit() != size)
                throw new UnsupportedOperationException("Unexpected aggregated constant structure.  Mismatched addressable sizes.  Expected " + size + " found " + repBundle.getBitsPerUnit()); 
