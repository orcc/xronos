...
/**
 * A Branch represents a choice of execution between two
 * {@link Component Components}, one representing a path
 * for a true condition, the other a path for a false
 * condition.  The choice is based upon
 * the output value of a {@link Decision}.
 *
 * @version $Id: Branch.java 78 2005-12-14 21:24:03Z imiller $
 */
public class Branch extends Module
{
    private static final String rcs_id = "RCS_REVISION: $Rev: 78 $";
     ...
     *
     * @param decision the decision which chooses the execution path
     * @param trueBranch the true path component
     * @param falseBranch the false path component, null if there is none
     */
    public Branch (Decision decision, Module trueBranch, Module falseBranch)
    {
        ...
        this.falseBranch = falseBranch == null ? new Block(Collections.EMPTY_LIST) : falseBranch;
    ...
    /**
     * Gets the {@link Component} that is executed if the {@link Decision}
     * is true.
     */
     ...
     * of the children of this Branch.  Also creates the {@link Exit Exits} for this
     * Branch based upon the {@link Exit Exits} of its components.
     */
    private void setControlDependencies ()
    {
        ...
        addDependencies(decision.makeEntry(inBuf.getExit(Exit.DONE)), clockBus, resetBus, goBus);

        final Component trueComponent = getTrueBranch();
        addDependencies(trueComponent.makeEntry(decision.getTrueBus().getOwner()), clockBus,
            resetBus, decision.getTrueBus());

        final Component falseComponent = getFalseBranch();
        addDependencies(falseComponent.makeEntry(decision.getFalseBus().getOwner()), clockBus,
            resetBus, decision.getFalseBus());
...
//          assert correlation.containsKey(this.decision) : "No decision in correlation map";
//          assert correlation.containsKey(this.trueBranch) : "No truebranch in correlation map";
//          assert correlation.containsKey(this.falseBranch) : "No falsebranch in correlation map";
