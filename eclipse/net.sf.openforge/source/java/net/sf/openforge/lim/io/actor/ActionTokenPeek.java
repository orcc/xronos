...
/**
 * ActionTokenPeek is a {@link Component} which represents an
 * access to the ActorPort in order to peek at the value specified by
 * an offset index into the queue.  The process of peeking at a value
 * does not affect the number or value of tokens in the queue. 
 * The component has a single output bus and always completes
 * combinationally. 
 *
...
public class ActionTokenPeek extends FifoAccess
{
    ...
    private ActionTokenPeek (FifoIF fifoIF, SimplePin indexPin, SimplePin dataPin)
    {
        ...
        Port index = makeDataPort(); // unused right now
        ...
        /* We only support token peeks to the head of the queue.  If
         * we leave this pin write here then resource sequencing will
         * enforce that 2 peeks are in different clock cyles due to
         * their index port writes.
