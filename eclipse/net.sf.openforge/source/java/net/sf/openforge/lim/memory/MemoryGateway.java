    ...
    public MemoryGateway (LogicalMemoryPort resource, int reads, int writes, int maxAddressWidth)
    {
        ...
        if (writes > 0)
        {
            ...
            for (int i = 0; i < writes; i++)
            {
                ...
                /*
                 * Just in case the incoming data isn't the same
                 * size as the memory that's being written, cast it
                 * to the width of the data path to the memory.
                 */
