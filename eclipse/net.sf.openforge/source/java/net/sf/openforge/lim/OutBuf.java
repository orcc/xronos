...
/**
 * An OutBuf is used to bring a structural flow to the outside
 * of a {@link Module} from its inside.  An OutBuf is created
 * automatically for each {@link Exit} of the {@link Module},
 * and adding a data {@link Bus} to the {@link Exit} also adds
 * a corresponding {@link Port} to the OutBuf.  The OutBuf itself
 * has no {@link Exit} and no {@link Bus Buses}.  Logically, the
 * {@link Bus Buses} of the {@link Exit} are the continuation of
 * the OutBuf's input {@link Port Ports}.
 *
...
public class OutBuf extends Component implements Emulatable
{
    ...
    /** The gate depth of this outbuf.  Always 0 except at the
     ...
     *
     * @return the {@link Exit} whose {@link Bus Buses} represent the continutation
     *         of this OutBuf's input {@link Port Ports}.
     */
    //public Exit getExit ()
    public Exit getPeer ()
    {
    ...
    /**
     * Constructs a new OutBuf.  It is package private since only {@link Exit}
     * is meant to call it.
     *
     * @param module the owner of this OutBuf
     * @param exit the peer {@link Exit} of this OutBuf
     */
     ...
     * value for {@link Component#producesDone()}.
     */
    ...
    /**
     * Sets the gate depth for this OutBuf, called by the Pipeliner to
    ...
    /**
     * Overrides method in Component to provide the OutBuf depth when
    ...
    /**
     * Pushes Value information across the Module boundry and returns
     * true if the pushed Value contains any new information.
     *
     * @return true if the Value pushed across the module boundry
    ...
    public boolean pushValuesForward ()
    {
        ...
        for (Iterator iter = getPorts().iterator(); iter.hasNext();)
        {
            ...
            {
                /*
                 * Do nothing for clock or reset, since they are not passed
                 * out of the OutBuf's module.  Also do nothing if the go
                 * is not being fed with a value.
                 */
            }
            else
            {
                ...
                if (peerValue == null)
                {
                    port.getPeer().setSize(pushedValue.getSize(), pushedValue.isSigned());
                 ...
                 * precise here, since an OutBuf is part Module and
                ...
                for (int i = 0; i < pushedValue.getSize(); i++)
                {
                    ...
                    if (!pushedValue.bitEquals(i,peerValue,i) && peerBit.isCare())
                    {
                        if (getOwner().isOpaque())
                        {
                            /*
                             * If this OutBuf lives in an opaque Module, then
                             * we have to avoid pushing through internal non-global
                        ...
                        {
                            /*
                             * Otherwise, this OutBuf lives in a transparent Module, and
                             * we can push through both constant and internal Bus Bits.
                             */
                            peerValue.setBit(i, pushedBit);
                            isModified = true;
                        }
                ...
                if (pushedValue.getCompactedSize() != peerValue.getCompactedSize())
                {
                    final Bit msb = peerValue.getBit(pushedValue.getCompactedSize()-1);
                    for (int i = pushedValue.getCompactedSize(); i < peerValue.getSize(); i++)
                    {
    ...
    /**
     * Determines the consumed {@link Value} of each outbuf Port's
     * peer Bus and updates the Port's value with that information.
