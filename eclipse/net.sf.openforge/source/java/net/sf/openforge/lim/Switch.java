...
/**
 * A Switch is implemented in terms of other constructs.  Primarily, it is a {@link Block}
 * that contains only one component: another {@link Block} that is the content of the
 * switch.  The reason for this indirection is that the BREAK {@link Exit Exits} of the
 * body must be diverted to DONE {@link Exit} of the Switch.  Other than this variation
 * in its construction, a Switch behaves exactly like any other {@link Block}.  So it is
 * up to the caller to provide appropriate contents.
 * <P>
...
public class Switch extends Block
{
     ...
     *
     * @param body the contents of the switch.
     */
    ...
    protected void setControlDependencies ()
    {
        ...
        /*
         * Merge plain BREAK Exit and Exit with same label as the
         * switch statement into the DONE Exit.
         */
    ...
    /**
     * Transfers the control dependencies from a given {@link Exit} to the
     * done {@link Exit}, creating one if necessary.  The given {@link Exit}
     * and its peer are then removed.
     */
    private void mergeBreakExit (Exit breakExit)
    {
        ...
        for (Iterator iter = breakOutBuf.getEntries().iterator(); iter.hasNext();)
        {
