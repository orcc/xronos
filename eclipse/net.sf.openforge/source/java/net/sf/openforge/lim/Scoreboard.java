...
/**
 * Scoreboard represents a logical AND in which the inputs may arrive
 * during different clock cycles.  In order to track which inputs have
 * recieved a logical true, the inputs must be latched when activated
 * until all inputs have been activated one time, at which point the
 * single output bus is set true for one cycle and the input latches
 * are all cleared.
 *
...
public class Scoreboard extends Module implements Composable
{
    ...
    private And scbdAnd;
    /* The result of this scoreboard... may be logically ORed with the
     * reset signal if any. */
     ...
     *
     * @param buses a value of type 'Collection'
     */
     ...
     * the feedback points in this Scoreboard.  This set is populated
     * from the Reg's created for this scoreboard since their Q is
     * directly connected to D and the 'done' from the scoreboard
     * feeds back around to the Reg.
     *
    ...
    protected void createSlice(int number,Port andPort,Port inputPort, boolean isStallPort)
    {
        // ok, what do we need....
        ...
        if (isStallable())
        {
            ...
            resetOr.getResultBus().setIDLogical(ID.showLogical(this) + "_sliceOr" + number);
            ...
            if (isStallPort)
            {
                ...
                maskAnd.getResultBus().setIDLogical(ID.showLogical(this) + "_sliceMaskAnd" + number);
                maskNot.getResultBus().setIDLogical(ID.showLogical(this) + "_sliceMaskNot" + number);
                maskNot.getDataPort().setBus(setBus);
                ((Port)maskAnd.getDataPorts().get(0)).setBus(getScbdResult());
                ((Port)maskAnd.getDataPorts().get(1)).setBus(maskNot.getResultBus());
        ...
        r.getResetPort().setBus(resetBus); // I dont think that this does anything...
        ...
        if (!isStallPort)
        {
            ...
            or.getResultBus().setIDLogical(ID.showLogical(this)+"_resOr"+number);
            addComponent(or);
            List l=(List)or.getDataPorts();
    ...
    /**
     * Gets the {@link Bus} which is asserted to indicate that all inputs
     * have been asserted.
     */
