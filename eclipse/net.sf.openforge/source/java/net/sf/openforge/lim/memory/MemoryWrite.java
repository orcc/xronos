...
public class MemoryWrite extends MemoryAccess implements StateAccessor
{
    ...
    /**
     * Returns a copy of this MemoryWrite by creating a new memory write
    ...
    public Object clone ()
    {
        ...
        final MemoryWrite clone = new MemoryWrite(isVolatile(), getWidth(), this.isSigned());
    ...
    public class Physical extends PhysicalImplementationModule
    {
         ...
         * port-bus connections of the MemoryWrite.
         */
        public Physical()
        {
            super(0); 
...
//             addressIn.getPeer().setSize(memWriteAddress.getBus().getSize(), memWriteAddress.getBus().getValue().isSigned());
...
//             sizeIn.getPeer().setSize(getSizePort().getBus().getSize(), getSizePort().getBus().getValue().isSigned());
            sizeIn.setUsed(getSizePort().isUsed());
            sizeIn.setBus(getSizePort().getBus());
            
//             dataIn.getPeer().setSize(memWriteData.getBus().getValue().getSize(), false);
...
//             assert (memWriteGo.getBus().getValue() != null) : "Bus attached to MemoryWrite's go port has no value.";
//             go.getPeer().setValue(new Value(go.getPeer(), memWriteGo.getBus().getValue()));
...
//             assert (memWriteClock.getBus().getValue() != null) : "Bus attached to MemoryWrite's clock port has no value.";
//             clk.getPeer().setValue(new Value(clk.getPeer(), memWriteClock.getBus().getValue()));
...
//             assert (memWriteReset.getBus().getValue() != null) : "Bus attached to MemoryWrite's reset port has no value.";
//             reset.getPeer().setValue(new Value(reset.getPeer(), memWriteReset.getBus().getValue()));
            ...
            Bus memWriteDone = (Bus)MemoryWrite.this.getExit(Exit.DONE).getDoneBus();
...
//             done.setValue(new Value(done, memWriteDone.getValue()));
            for (Iterator doneConsumers = memWriteDone.getPorts().iterator(); doneConsumers.hasNext();)
            {
...
//             sideWriteFinished.getPeer().setSize(1, true);
            ...
            sideAddress.setSize(memWriteAddress.getBus().getSize(), memWriteAddress.getBus().getValue().isSigned());

            sideData = physicalExit.makeDataBus(Component.SIDEBAND);
            sideData.setIDLogical(ID.showLogical(MemoryWrite.this) + "_WD");
            sideData.setSize(memWriteData.getBus().getSize(), memWriteData.getBus().getValue().isSigned());
            ...
            sideSize.setSize(MemoryWrite.this.getSizePort().getBus().getSize(), MemoryWrite.this.getSizePort().getBus().getValue().isSigned());
            ...
            List doneAndPorts = doneAnd.getDataPorts();
            Bus doneAndResult = doneAnd.getResultBus();
            addComponent(doneAnd);

            Or resetOr = new Or(2);
            List resetOrPorts = resetOr.getDataPorts();
        ...
        {
            return Collections.singleton(this.goRegister);
        }
