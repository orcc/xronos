...
/**
 * A TaskCall is an atomic node in the LIM which implements the
 * 'firing' of a top level design {@link Task}.  Specifically, the
 * TaskCall has a target Task which is enabled when the Go port of the
 * TaskCall is enabled.  The TaskCall stalls until the target task
 * issues a Done signal.  The Latency of this TaskCall is the same as
 * the Latency of the called Task.  
 *
 * @author  imiller
...
{
    private static final String rcs_id = "RCS_REVISION: $Rev: 153 $";
    ...
    /**
     * Constructs a new TaskCall with the specified {@link Task}
     * target. 
     */
    ...
    {
        if (target == null)
            throw new IllegalArgumentException("Cannot set Task Call to null target");

        if (this.targetTask != null)
            throw new IllegalStateException("Cannot change the target of a Task Call");
    ...
    /**
     * Returns the latency of this call by deferring to the target
     * task latency.
     */
    ...
    {
        throw new UnsupportedOperationException("Cannot modify the components internal to a task call");
    }
    ...
    private void build ()
    {
        ...
        {
            goPin = (SimplePin)((SimplePinRead)theCall.getGoPort().getBus().getOwner().getOwner()).getReferenceable();
        }
        
        if (!theCall.getExit(Exit.DONE).getDoneBus().isConnected())
        {
            final Module owner = theCall.getOwner();
            donePin = new SimpleInternalPin(1, theCall.showIDLogical() + "_done");
        ...
        else
        {
            ...
            assert owners.size() == 1 : "Task done must have only one target component " + owners.size();

            donePin = (SimplePin)((SimplePinWrite)owners.iterator().next()).getReferenceable();
        }
        ...
        getExit(Exit.DONE).getDoneBus().getPeer().setBus(doneRead.getResultBus());
    }
    ...
    {
        VariableLatencyExit (Module owner, int dataCount, Exit.Type type, String label)
        {
    ...
    /**
     * Performs the clone by returning an empty (untargetted) task
     * call which can be 'deep' cloned by calling setTarget with the
     * target of the original.
     */
    public Object clone ()
    {
