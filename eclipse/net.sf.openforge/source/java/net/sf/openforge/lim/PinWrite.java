...
/**
 * An {@link OutPinBuf} write access.  The write can be synchronous,
 * in which case it takes effect at the next clock edge, or
 * immediate.
 *
...
public class PinWrite extends PinAccess
{
    ...
    /**
     * Returns a copy of this PinWrite by creating a new access
    ...
    /**
     * The data in to the pin write should be sized to 
     * that of the the pin size is defined to be.
     *
     * @return A value of type 'boolean'
     */
    ...
    {
        /*
         * The OutBuf identity is dynamic.  The best we could do here is choose the max
         * size of any possible OutBuf after the ObjectResolver has run. --SGE
         */
        return false;
    }
    ...
    public class Physical extends PhysicalImplementationModule
    {
         ...
         * port-bus connections of the PinWrite.
         */
        public Physical()
        {
            super(0); 
            ...
            
            {
                ...
                address_in.getPeer().setSize(writeAddrBus.getSize(), writeAddrBus.getValue().isSigned());
            }
            ...
            
            {
                ...
                data_in.getPeer().setSize(writeDataBus.getSize(), writeDataBus.getValue().isSigned());
            }
            ...
            {
                Bus writeAddrBus = pinWrite_address.getBus();
                side_address.setSize(writeAddrBus.getSize(), writeAddrBus.getValue().isSigned());
            }
            ...
            {
                Bus writeDataBus = pinWrite_data.getBus();
                side_data.setSize(writeDataBus.getSize(), writeDataBus.getValue().isSigned());
            }
