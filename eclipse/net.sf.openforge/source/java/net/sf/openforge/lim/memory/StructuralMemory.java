...
public class StructuralMemory extends Module 
{
    ...
    public StructuralMemory (int memoryWidth, int memDepth, int bankWidth,
        List memoryPorts, String baseName, MemoryImplementation impl, int maxAddressWidth)
    {
     ...
     */
    public StructuralMemory (int memoryWidth, int memDepth, int bankWidth,
        List memoryPorts, String baseName, MemoryImplementation impl,
        AddressStridePolicy addrPolicy, int maxAddressWidth)
    {
        super();
        ...
        //
        //assert ((bankWidth % 8 == 0) || (memoryWidth == bankWidth)): "Memory Bank width must be power of 8, or width of memory";
        ...
        final int locationsPerLine = (int)Math.ceil(
            ((double)memoryWidth)/((double)Math.min(bitsPerAddress, memoryWidth)));
        ...
        for (Iterator iter = memoryPorts.iterator(); iter.hasNext();)
        {
            ...
            // Generate the DONE
            final Bus doneBus = createDone(structMemPort,
            ...
            if (this.addrShiftBits > 0)
            {
                ...
                if (getGenericJob().getUnscopedBooleanOptionValue(OptionRegistry.LITTLE_ENDIAN))
                {
                    shiftMagnitude = getLittleEndianDataShiftMagnitude(
                        structMemPort.getAddressPort().getPeer(), bitsPerAddress);
                    ...
                    dataOut = getLittleEndianShiftedDataOut(structMemPort,
                        doutConcatOr.getResultBus(), shiftMagnitude, noShifter);
                }
                else
                {
                    ...
                    shiftedDataIn = getBigEndianShiftedDataIn(structMemPort,
                        shiftMagnitude, memoryWidth, noShifter, bitsPerAddress);

                    dataOut = getBigEndianShiftedDataOut(structMemPort, 
                        doutConcatOr.getResultBus(), shiftMagnitude, memoryWidth, noShifter, bitsPerAddress);
                }

                // Create the n-bit select bus
                selectBus = getSelectBus(
                    structMemPort.getSizePort().getPeer(),
            ...
            for (int i=0; i < bankCount; i++)
            {
                ...
                MemoryBank.BankPort bankPort = bank.createPort(
                    structMemPort.isRead(), structMemPort.isWrite(), memPort.getWriteMode());
                ...
                Bus bankBus =  bank.instantiateDataLogic(this,
                    shiftedDataIn, bankWidthConst.getValueBus(), i,
                    memoryWidth, bankPort);
    ...
    private StructuralMemoryPort createInterface (LogicalMemoryPort memPort, int memWidth)
    {
        ...
        StructuralMemoryPort port = new StructuralMemoryPort(
            address, din, en, wen, size, dout, done, read, write);
    ...
    private Bus getShiftedAddress (Bus address)
    {
        ...
        if (this.addrShiftBits > 0)
        {
            ...
            //int addrStages = (int)Math.ceil(Math.log(this.addrShiftBits)/Math.log(2.0));
     ...
     * bus.
     */
    private Bus getLittleEndianShiftedDataIn (StructuralMemoryPort structMemPort,
     ...
     */
    private Bus getLittleEndianShiftedDataOut (StructuralMemoryPort structMemPort,
        Bus dataBus, Bus shiftMagnitude, boolean noShifter)
    {
        Bus dataOut;
        if (structMemPort.isRead())
        {
            ...
            else
            {
                ...
                Bus delayedShiftMagnitude = delayBus(shiftMagnitude, "shiftDelay",
                    this.memImpl.getReadLatency().getMinClocks(),
                    getClockPort().getPeer(), getResetPort().getPeer());
...
//         final double dataBytes = ((double)this.getDataWidth()) / ((double)8);
    ...
    private Bus getBigEndianShiftedDataIn (StructuralMemoryPort structMemPort,
        Bus shiftMagnitude, int memWidth, boolean noShifter, int bitsPerAddress)
    {
        Bus shiftedDataIn;
        if (structMemPort.isWrite())
        {
    ...
    private Bus getBigEndianShiftedDataOut (StructuralMemoryPort structMemPort, Bus dataBus,
        Bus shiftMagnitude, int memWidth, boolean noShifter, int bitsPerAddress)
    {
        Bus dataOut;
        if (structMemPort.isRead())
        {
            ...
            {   
                ...
                final Bus delayedShiftMagnitude = delayBus(shiftMagnitude, "shiftDelay", 
                    this.memImpl.getReadLatency().getMinClocks(),
                    getClockPort().getPeer(), getResetPort().getPeer());
...
//         final double bankBytes = ((double)this.bankWidth) / ((double)8);
     ...
     */
    private Bus createDone (StructuralMemoryPort memPort, 
        Bus clk, Bus rst, MemoryImplementation imp, String baseName)
    {
        ...
        {
            Bus readDelayed = delayBus(memPort.getEnablePort().getPeer(), baseName + "_re_delay",
                imp.getReadLatency().getMaxClocks(), clk, rst);
            Bus writeDelayed = delayBus(memPort.getWriteEnablePort().getPeer(), baseName + "_we_delay",
                imp.getWriteLatency().getMaxClocks(), clk, rst);
        ...
        {
            done = delayBus(memPort.getEnablePort().getPeer(), baseName + "_re_delay",
                imp.getReadLatency().getMaxClocks(), clk, rst);
        }
        else
        {
            done = delayBus(memPort.getWriteEnablePort().getPeer(), baseName + "_we_delay",
                imp.getWriteLatency().getMaxClocks(), clk, rst);
        }
        return done;
    }
    ...
    public class StructuralMemoryPort
    {
        ...
        public StructuralMemoryPort(Port a, Port d, Port e, Port w,
            Port s, Bus b, Bus dn, boolean rd, boolean wr)
        {
            this.addr = a;
            this.din = d;
            this.en = e;
        ...
        public Port getSizePort () { return this.size; }
        public Bus getDataOutBus () { return this.dout; }
        public Bus getDoneBus () { return this.done; }
        ...
        public void remove ()
        {
            ...
            if (this.dout != null)
                StructuralMemory.this.getExit(Exit.DONE).removeDataBus(this.dout);

            if (this.done != null)
                StructuralMemory.this.getExit(Exit.DONE).removeDataBus(this.done);
        }
    }
