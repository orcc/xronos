...
public class Register extends Storage implements StateHolder, Arbitratable
{
    ...
    public int getSpacing (Referencer from, Referencer to)
    {
        ...
        /*
        if ((from instanceof RegisterWrite) || (from instanceof RegisterRead))
            return 1;
        else
    ...
    public class Physical extends PhysicalImplementationModule
    {
        ...
        private Physical(List readers, List writers, String logicalId)
        {
            ...
            /*
             * Use the byte representation because we want the
             * register to have exactly the same byte representation
             * as the memory (endian wise) and the toConstant() method
             * will always return a little endian representation.
             */
...
//             Constant initConstant = SimpleConstant.createConstant(initRep);
            ...
            //initConstant.lock(); // at this point we MUST be able to resolve the constant.
            ...
            if (writers.size() > 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                         ...
                         * the fact that I tie off the address ports to a one bit
                         * constant.  Should be easy to fix (when
                         * needed).  To get working this will also
                         * need to ensure that there are equal sized
                         * writer/reader lists.  Code in
                         * GlobalConnector exists but is commented out
                         * (see visit(Task))
                         */
        ...
        public String toString ()
        {
            //String oneBits = (oneBitRegs == null ? "null":Integer.toString(oneBitRegs.length));
        ...
        {
            // assert false : "Nobody should visit this directly. " + getClass(); // nobody should be visiting this component directly
