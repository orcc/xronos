...
public class GnuOptionValidator 
{
        ...
        for (i=0; i<args.length; i++)
        {
            ...
            if (currentToken.charAt(0) == '-')
            {
                ...
                if (currentToken.charAt(1) == '-')
                {
                    ...
                    {
                        i++;
                        break; // no longkey means just "--" was found, end of options
                    }
                    ...
                    if (def != null) 
                    {
                                ...
                                ((argFlag & GnuOptionDefinition.REQUIRED_ARGUMENT) != 0))
                        {
                            goodOptions.add(def);
                            putArg(def, arg);
                        } else badOptions.add(currentToken);
                    }
                    else badOptions.add(currentToken);
                }
                else
                {
                    ...
                    {
	                    GnuOptionDefinition def = dict.getDefinition(currentToken.charAt(j));
	                    if (def != null) 
                        {
	                        int argFlag = def.getArgFlag();
	                        if ((argFlag & (GnuOptionDefinition.OPTIONAL_ARGUMENT | 
	                                GnuOptionDefinition.REQUIRED_ARGUMENT)) != 0)
	                        {
	                            String arg=null;
	                            ...
	                            else
	                            {
	                                try {
	                                    ...
	                                    if (arg.charAt(0) == '-')
	                                    {
	                                        // next token is another option, not this
	                                        // option's argument
	                                        arg = null;
	                                        i--;
	                                    }
    ...
    private void putArg(GnuOptionDefinition definition, String arg)
    {
        ...
        if (argument == null) argument = new ArrayList();
        argument.add(arg);
    ...
    /**
     * Returns the list of command arguments which have
     * been collected for the most recently validated
     * command line.
     * 
	...
	public String toString() 
	{
		...
		if (isValid()) buf.append("valid");
		else buf.append("invalid");
		buf.append("; ");
		buf.append("good: " );
