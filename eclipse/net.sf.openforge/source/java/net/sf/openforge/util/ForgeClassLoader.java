...
public class ForgeClassLoader extends ClassLoader
{
    ...
    /**
     * Construct a new ForgeClassLoader which uses the paths contained
     * in 'classpath' when finding classes.  Any classes not found in
     * these paths will use the system class loader.  Any system
     * and/or API classes will also use the system class loader.
     *
     * @param classpath a value of type 'String[]'
     */
     ...
     * @param resolve true if the class is to be resolved after loading
    ...
    throws ClassNotFoundException
    {
        ...
        //
            if (ForgeKnownClasses.isSystemClass(name) || ForgeKnownClasses.isAPIClass(name))
            {
                try
                {
     ...
     *
     * @param size the number of bytes that must be accommodated in
     *             the buffer
     */
    ...
    {
        String className = rootFilename.replace('.', File.separatorChar) + ".class";
