...
/**
 * GCC is a class to interface with the GNU C Compiler. It executes the
 * GNU C compiler, {@link GCC}, on a given stream of input.  The compiler
 * is executed in a separate process, and its standard output and error
 * are redirected to specified output streams.
 * <P>
 ...
 * NOW gcc is expected to be in ForgeHome/gcc/bin/gcc
 * This can be overriden by defining the FORGE_GCC environment var.
 *
...
{
    /**
     * The path of the GNU C Compiler - can't initialize statically
     * because failure throws an exception, which screws up
     * under obfuscation
     */
     ...
     *
     * @param commandLine a value of type 'String'
     */
    ...
    public boolean success() { return status==0; }

    public String[] getArguments () { return commandArgs; }    

    /**
     * If the platform is windows, this method will wrap any command
     * line argument which contains whitespace with double quotes.
     *
     ...
     *
     * @param out a stream to which the preprocessor's standard output will be written
     * @param err a stream to which the preprocessor's standard error will be written
     *
     * @return the exit value from the preprocessor
     *
     * @exception java.io.IOException if an error occurs on the input or output streams
     * @exception ExecutionException if the preprocessor cannot be executed
     */
    ...
    throws java.io.IOException, ExecutionException
    {
        ...
        {
            throw new ExecutionException("C compiler " + gccExe + " not found.  Please check your installation - the Forge expects to find gcc"
                                         + File.separator + "bin" + File.separator + "gcc in the Forge install directory.");
        }

        final Drain outConduit = new Drain(process.getInputStream(), new PrintWriter(out));
        final Drain errConduit = new Drain(process.getErrorStream(), new PrintWriter(err));
    ...
    /**
     * Finds gcc or gcc.exe in forge.home + "/gcc/bin".  Returns "gcc"
     * otherwise.
     */
    private static String findExecutable ()
    {
        ...
        if (forgeHome != null)
        {
            final String gccHome = forgeHome + File.separator + "gcc" + File.separator + "bin";
            ...
            if (gccFile.exists())
            {
                ...
                for (int i = 0; i < gccNames.length; i++)
                {
                    ...
                    try
                    {
                        final String gccPath = gccHome + File.separator + gccNames[i];
                        final File canonicalFile = new File(gccPath).getCanonicalFile();
     ...
     * whether the path separator is ":" or ";".  True, 'tis ugly, but don't
     * you know in your heart that it will always work?
     */
