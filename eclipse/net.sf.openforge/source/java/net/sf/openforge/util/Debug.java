...
import net.sf.openforge.lim.graph.*;
 ...
 * it is compiled in, then debug output will be printed. However, you can control
 * the levels of debug by selecting and deselecting the bitmask of levels via System.property
 * settings. 2 settings are used:
 *
 * debug.internal.<tag>=? true for visible, false for invisible
 * debug.internal.<tag>.levels=? integer bitmask of levels to display.
 *
 * In each case, first the System properties are checked, and if it is not found there, the
 * property name has it's periods (.) replaced with underscores (_) and the
 * environment is checked iff a Properties object was given at construction.
 *
 * Each line of output is of the form:
 * <pre><tag><preface>: <debug text></pre> and multiple lines will be printed as individual
 * lines.
 *
 * @author cschanck
...
public class Debug
{
    private static final String rcs_id = "RCS_REVISION: $Rev: 107 $";
    ...
    /** Used for temporary debug statements.
     * If no level is specified, these will display.
     */
    ...
    public static final int GR_DEPENDENCY=GR_DATA|GR_CONTROL|GR_LOGICAL|GR_STRUCTURAL;
    ...
    public static final int GR_DEFAULT=GR_DATA|GR_CONTROL|GR_LOGICAL|GR_STRUCTURAL|GR_PHYSICAL;
     ...
     *
     * @param controlClass Class containing initialization of this object
     * @param tag Tag to use to preface each line of output
     * @param compiledIn true/false final value as to whether debug should be compiled in
     * @param visible true/false flag as to whether debug is displayed
     * @param levels debug level mask to use
     * @param os OutputStream to output debug to
     */
    public Debug(Class controlClass,String tag,boolean compiledIn,boolean visible,int levels,OutputStream os)
    {
        ...
        if(tag==null)
        throw new IllegalArgumentException("Must specify tag for debug class: "+controlClassName);
    ...
    public Debug(Class controlClass,String tag,boolean compiledIn,boolean visible,OutputStream os)
    {
        this(controlClass,tag,compiledIn,visible,DEFAULT_LEVELS,System.out);
    }
    
    public Debug(Class controlClass,String tag,boolean compiledIn,boolean visible,int levels)
    {
        this(controlClass,tag,compiledIn,visible,levels,System.out);
    }

    public Debug(Class controlClass,String tag,boolean compiledIn,boolean visible)
    {
     ...
     * 
     * @param pad text to use to indent
     * @param src source text to indent
     ...
     */
    public final boolean on() { return compiledIn; }
    ...
    public void inspect(Object o, String fileName)
    {
        if (visible)
        {
            ...
            {
                System.err.println("Could not write "+fileName+" to inspect "+o.getClass().getName()+": error: "+e);
            }
        }
    }
     ...
     * @param file filename to write (process with dotty after running)
     * @param landscape true if graph should be printed in landscape mode
     */
    ...
    public void dependencyGraph(Visitable o, String title, String fileName, int flags)
    {
        if (visible)
        {
     ...
     * @param landscape true if graph should be printed in landscape mode
     */
    public static void lgraphTo (Visitable o, String title, String fileName, int flags)
    {
        try
        {
        ...
        {
            System.err.println("Could not write "+fileName+" to graph "+o.getClass().getName()+": error: "+exception);
        }
    }
     ...
     *
     * @param o a object of either Design or Module
     * @param fileName a value of type 'String'
     */
    ...
    /**
     * write an LWireGraph  graphviz file describing the wire connections of an object
     * @param o object of interest
     * @param title of graph
     * @param file to write to
     */
     ...
     *
     * @param o a value of type 'Visitable'
     * @param title a value of type 'String'
     * @param fileName a value of type 'String'
     * @param flags a value of type 'int'
     */
    public static void depGraphTo(Visitable o, String title, String fileName, int flags)
    {
        try
        {
            LDependencyGraph lgraph = new LDependencyGraph(title, o, flags);
    ...
    private void checkActive()
    {
        if(compiledIn)
        {
            ps.println(tag+" ** Debug Info Potentially Compiled In Class: "+controlClassName+" **");
            String result=getProperty(tag);
            if(result!=null)
            {
                visible=Boolean.valueOf(result).booleanValue();
                ps.println(tag+" ** Property "+lastPropertyName+" overriding: visible="+visible+" **");
            }
            if(visible)
            {
                ...
                if(result!=null)
                {
                    ...
                    ps.println(tag+" ** Property "+lastPropertyName+" overriding: levels=0x"+HF.hex(levels)+" **");
                }
        
            }
            ps.println(tag+" ** Debug ["+tag+"] visible: "+visible+"; levels: 0x"+HF.hex(levels)+" **");
        }
    }
    ...
    private int getIntegerValue(String s)
    {
        s=s.toLowerCase();
        if(s.startsWith("-"))
            throw new IllegalArgumentException("Invalid value for debug level: "+s);
        if(s.startsWith("0x"))
        {
            // parse as hex
            s=s.substring(2);
