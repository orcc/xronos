    ...
    public void visit (TaskCall mod)
    {
        ...
        for (Iterator iter = pid.getProcessKeys().iterator(); iter.hasNext();)
        {
            ...
            if (pid.stateToFirst.containsKey(key))
                targets.addAll((Collection)pid.stateToFirst.get(key));
            if (pid.stateToLast.containsKey(key))
                targets.addAll((Collection)pid.stateToLast.get(key));
    ...
    public void visit (Branch branch)
    {
        ...
        for (Iterator iter = keys.iterator(); iter.hasNext();)
        {
            ...
            if (trueIdentifier.stateToFirst.containsKey(key))
                targets.addAll((Collection)trueIdentifier.stateToFirst.get(key));
            if (falseIdentifier.stateToFirst.containsKey(key))
                targets.addAll((Collection)falseIdentifier.stateToFirst.get(key));
            
            if (trueIdentifier.stateToLast.containsKey(key))
                targets.addAll((Collection)trueIdentifier.stateToLast.get(key));
            if (falseIdentifier.stateToLast.containsKey(key))
                targets.addAll((Collection)falseIdentifier.stateToLast.get(key));
