...
/**
 * CycleCTestbench is responsible for writing a <code>main</code>
 * function that will parse the <code>xxxx_input_blocks.vec</code> and
 * the <code>xxxx_output_expected.vec</code> and use those values to
 * verify that the cycle accurate C model generates the correct
 * results.  It does NOT verify the cycle accuracy, however it does
 * generate a <code>test_c_sim.results</code> file that matches the
 * format generated by the verilog simulation and which may be used to
 * verify the cycle accuracy.
 *
 *
 * <p>Created: Fri Mar 18 17:30:01 2005
 *
...
//         ps.println("");
        ...
        else if (ioHandle.getInputCount() == 0 && ioHandle.getOutputCount() != 0)
    ...
    public static void writeQueueTestingMain (PrintStream ps, String fxnBaseName, String baseName, IOHandler ioHandle, Set<String> goVars)
    {
        ...
        ps.println("int main (int argc, char *argv[])");
        ps.println("{");
        if (DO_CYCLES) ps.println("    FILE *cycleResults = fopen(\""+baseName+"_csim.cycles\", \"w\");");
        ...
        {
            ps.println("    queueId = registerVectorFile(\""+baseName+"_"+portName+".vec\");");
            ps.println("    simId = getInterfaceID(\""+portName+"\");");
            ps.println("    if (DO_DEBUG) printf(\""+portName+" qid %d simid %d\\n\", queueId, simId);");
        ...
        for (Iterator iter = goVars.iterator(); iter.hasNext();)
        {
            String goVar = (String)iter.next();
            dbg1 += "%d ";
            dbg2 += goVar;
            if (iter.hasNext()) dbg2 += " ,";
        }
        ...
        if (DO_CYCLES)
        {
            ...
            for (int i=0; i < ports.size(); i++)
            {
                ...
                String send = id+"->send";
                String ack = id+"->ack";
                cycleString2 += "(" + send + " || " + ack + ") ? " + id+"->data : 0, " + id + "->send, " + id + "->ack";
                if (i < (ports.size()-1)) cycleString2 += ", ";
            }
        ...
        ps.println("        ");
        ps.println("");
        if (DO_CYCLES) ps.println("    fclose(cycleResults);");
