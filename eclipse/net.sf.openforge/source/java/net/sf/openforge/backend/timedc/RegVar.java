...
/**
 * RegVar maintains all the stateful variables for a simple
 * flip-flop.  The stateful variables are only for current and next
 * value.  All the management of enables, set, reset, etc are handled
 * during the combinational phase such that the 'next' value is always
 * correct and will always be clocked through.  In the case of an
 * enabled register whose enable is not correct, the next value is
...
public class RegVar extends OpHandle implements StateVar
{
     ...
     * correct initial value for the flop.
     *
    ...
    public void declareGlobal (PrintStream stream)
    {
        ...
        if (this.reg.getInitialValue() != null)
        {
            ...
            initNum = this.reg.getInitialValue().toNumber().numberValue().longValue();
        }
        assert this.reg.getResultBus().getValue() != null;
        String declType = StateVar.STORAGE_CLASS + getTypeDeclaration(this.reg.getResultBus().getValue());
