 ...
 * @author sb
 *
 * An Engine represents a single compilation task processed under
 * a specific set of user preferences. This is the compiler's
 * source for all user-configurable settings. Also, all access 
 * to the "environment" passes through the Engine, so that interested
 * listeners can monitor what is going on and each engine can potentially
...
public abstract class Engine extends Stoppable.Adapter implements Runnable{
    ...
    /** The LIM design produced by this job. The design may be null, or the result
     * of any stage of compilation. */
     ...
     */
    public GenericJob getGenericJob() { return gj; }
    
    /**
     * Gets the design produced by this Engine. May be null, or the result of any
    ...
    /**
     * Tell the Engine to start processing. The Engine status 
     * should only change to IN_PROGRESS when the current
     * handler fires an event to indicate that processing
     * is underway.
     *
     */
    public final void begin()
    {
        try
        {
            ...
            if(design!=null)
            {
            	...
            	System.out.println("LIM Compiled in: " + ((float) (t1 - t0) / (float) 1000)
					+ "s\n");
            }
            else
            {
    ...
    /**
     * Stop the processing of the current
     * job by passing on the stop request to
     * the current job handler. This returns
     * immediately, regardless of whether
     * the handler has actually stopped or not.
     * Use status() to determine if the job 
    ...
    /**
     * Runs this Job by calling begin(). Allows this
     * Job to be attached to a Thread.
     */
    public void run()
    {
        begin();
    ...
    /**
     * Return the current GenericJob of the current Engine for the
     * current thread.
     *
     ...
     *
     * @param event the JobEvent to distribute
     */
    public void fireEvent(JobEvent event)
    {
        ...
        {
            ((JobListener)it.next()).jobNotice(event);
        }
    } // fireEvent()
     ...
     *
     * @param handler the JobHandler which is starting
     */
     ...
     *
     * @param handler the JobHandler which has finished
     */
     ...
     * engine.  This is a duplicate of GenericJob.fatalError(String s); so call that
     *
