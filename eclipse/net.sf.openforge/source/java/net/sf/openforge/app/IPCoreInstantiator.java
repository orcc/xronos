 ...
 * 
 * IPCoreInstantiator is responsible for adding each IPCore module
 * instantiation to the design. Each instantiation's ports and buses
 * connection is being created also.
 *
 * @author ysyu
...
{
    private static final String _RCS_ = "RCS_REVISION: $Rev: 282 $";

    private Design design = null;

    /** IPCoreStorage to IPCoreCall map */
    private HashMap ipCoreCallMap = new HashMap(3);
    
    /** API Buffer to LIM Pin map */
    private HashMap apiToLimPins = new HashMap(3);

    /** IPCoreStorage -> Collection of user accessed pins to the core
     * tracked by that storage. This contains only those pins that the
     * user actually accesses in their design. */
    private HashMap coreStorageToUserPins = new HashMap();
    
    /**
     * Instantiates each ipcore module and ports & buses. 
     *
     * @param design the whole design
     */
    ...
    /**
     * 1. ip core pins will be put into each ip core's pin list 
     * 2. make ip core module instantiation 
     * 3. add each ip core call to the design 
     */
    private void instantiate()
    {
        for (Iterator iter = design.getPins().iterator(); iter.hasNext();)
        {
            ...
            final IPCoreStorage storage = Core.getIPCoreStorage(pin.getApiPin());
            if (storage != null)
            {
                Collection pins = (Collection)coreStorageToUserPins.get(storage);
                if (pins == null)
                {
                    pins = new HashSet();
        ...
        for(Iterator iter = design.getPins().iterator(); iter.hasNext();)
        {
            ...
            if(storage != null)
            {
                ...
                /*
                 * Create ip core module instantiation
                 * and a dummy procedure to go with it
                 */
...
//         }
    ...
    private static void createPort (Call call, String portName, Bus bus, int width)
    {
    ...
    /**
     * The clock pin to use in place of the AUTOCONNECT domains clock
     * is the clock that is associated with each data pin connected to
     * the core.
     *
     * @param ipcs a value of type 'IPCoreStorage'
...
//         assert ckPin != null;
    ...
    /**
     * The reset pin to use in place of the AUTOCONNECT domains reset
     * is the reset that is associated with each data pin connected to
     * the core.
     *
     * @param ipcs a value of type 'IPCoreStorage'
     * @return a value of type 'InputPin'
...
//         assert rstPin != null;
    ...
    private void makeName()
    {
        for(Iterator it = ipCoreCallMap.keySet().iterator(); it.hasNext();)
        {
            ...
            {
                call.setIDLogical(ipcs.getModuleName() + "_" + ipcs.getHDLInstanceName());
            }
        ...
        for(Iterator it = ipCoreCallMap.keySet().iterator(); it.hasNext();)
        {
            ...
            for(Iterator iter = userPins.iterator(); iter.hasNext();)
            {
                ...
                if((pin instanceof InputPin))
                {
                    ...
                    // port on ipcore call.  
                    callPort.setBus(pin.getInPinBuf().getPhysicalComponent().getResultBus());
                ...
                else if ((pin instanceof OutputPin) || (pin instanceof BidirectionalPin))
                {
...
//                     final Bus callBus = call.getExit(Exit.DONE).makeDataBus();
//                     final Bus procBus = proc.getBody().getExit(Exit.RETURN).makeDataBus();
...
//                     // name the bus on procedure and call with the name of ipcore
//                     // user pin.
...
//                     procBus.setIDLogical(name);
    ...
    private void makePublishPins()
    {
        for(Iterator it = ipCoreCallMap.keySet().iterator(); it.hasNext();)
        {
            ...
            for(Iterator iter = ipcs.getPublishedPins().iterator(); iter.hasNext();)
            {
                final Buffer buffer = (Buffer)iter.next();
                final int width = buffer.getSize();
                ...
                {
                    final Bus bus = ((Exit)call.getExits().iterator().next()).makeDataBus();
                    bus.setUsed(true);

                    final Exit exit = (Exit)proc.getBody().getExits().iterator().next();
                ...
                if(buffer instanceof PinInOutTS)
                {
                    ...
                    final BidirectionalPin io = new BidirectionalPin(width, buffer.isSigned());
                    io.setIDLogical(publish_name);
                    io.getPort().setBus(bus);
                    io.setApiPin(buffer);

                    throw new IllegalStateException("No longer supports bidirectional pins");
//                     design.addBidirectionalPin(io);
    ...
    private void makeNoConnectPins()
    {
        for(Iterator it = ipCoreCallMap.keySet().iterator(); it.hasNext();)
        {
            ...
            for(Iterator iter = ipcs.getNoConnectPins().iterator(); iter.hasNext();)
            {
                final Buffer buffer = (Buffer)iter.next();
                final int width = buffer.getSize();
     ...
     *
     * PinIn => no connect pin
     * PinOut => feed it with its reset value
     */
    private void makeUnusedPins()
    {
        for(Iterator it = ipCoreCallMap.keySet().iterator(); it.hasNext();)
        {
            ...
            for(Iterator itt = ipcs.getAllPins().iterator(); itt.hasNext();)
            {
                ...
                {
                    Constant constant = new SimpleConstant(buffer.getResetValue(), buffer.getSize(), true);
    ...
    /** 
     * Each IP Core published pin should also have the unique name
     * in the user application. All the published name will be
     * checked againest all the names of pins on user application. 
     */
    private void checkPublishPinNames()
    {
        Set names = new HashSet();
        for(Iterator iter = Core.getIPCoreStorages().iterator(); iter.hasNext();)
        {
            IPCoreStorage storage = (IPCoreStorage)iter.next();
            
            // Check for duplicated name
            for(Iterator iterate = storage.getPublishedNames().iterator(); iterate.hasNext();)
            {
                String pname = (String)iterate.next();
                
                if(!names.add(pname))
                {
                    throw new ForgeApiException("Duplicate Publish Pin Name Found: " + pname);
                }
            }
        }
    }
