...
/** 
 * An option which allows an integer to be used for the value, with
 * units picked from a pre-defined set of possible strings.
 *
...
public class OptionIntUnit extends OptionPickOne
{
     ...
     * @param possible_units an array of all possible values this option can have
     * @param hidden whether this option should be hidden from the user
     */    
    public OptionIntUnit(OptionKey key,
    				...
    				if(i < (tokens.size() - 2))
    				{
    					...
    					if(!this.isValid(value1))
    					{
    						...
    						if(this.isValid(value1))
    						{
    	    					...
    	    					//remove first token
    	    					tokens.remove(i+1);
    	    					//remove second token. Note that in the prev step, the elements moved left by 1
    	    					tokens.remove(i+1);
    	    					tokens.add(i+1, s);		
    						}
    						else
    						{
    							throw new NewJob.InvalidOptionValueException(key, value1);
    						}
    					}
    					else
    					{
    ...
    /**
     * This method checks to see if the value passed to the option is 
     * of the right type. If true, it replaces the value of the option 
     * with that new value. Otherwise, it throws and InvalidOptionValue Exception.
     * @param slabel SearchLabel that can be used to get the scope.
     * @param val value to the option.
     */
