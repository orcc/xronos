...
/**
 * The Optimization entry to the pipelining functionality.  The
 * optimization makes 2 passes over the design.  The first pass does a
 * traversal to determine several things:
 * <ul>
 ...
 * <ul>
 * <li>{@link OptionRegistry#SCHEDULE_PIPELINE_ENABLE} - turns
 * pipelining on or off
 * <li>{@link OptionRegistry#SCHEDULE_PIPELINE_AUTO_LEVEL} -
 * pipelining will be done with a global target level of maxdepth /
 * apl level
 * <li>{@link OptionRegistry#SCHEDULE_PIPELINE_GATE_DEPTH} -
 * pipelining will be done with a global level of the specified depth
 * </ul>
 *
...
{
    private static final String _RCS_ = "$Rev: 198 $";
    ...
    {
        if (_d) _dbg.ln("Calculating gate depth prior to pipelining -- pre-scheduling");
        ...
        gj.info("Maximum unbreakable combinational complexity: " + design.getUnbreakableGateDepth());
        if (_d) Debug.depGraphTo(design,"Pipelining","pipe-before.dot",Debug.GR_DEFAULT);
        
        if (gj.getUnscopedBooleanOptionValue(OptionRegistry.SCHEDULE_PIPELINE_ENABLE))
        {
            Option option = gj.getOption(OptionRegistry.SCHEDULE_PIPELINE_AUTO_LEVEL);
            final int auto_level = ((OptionInt)option).getValueAsInt(CodeLabel.UNSCOPED);
            option = gj.getOption(OptionRegistry.SCHEDULE_PIPELINE_GATE_DEPTH);
            final int spec_level = ((OptionInt)option).getValueAsInt(CodeLabel.UNSCOPED);

            if (spec_level > 0)
            {
                final String gateDepthOpt = OptionRegistry.SCHEDULE_PIPELINE_GATE_DEPTH.getHelpFormattedKeyList();
                //gj.warn("Use of a deprecated option '" + gateDepthOpt + "' detected.");
                if (auto_level > 0)
                {
                    gj.warn("\tThe auto pipelining level specification will overide the value supplied by '" + gateDepthOpt + "'");
                }
            }
            
            int targetGateDepth = -1;
            
            if (_d) _dbg.ln("-->level "+auto_level);
            if (auto_level >= 1)
            {
            ...
            if ((targetGateDepth > 0) || (predictedRegCount > 0))
            {
                ...
                {
                    gj.info("Target post-pipelining combinational complexity: " + targetGateDepth);
                }
                else
                {
                    gj.info("Expected register insertion count: " + predictedRegCount);
                }
                ...
                if (_d) Debug.depGraphTo(design,"Pipelining","pipe-after.dot",Debug.GR_DEFAULT);
            }
        }
        gj.dec();
    }

    public int getCount() { return pipelineCount; }
