 ...
 * analyzing each component in the LIM and replaces it with a floating
 * point constant node if all inputs are constant valued. Floating point
 * constant values are represented with the raw long/int bits representation.
 *
 * @author ysyu
...
class FullFloatingPointHelper 
{
     ...
     * @param fullv the visitor that called this method.
     */    
...
//     }
    ...
    /**
     * Copies any constant value applied to the OutBuf back over
     * to the output Buses of the Call.
     *
     * @param call a method call.
     * @param fullv the visitor that called this method.
     */
...
//                         cValue = new SimpleConstant(Double.doubleToRawLongBits(dfp.doubleValue()), 64);
//                     }
//                     else
//                     {
//                         Number ffp = new Float(Float.intBitsToFloat((int)bus.getValue().getValueMask()));
//                         cValue = new SimpleConstant(Float.floatToRawIntBits(ffp.floatValue()), 32);
//                     }
...
//                         for(Iterator diter = entry.getDependencies(port).iterator(); diter.hasNext();)
//                         {
...
//     }
    ...
    static void makeFPConstant(AddOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
    ...
    static void makeFPConstant (CastOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
    ...
    static void makeFPConstant (DivideOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[1].doubleValue() == 0.0)
        {
            ...
            if (consts[1] instanceof Double)
                fpc = new SimpleConstant(0, 64);
            else
                fpc = new SimpleConstant(0, 32);
        }
    ...
    static void makeFPConstant (EqualsOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        if(consts[0].doubleValue() == consts[1].doubleValue())
        {    
            fullv.replaceComponent(comp, new SimpleConstant(1,1));
        }
        else
        {
            fullv.replaceComponent(comp, new SimpleConstant(0,1));
        }
    }
    
    static void makeFPConstant (GreaterThanEqualToOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        if(consts[0].doubleValue() >= consts[1].doubleValue())
        {
            fullv.replaceComponent(comp, new SimpleConstant(1,1));
    ...
    static void makeFPConstant (GreaterThanOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        if(consts[0].doubleValue() > consts[1].doubleValue())
        {
            fullv.replaceComponent(comp, new SimpleConstant(1,1));
    ...
    static void makeFPConstant (LessThanEqualToOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        if(consts[0].doubleValue() <= consts[1].doubleValue())
        {
            fullv.replaceComponent(comp, new SimpleConstant(1,1));
    ...
    static void makeFPConstant (LessThanOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        if(consts[0].doubleValue() < consts[1].doubleValue())
        {
            fullv.replaceComponent(comp, new SimpleConstant(1,1));
    ...
    static void makeFPConstant (MinusOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[0] instanceof Double)
        {
    ...
    static void makeFPConstant (ModuloOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[0].doubleValue() == 0.0)
        {
        	EngineThread.getGenericJob().warn("    Found devide by 0 at line " + comp.getIDSourceInfo().getSourceLine() + ", replacing with 0.");
            if (consts[0] instanceof Double)
                fpc = new SimpleConstant(0, 64);
            else
                fpc = new SimpleConstant(0, 32);
    ...
    static void makeFPConstant (MultiplyOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[0] instanceof Double || consts[1] instanceof Double) 
        {
    ...
    static void makeFPConstant (NoOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[0] instanceof Double)
        {
            fpc = new SimpleConstant(Double.doubleToRawLongBits(consts[0].doubleValue()), 
                               comp.getResultBus().getValue().getSize());
        }
        else
        {
            fpc = new SimpleConstant(Float.floatToRawIntBits(consts[0].floatValue()), 
                               comp.getResultBus().getValue().getSize());
        }
    ...
    static void makeFPConstant (NotEqualsOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        if(consts[0].doubleValue() != consts[1].doubleValue())
        {
            fullv.replaceComponent(comp, new SimpleConstant(1,1));
    ...
    static void makeFPConstant (NumericPromotionOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[0] instanceof Double)
        {
    ...
    static void makeFPConstant (SubtractOp comp, Number[] consts, FullConstantVisitor fullv)
    {
        Constant fpc;
        if(consts[0] instanceof Double || consts[1] instanceof Double)
        {
