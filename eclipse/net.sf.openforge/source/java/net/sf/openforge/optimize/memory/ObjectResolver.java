...
/**
 * <code>ObjectResolver</code> maps out the potential flow of pointer
 * values through a user's {@link Design}.  The purpose is to
 * determine which memory {@link Location Locations} each {@link
 * LValue} operation may access.  This information can be used to
 * optimize the number and configuration of the memories in the
 * design.
 * <P>
 * ObjectResolver works by repeatedly visiting the Design and pushing
 * any {@link Pointer} values it finds through the data flows.  These
 * values are traced through {@link Port} and {@link Bus} connections
 * as well as storage into and retrieval from global memory {@link
 * Location Locations}. The initial set of {@link Pointer} values are
 * introduced by the contents of global memory (Pointer initial values
 * of global fields) and any {@link LocationConstant
 * LocationConstants} that appear in the design.  As the design is
 * traversed, various operations may introduce new {@link Pointer}
 * values based upon existing {@link Pointer} values.  Iteration stops
 * when no new accessed {@link Location Locations} are discovered for
 * any {@link LValue} operation.
 * <P>
 * In addition to {@link Location Locations}, ObjectResolver also
 * records for each {@link LValue} which {@link LocationValueSource
 * LocationValueSources}, if any, provide a base address {@link
 * Location} that is used by the {@link LValue}, either directly or
 * through a {@link Location} that was derived from the base address.
 * These {@link LocationValueConstant LocationVaulueConstants} are
 * known as <i>address sources</i>, and the ObjectResolver can be
 * queried for the identity of these.  Typically each such constant
 * operation is constrained to refer to the same {@link LogicalMemory}
 * as any {@link LValue} that references its {@link Location}.
 * <P>
 * As a side effect of analyzing the design, the ObjectResolver will
 * also re-add each {@link LValue} to its {@link LogicalMemoryPort}
 * with the {@link Location Locations} that were found for it.
 *
 * <b>NOTE:</b> It may appear that object resolver is storing a lot of
 * redundant information.  This is primarily because of the fact that
 * Location objects are not necessarily unique objects.  Thus two
 * independent operations (eg an add and a subtract) may actually
 * generate Pointers to the exact same Location object.  For this
 * reason, the Location objects cannot be used to trace back to a
 * source operation and thus this class tracks source operations and
 * address sources in addition to accessed Locations.
 *
...
public class ObjectResolver extends DataFlowVisitor
{
    ...
    /** A map of Location object to a Set of objects that may produce
     * that location.  THIS MAP ONLY CONTAINS LOCATIONS GENERATED AT
     * RUNTIME.  ie, it contains Locations created by any
     * location.createXXX call in this class.  This map is used to
     * detect feedback and prevent infinite looping. */
    private Map locationSourceMap = new HashMap();
    
    /**
     * Creates a new <code>ObjectResolver</code> instance, runs the instance
     * on a given {@link Design}, and then returns the instance for querying
     * of results.
     *
     * @param design the design to be analyzed
     * @return the completed resolver, which can be queried for its results
     * @throws IllegalArgumentException is <code>design</code> is null
     */
     ...
     * by the given access.
     *
     * @param lvalue a valid access in the design
    ...
    /**
     * Gets the address sources for a given memory <code>access</code>.  An address source
     * is an {@link LValue} that provides the base component of the accessed
     * location, and so must be associated with the same memory as the <code>access</code>.
     *
     * @param access a memory access
     * @return a set of {@link LocationValueSource
     * LocationValueSources} representing the address sources for the
     * <code>access</code>
     */
    ...
    protected ObjectResolver (Design design)
    {
        ...
        for (Iterator iter = design.getLogicalMemories().iterator(); iter.hasNext();)
        {
            ...
            for (Iterator allocIter = logicalMemory.getAllocations().iterator(); allocIter.hasNext();)
            {
                final Allocation allocation = (Allocation)allocIter.next();
                allocation.getInitialValue().accept(new Initializer(allocation));
            }
        ...
        for (Iterator iter = design.getRegisters().iterator(); iter.hasNext();)
        {
            ...
            MemoryVisitable init = (MemoryVisitable)reg.getInitialValue();
            init.accept(new RegPtrFinder(reg));
        }
    }

    /**
     * The PtrFinder traverses a LogicalMemory finding all Pointer
     * initial values and then sets each as its own address source.
     * This is correct because all initial values are their own
     * address source, all other Pointers in the LIM come from
     * LocationConstants or are derived from one of these two
     * sources.
     */
    private class PtrFinder extends DefaultMemoryVisitor
    {
        public void visit (Pointer ptr)
        {
            ...
            {
            	EngineThread.getGenericJob().warn("null target of pointer.  Continuing.");
            }
        }
    }
    
    /**
     * Simply used to identify Pointer initial values and add that
     * pointer as the initial value of the Register.
     */
    private class RegPtrFinder extends PtrFinder
    {
        private Register reg;
        public RegPtrFinder (Register reg) { this.reg = reg; }
        public void visit (Pointer ptr)
        {
            super.visit(ptr);
            setValues(this.reg, Collections.singleton(ptr));
            // IDM  new
            ptr.getTarget().getLogicalMemory().addAccessor(ptr);
        }
    }
    ...
    {
        /*
         * Re-add all LValues to their LogicalMemories with the Locations
         * we've found.
         */
        for (Iterator iter = accessedLocationMap.entrySet().iterator(); iter.hasNext();)
        {
            final Map.Entry entry = (Map.Entry)iter.next();
            final LValue access = (LValue)entry.getKey();
            final Set locations = (Set)entry.getValue();
            
            if (debug) System.out.println("Access " + access + " to " + locations);
            ...
            if (!locations.isEmpty())
            {
                 ...
                 */
                final LogicalMemoryPort logicalMemoryPort = access.getLogicalMemoryPort();
...
//                 }
                ...
                /*
                 * Add the access back to its LogicalMemoryPort for all Locations.
                 * Verify that the Locations all refer to the correct LogicalMemory.
                 */
                final LogicalMemory logicalMemory = logicalMemoryPort.getLogicalMemory();
                if (debug) System.out.println("\taccess mem " + logicalMemory);
                for (Iterator locIter = locations.iterator(); locIter.hasNext();)
                {
                    final Location location = (Location)locIter.next();
                    if (debug) System.out.println("\tlocation mem " + location.getLogicalMemory());
    ...
    {
        /*
         * If neither input has an associated Location, then the output will have
         * no Location; likewise, if both inputs have associated Locations, then we
         * have a possible attempt to add two pointers, which will also result in
         * no Locations on the output.
         */
        ...
        {
            final Map inputLocations = leftLocations.isEmpty() ? rightLocations : leftLocations;
            final Value testValue = leftLocations.isEmpty() ? leftValue : rightValue;
            ...
            for (Iterator iter = inputLocations.entrySet().iterator(); iter.hasNext();)
            {
                final Map.Entry entry = (Map.Entry)iter.next();
                ...
                /*
                 * Create the output location with a size of 0, since it
                 * is a pure pointer -- in this context, the number of
                 * bytes being accessed is unknown.
                 */
                if (isConst && !isLoop)
                {
    ...
    public void visit (ArrayRead arrayRead)
    {
        ...
        final Set readLocations = new HashSet();
        for (Iterator iter = baseLocations.iterator(); iter.hasNext();)
        {
            final Location baseLocation = (Location)iter.next();
            final Location readLocation = baseLocation.createIndex(arrayRead.getAccessLocationCount());
        ...
        for (Iterator iter = getAccessedLocations(arrayRead).iterator(); iter.hasNext();)
        {
            final Location readLocation = (Location)iter.next();
    ...
    public void visit (ArrayWrite arrayWrite)
    {
        ...
        {
            final Location baseLocation = (Location)iter.next();
            final Location writeLocation = baseLocation.createIndex(arrayWrite.getAccessLocationCount());
        ...
        for (Iterator iter = getAccessedLocations(arrayWrite).iterator(); iter.hasNext();)
        {
            final Location writeLocation = (Location)iter.next();
    ...
    {
        final boolean isTopLevel = this.topLevelComponents.contains(call);
        ...
        if (!isTopLevel)
        {
            ...
            {
                /*
                 * Explicitly set the locations of each procedure port
                 * to be the set of locations from the corresponding call port.
                 */
                final Port callPort = (Port)portIter.next();
        ...
        if (!isTopLevel)
        {
            ...
            {
                /*
                 * Set the locations of each call bus to be the set
                 * of locations from the corresponding procedure bus.
                 */
                final Bus callBus = (Bus)busIter.next();
    ...
    /**
     * Propagate any values stored in the register to the output bus
    ...
    public void visit (HeapRead heapRead)
    {
        ...
        {
            final Location baseLocation = (Location)iter.next();
            final Location readLocation = baseLocation.createOffset(heapRead.getAccessLocationCount(),
                heapRead.getOffset());
        ...
        final Set readValues = new HashSet();
        for (Iterator iter = getAccessedLocations(heapRead).iterator(); iter.hasNext();)
        {
            final Location readLocation = (Location)iter.next();
            final Set values = getValues(readLocation);
            readValues.addAll(values);
    ...
    public void visit (HeapWrite heapWrite)
    {
        ...
        {
            final Location baseLocation = (Location)iter.next();
            final Location writeLocation = baseLocation.createOffset(heapWrite.getAccessLocationCount(),
                heapWrite.getOffset());
        ...
        for (Iterator iter = getAccessedLocations(heapWrite).iterator(); iter.hasNext();)
        {
            final Location writeLocation = (Location)iter.next();
            addValues(writeLocation, writeValues);
        }
    ...
    {
        for (Iterator iter = inbuf.getOwner().getDataPorts().iterator(); iter.hasNext();)
        {
            final Port port = (Port)iter.next();
            final Set values = new HashSet();

            for (Iterator eiter = inbuf.getOwner().getEntries().iterator(); eiter.hasNext();)
            {
                final Entry entry = (Entry)eiter.next();
                final Collection dependencies = entry.getDependencies(port);

                /*
                 * We have to allow for Entries that have no dependencies for a given Port;
                 * for instance, a Port of a LoopBody may have an initial value but not a
                 * feedback value.
                 */
                assert dependencies.size() <= 1 : ("data port dependencies for a single entry: expected 0 or 1, but was "
                    + dependencies.size() + " port=" + port + " of " + port.getOwner());

                if (!dependencies.isEmpty())
                {
                    final Dependency dependency = (Dependency)dependencies.iterator().next();
    ...
    /**
     * A latch can be a feedback point in the LIM, therefore it has the
     * ability to accumulate LogicalValues on its result bus on
     * successive passes through object resolving.  To account for
     * this, it must be able to set the isModified flag and cause
     * object resolving to iterate again.
     */
    public void visit (Latch latch)
    {
        /*
         * A Latch does not create any new values, so just pass through what comes in.
         */
    ...
    public void visit (LocationConstant locationConstant)
    {
        ...
        if (getValues(locationConstant.getValueBus()).isEmpty())
        {
            ...
            setValues(locationConstant.getValueBus(), Collections.singleton(outputValue));
        }
        locationConstant.getTarget().getLogicalMemory().addLocationConstant(locationConstant);
    }
    ...
    public void visit (NoOp comp)
    {
        ...
        for (Iterator dbIter = exit.getDataBuses().iterator(),
             portIter = comp.getDataPorts().iterator(); dbIter.hasNext();)
        {
    ...
    public void visit (OutBuf outbuf)
    {
        for (Iterator iter = outbuf.getDataPorts().iterator(); iter.hasNext();)
        {
            ...
            {
                for (Iterator eiter = outbuf.getEntries().iterator(); eiter.hasNext();)
                {
                    final Entry entry = (Entry)eiter.next();
                    final Collection dependencies = entry.getDependencies(port);

                    /*
                     * Allow for Entries with no Dependencies here, because it is
                     * possible for control flow to skip an Entry under certain
                     * condition.  For example, in a while(1) loop.
                     */
                    if (!dependencies.isEmpty())
                    {
                        assert dependencies.size() == 1 : ("dependency count for Port/Entry tuple: expected 1, was "
                            + dependencies.size() + "\n" + outbuf + " lineage=" + outbuf.getLineage()
                            + "\npeer=" + outbuf.getPeer()
                            + "\nowner exits=" + outbuf.getOwner().getExits());
    ...
    {
        /*
         * A Reg does not create any new values, so just pass through what comes in.
         */
    ...
    public void visit (Task task)
    {
        ...
        if (thisPort != null)
        {
            ...
            setValues(procPort.getPeer(), Collections.singleton(new Pointer(task.getMemoryKey())));
        }
        super.visit(task);
    }
     ...
     *
     * @param subtractOp a subtract of two pointers, or of a pointer and an integer
     */
    public void visit (SubtractOp subtractOp)
    {
         ...
         * If neither input has an associated Location, then the output will have
         * no Location; likewise, if both inputs have associated Locations, then we
         * have a possible attempt to subtract two pointers, which will also result in
         * no Locations on the output.
         */
    ...
    /**
     * Sets the output values for a given {@link Bus} based upon a set of
     * input locations.  For each input location, a value is produced which
     * represents a pointer to the absolute base of the input location.  This
     * is our way of throwing up our hands when some unknown arithmetic is
     * being applied to the input location.
     *
     * @param inputValues a set of {@link LogicalValue}, representing the input
     *          addresses that are being munged somehow
     * @param outputBus the bus to which the munged address values are being added
     */
    private void addDefaultValues (Set inputValues, Bus outputBus)
    {
        final Set outputValues = new HashSet();
        for (Iterator iter = inputValues.iterator(); iter.hasNext();)
        {
            ...
            {
                final Pointer pointer = new Pointer(inputLocation.getAbsoluteBase());
     ...
     */
    private static Set toLocations (Collection logicalValues)
    {
        final Set locations = new HashSet();
        for (Iterator iter = logicalValues.iterator(); iter.hasNext();)
        {
            final LogicalValue logicalValue = (LogicalValue)iter.next();
    ...
    /**
     * Gets all non-INVALID Locations from a given set of LogicalValues
     * and returns the results as a Map of LogicalValue to Location.
     */
    private static Map toLocationMap (Collection logicalValues)
    {
    ...
    /**
     * Gets the input Bus for a given Port.  If the Port is connected, its connected Bus
     * is returned.  Otherwise, it is expected that the Port's owner will have a single
     * Entry with a single Dependency for the Port from which the Bus can be derived.
     */
    private Bus getInputBus (Port port)
    {
        ...
        {
            final Collection entries = port.getOwner().getEntries();
            assert entries.size() == 1 : "attempt to get single input Bus for Port with multiple Entries";
            final Entry entry = (Entry)entries.iterator().next();

            final Collection dependencies = entry.getDependencies(port);

            assert dependencies.size() == 1 : ("dependencies for data Port of " + port.getOwner()
                + " (lineage: " + port.getOwner().getLineage() + "); expected 1, was "
                + dependencies.size());
    ...
    /**
     * Returns the Set of {@link LogicalValue} objects that have been
     * identified for the specified port, mostly {@link Pointer}
     * objects. 
     *
    ...
    {
        final Set set = (Set)valueMap.get(port);
    ...
    {
        final Set set = (Set)valueMap.get(key);
    ...
    private void setValuesForObject (Object key, Set values)
    {
        if (debug)
        {
            Set oldSet = (Set)valueMap.get(key);
            if (oldSet == null) oldSet = new HashSet();
            else                oldSet = new HashSet(oldSet);
     ...
     * @param source the operation that produces the pointer value
     */
    private void setAddressSource (LogicalValue value, LocationValueSource/*LValue*/ source)
    {
        addressValueSourceMap.put(value, source);
    }

    /**
     * Records the source of a LogicalValue to be the same as that
     * of another LogicalValue.
     *
     * @param value the value whose source is to be derived
     * @param oldValue the value whose source is copied
     */
    private void deriveAddressSource (LogicalValue value, LogicalValue oldValue)
    {
        ...
        else
        {
            ...
            // LocationConstants.
            throw new RuntimeException("Illegal internal state.  Location has no address source");
        }
    }
     ...
     *
     * @param value a pointer value
     * @return the {@link LocationValueSource} that produced the base
     * address of the given LogicalValue.
     */
    ...
    /**
     * Records for a given access the address sources that provide
     * any of a given set of pointer values.
     *
     * @param access a memory access
     * @param addressValues the set of address LogicalValues accessed
    ...
    private void addAddressSources (LValue access, Set addressValues)
    {
        ...
        {
            final LocationValueSource source = getAddressSource((LogicalValue)iter.next());
            if (source != null)
            {
                sources.add(source);
            }
     ...
     *
     * @param access a memory access
     * @param addressValues the set of additional address Locations accessed
     *          by the memory access
     */
    ...
    /**
     * Adds an objects as one source of a given Location.  There may
     * be multiple sources because a Location refers only to a region
     * in memory and may thus be created by multiple operations and
     * the way that Locations are built we may use a 'cached' version.
     *
     * @param loc a value of type 'Location'
     * @param source a value of type 'Object'
     */
    private void defineLocationSource (Location loc, Object source)
    {
        Set sources = (Set)this.locationSourceMap.get(loc);
        if (sources == null)
        {
            sources = new HashSet();
            this.locationSourceMap.put(loc, sources);
        }
        sources.add(source);
    }
    
    /**
     * This method analyses the given Location (test) to determine if
     * it is possible that the specified oject (op) may have been
     * responsible for the creation of test or any of its bases.  This
     * is an indication that the object in question may be part of a
     * feedback loop generating pointers.  This is not, however, a
     * fully definative test for the looping case.  It is guaranteed
     * to return true for all feedback conditions, but it may be
     * possible for it to return a false positive if the test location
     * is one which can also be generated by the operation.
     * <p>It is likely that the first iteration of the ObjectResolver
     * will not identify any feedback locations, but subsequent
     * iterations should find them.
     * <p>This method is used to ensure that we dont end up creating
     * an infinite number of Locations because of constructs like
     * ptr++ in a loop.
     *
     * <p>Testcase: c/language/pointer/VarInitViaSmallerPointer.c
     *
     * @param op an Object which may be responsible for the generation
     * of the test location (eg an AddOp, SubtractOp, etc)
     * @param test a Location to test for a feedback condition.
     * @return a false if there is no way that the given operation can
     * generate the test Location or any of its bases.
     */
    ...
    private class Initializer extends DefaultMemoryVisitor
    {
        ...
        {
            int delta = 0;
            for (Iterator iter = record.getComponentValues().iterator(); iter.hasNext();)
            {
                final LogicalValue nextValue = (LogicalValue)iter.next();
                final Location location = baseLocation.createOffset(nextValue.getSize(), delta);
