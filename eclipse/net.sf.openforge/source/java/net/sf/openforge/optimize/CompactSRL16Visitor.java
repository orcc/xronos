 ...
 * compacts each chain as either SRL16s or SRL16Es. Only the Regs without
 * reset will be compacted and replaced.
 *
 * <p>Created: Thu Oct 22 09:35:36 2002
 * <p>Rewritten: July 06, 2006
 *
 * @author imiller, cwu
...
public class CompactSRL16Visitor extends DefaultVisitor
{
    ...
    /** DOCUMENT ME! */
    private static final String _RCS_ = "$Rev: 204 $";

    private Map<Integer, Set<Reg>> groupableRegs = new HashMap();
    ...
    {
        // Prime the Map
        groupableRegs.put(new Integer(Reg.REG), new HashSet());
        groupableRegs.put(new Integer(Reg.REGE), new HashSet());
    }

    /**
     * DOCUMENT ME!
     *
     * @param design DOCUMENT ME!
     */
    public void visit (Design design)
    {
        ...
        // types of regs.
        Set<List<Reg>> regChains = getChains(groupableRegs.get(new Integer(Reg.REG)));
        Set<List<Reg>> regEChains = getChains(groupableRegs.get(new Integer(Reg.REGE)));
    ...
    public void visit (Reg reg)
    {
        ...
        final Set regOutputTargets = new HashSet();
    ...
    /**
     * Takes a Set of Reg objects and constructs a Set of Lists of Reg
     * objects.  Each List represents a single contiguous chain of
     * registers.  The Lists may contain a single reg, but will never
     * be empty.  All Reg objects in the parameter Set will be
     * contained in exactly one List in the returned Set of Lists.
     */
    private Set<List<Reg>> getChains (Set<Reg> regSet)
    {
        Set<List<Reg>> chains = new HashSet();
        List<Reg> allRegs = new LinkedList(regSet);
        while(!allRegs.isEmpty())
        {
            Reg reg = allRegs.remove(0);
            List<Reg> chain = new LinkedList();
            chain.add(reg);
            boolean added = true;
            while(added)
            {
                added=false;
                Component source = chain.get(0).getDataPort().getBus().getOwner().getOwner();
                if (allRegs.contains(source))
                {
                    chain.add(0, (Reg)source);
                    added=true;
                }
                for (Port port : chain.get(chain.size()-1).getResultBus().getPorts())
                {
    ...
    /**
     * <code>compactRegChain</code> Consumes a Collection of Lists of
     * Reg objects, converting each List to a series of SRL16
     * instantiations based on the type specified (the type matches
     * SRL16.getType()).  
     *
    ...
    private void compactRegChain (Collection<List<Reg>> chainList, int type)
    {
        ...
        {
            OptionInt minLengthOption = (OptionInt)EngineThread.getGenericJob().getOption(OptionRegistry.SRL_COMPACT_LENGTH);
            final int MIN_COMPACT_LENGTH = minLengthOption.getValueAsInt(regList.get(0).getSearchLabel());
            final boolean REGISTER_OUTPUT = !((OptionBoolean)EngineThread.getGenericJob().getOption(OptionRegistry.SRL_NO_OUTPUT_REG)).getValueAsBoolean(regList.get(0).getSearchLabel());
            ...
            for (Reg reg : regList)
            {
                ...
                {
                    throw new IllegalStateException("Unexpected register chain configuration.  All registers in chain are not in the same hierarchy");
                }
            }
            ...
            {
                final int terminal = regList.size() > 16 ? 16:(REGISTER_OUTPUT ? (regList.size()-1):regList.size());
                // List.subList() is <inclusive, exclusive>
                final List<Reg> subList = new LinkedList(regList.subList(0, terminal));
                ...
                else
                {
                    ...
                    srl.getEnablePort().setBus(subList.get(0).getEnablePort().getBus());
                }
                srl.getClockPort().setBus(subList.get(0).getClockPort().getBus());
                srl.getResetPort().setBus(subList.get(0).getResetPort().getBus());
                srl.getGoPort().setBus(subList.get(0).getGoPort().getBus());
                // Connect the input port
                srl.getInDataPort().setBus(subList.get(0).getDataPort().getBus());
                // Connect the SRL result bus to all targets of the
                // original (tail) register.
                for (Port port : subList.get(subList.size()-1).getResultBus().getPorts())
                {
