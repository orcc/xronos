...
public class ModuloOpRule 
{
    ...
    public static boolean halfConstant(ModuloOp op, Number[] consts, ComponentSwapVisitor visit)
    {
        ...
        else if(p2 != null && p2.longValue() == 1)
        {
            ...
            // a % 1 = 0.  Simply delete the component and replace with constant 0
            visit.replaceComponent(op, new SimpleConstant(0, op.getResultBus().getValue().getSize(), op.getResultBus().getValue().isSigned()));
        ...
        {
            if (_optimize.db) _optimize.ln(_optimize.HALF_CONST, "\tRemoving " + op + " due to (a % -1)");
            // a % -1 = 0. Simply delete the component and replace with constnat 0
            visit.replaceComponent(op, new SimpleConstant(0, op.getResultBus().getValue().getSize(), op.getResultBus().getValue().isSigned()));
        ...
        else if (p2 != null && p2.longValue() == 0)
        {
            ...
            EngineThread.getGenericJob().warn("    Found Modulo by 0 at line " + op.getIDSourceInfo().getSourceLine() + ", replacing with 0.");
...
//                  ((Port)decision.getNot().getDataPorts().get(0)).setValue(new Value(1));
                ...
                // True Branch
                //
                Constant true_power = new SimpleConstant((1<<power)-1, argValue.getSize(), argValue.isSigned());
...
//                  Component.getDataBus(trueBranch).setBits(argValue.size());
                ...
                Constant false_power = new SimpleConstant((1<<power)-1, argValue.getSize(), argValue.isSigned());
