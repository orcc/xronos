...
public class MultiClockPipe implements HDLWriter
{
     ...
     * side clock from.
     */
    public MultiClockPipe(int dataPathSize,
                          ...
                          ClockPin readClock)
    {
        ...
        switch(dataPathSize)
        {
            ...
            default:
                throw new
                    net.sf.openforge.forge.api.ForgeApiException("Illegal data path size to MultiClockPipe: " + dataPathSize + ", expected 1, 8, 16, 32, or 64");
        }
    ...
    private void writeBlockRams(PrintWriter pw, int dataSize)
    {
         ...
         *
         * we don't change the rest of the circuit so the number of
         * address bits will always be 9, the data width however will
         * change and is configured by the user during construction of
         * this library element.  We will detect the device family and
         * write out the appropriate block memory.
         *
         * For VirtexII, VirtexIIPro, Spartan3 we'll use a 512 x 32 to
         * build all sizes since it is 512 locations deep and only
         * consumes 1 block.  For 64 bit widths, we'll instantiate
         * two.
         *
         * For all the other families we'll use a 512 x 8.  For 16,
         * 32, and 64 bits widths, we'll instantiate multiple 512 x 8
         * memories.
         *
         */
        switch(RunTime.getFamily())
