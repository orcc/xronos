...
/**
 * An EntryMethod is an entry point into the user's design.  It
 * consists of a target object and a method to be called on that
 * object. EntryMethods are created by calls in the user application
 * to the API method.
 *
 */
public class EntryMethod
{
    ...
    /**
     * Adds a design entry method. If the method signature is not
     * specified, the class is checked to verify that only one method
     * by the given name is present. An error is thrown if the method
     * name isn't present in the class or it is overloaded.
     *
     * @param object, the Object on which the entry method is to be
     * invoked, or null if entry method is static
     * @param clazz the target Class object of the entry method
     * @param methodName the name of the entry method
     * @param arguments the types of the method arguments
     * @param external true if this is an external method, false if it
     * is an AutoStart method (go asserted once on reset)
     *
     * @exception ForgeApiException if the given description is
     * erroneous
     * @exception NoSuchMethodException if the class of the given
     * object has no such method
     */
    protected EntryMethod (Object object, Class clazz, String methodName,
        Class[] arguments, boolean external)
        throws ForgeApiException, NoSuchMethodException
    {
        this.object = object;
        this.clazz = clazz;
        ...
        if (arguments == null)
        {
            ...
            for (int i = 0; i < methods.length; i++)
            {
                if (methods[i].getName().equals(methodName))
                {
                    if (foundName == true)
                    {
                        throw new ForgeApiException("method: "
        ...
        if (!external
                && ((arguments.length > 0)
                || !(method.getReturnType().equals(java.lang.Void.TYPE))))
        {
            throw new ForgeApiException("EntryMethod(" + methodName
    ...
    /**
     * Returns true if this EntryMethod object is equivalent to the
     * given Object.  Equivalence is determined by identical entry
     * method name, class and signature being invoked on the same
     * target (if non static).
     *
     * @param obj an Object to test for equality with this EntryMethod.
     *
    ...
    public boolean equals (Object obj)
    {
        if (obj instanceof EntryMethod)
        {
            ...
            return ((System.identityHashCode(getObject()) == System
                ...
                && (entryMethod.getMethod().equals(getMethod()))
                && (entryMethod.getObjectClass() == getObjectClass()));
        }
    ...
    /**
     * Returns the Object on which this entry method is to be invoked,
     * or null if this entry method is static.
     *
    ...
    /**
     * Gets an array of Class objects identifying the types of each
     * parameter to this entry method.
     *
     * @return an array of Class objects, primitives represented by
     * the Class fields in EntryMethod.
     */
     ...
     *
     * @return true if the method is an external start method, false
     * if autostart (go asserted once on reset)
     */
    ...
    {
        return (external
        ? "External" : "Auto") + "EntryMethod: " + method;
    }
    
    /**
     * Sets the clock domain to use for this method.
     * 
     * @param domain the clock domain to use
     */
    public void setDomain(ClockDomain domain)
    {
        if (domain == net.sf.openforge.forge.api.ipcore.IPCore.AUTOCONNECT)
        {
            throw new ForgeApiException("Cannot use the 'stand in' clock domain on entry methods.  It is only valid for setting the clock domain of an IPCore.");
        }
     ...
     *
     * @param arg the zero-indexed argument position
     *
