 ...
 * contains a 15 deep, programmable width, asynchronous FIFO that
 ...
 * families that implement LUT dual port ram.
 *
...
public class PetiteMultiClockPipe implements HDLWriter
{
    ...
    private static final String IPCORENAME =
        "PetiteMultiClockPipe_ForgeCore";
    ...
    {
        this(32,
             ClockDomain.GLOBAL.getClockPin(),
             ClockDomain.GLOBAL.getClockPin()
             );
    }
     ...
     * of the data path.  Can be any value from 1 through 64.
     ...
     * side clock from.
     */
    public PetiteMultiClockPipe(int dataPathSize,
                          EntryMethod writeSide,
                          EntryMethod readSide)
    {
        this(dataPathSize,
             writeSide.getClockPin(),
             readSide.getClockPin());
    }
                          ...
                          ClockPin readClock)
    {
        ...
        {
            throw new
                net.sf.openforge.forge.api.ForgeApiException("Illegal data path size to PetiteMultiClockPipe: " + dataPathSize + ", expected 1 through 64");
        }
     ...
     * side could appear to go from 3/4 th full to empty in a single
     * clock for example.  Due to the way this value is calculated,
     * care must be taken to not rely absolutely on the value, for
     * example, when writing bursts of 4 words, checking for 3/4 full
     * might cause an overflow, but checking for 2/4 or less and
     * writing 4 words subsequent will always work.  The rule of
     ...
     * 00 - <1/4 full
     * 
     * @return an <code>int</code> value indicating the 1/4 th full
     ...
     * status from the read side could appear to jump from 1/4 th
     * full to 3/4 full in a single clock for example.  Due to the
     * way this value is calculated, care must be taken to not rely
     * absolutely on the value, for  example, when reading bursts of
     * 4 words, checking for 1/4 full might cause an underflow, but
     * checking for 1/2 or more and reading 4 words subsequent will
