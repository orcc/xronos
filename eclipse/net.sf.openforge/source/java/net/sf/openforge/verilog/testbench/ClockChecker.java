...
public class ClockChecker 
{
    ...
    public void stateLogic (Module module, StateMachine mach)
    {
        ...
        final Logical.And simultaneous = new Logical.And(
            mach.getAllGoWire(), mach.getAllDoneWire());
        module.state(new Assign.Continuous(isPassThrough,
                         new Logical.And(ef, new Group(simultaneous))));

        // oldTime = (isPassThrough) ? globalTime : memory[rp];
        // cycles = globalTime - oldTime;
        module.state(new Assign.Continuous(oldTime,
                         new Conditional(
                             new Group(isPassThrough),
                             this.globalTime,
                             new MemoryElement(this.memory, this.readPointer))));
        ...
        ProceduralTimingBlock ptb = new ProceduralTimingBlock(
            ...
            new Assign.NonBlocking(this.globalTime,
                new net.sf.openforge.verilog.model.Math.Add(
                    this.globalTime,
                    new Constant(1, this.globalTime.getWidth())))
            );
    ...
    private void stateFifo (Module module, StateMachine mach, Wire isPassThrough)
    {
        ...
        wpBlock.add(new Assign.NonBlocking(
                        new MemoryElement(this.memory,this.writePointer),
                        this.globalTime));
        ...
        ifBlock.add(new ConditionalStatement(mach.getAllDoneWire(),
                        new Assign.NonBlocking(this.readPointer,
                            new net.sf.openforge.verilog.model.Math.Add(
                                this.readPointer, new Constant(1, this.readPointer.getWidth())
                                ))));
