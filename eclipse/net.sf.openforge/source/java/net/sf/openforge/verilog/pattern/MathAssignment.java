...
/**
 * A MathAssignment is a verilog math operation based on a LIM op which assigns the 
 * result to a wire. 
 * <P>
 *
...
public abstract class MathAssignment extends BinaryOpAssignment
{
    ...
    public static final class AddMulti extends MultiPortOpAssignment
    {
        ...
        protected Expression makeOpExpression (List expressions)
        {
            ...
            while (exprs.size() > 0)
            {
                result = new net.sf.openforge.verilog.model.Math.Add(result, (Expression)exprs.remove(0));
            }
    ...
    public static final class Multiply extends MathAssignment
    {
        ...
        public Multiply(MultiplyOp mul)
        {
            ...
            String baseName = ID.toVerilogIdentifier(ID.showLogical(mul.getResultBus()));
            Net aWire = new SignedWire(baseName+"_a_signed", this.left_operand.getWidth());
            Net bWire = new SignedWire(baseName+"_b_signed", this.right_operand.getWidth());
            
            add(new Assign.Continuous(aWire, left_operand));
            add(new Assign.Continuous(bWire, right_operand));
            add(new Assign.Continuous(result_wire, makeOpExpression(aWire, bWire)));
        ...
        protected Expression makeOpExpression(Expression left, Expression right)
        {
            ...
            // does not support
            return (new net.sf.openforge.verilog.model.Math.Multiply(left, right));
        }

        /**
         * Override the super so that we write out all the nets we
         * generate.
         *
    ...
    public static final class Subtract extends MathAssignment
    {
        ...
        protected Expression makeOpExpression(Expression left, Expression right)
        {
            return (new net.sf.openforge.verilog.model.Math.Subtract(left, right));
        }
    } // class Subtract
