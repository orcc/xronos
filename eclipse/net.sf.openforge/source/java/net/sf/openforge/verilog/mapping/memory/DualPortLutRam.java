...
public abstract class DualPortLutRam extends DualPortRam
{
    ...
    public StatementBlock initialize ()
    {
        ...
        initial_block.add(new InlineComment(" Initialization of Dual Port LUT ram now done through explicit parameter setting.", Comment.SHORT));
    ...
    private void initialize (ModuleInstance instance)
    {
        ...
        for (int w=0; w<getWidth(); w++)
        {
            ...
            for (int index = w; index<(getWidth() * getDepth()); index = index + getWidth())
            {
    ...
    public ModuleInstance instantiate ()
    {
        ...
        for (int i=0; i<getWidth(); i++)
        {
            ...
            if (dataInName[PORTA].length() > 0)
            {
                if (i < dataAttachWidth[PORTA])
                {
                    if (!datScalar[PORTA])
                    {
                        di_exp = dataInA.getRange(dataInStartBit[PORTA] + i, dataInStartBit[PORTA] + i);
                    }
                    else
                    {
        ...
        for (int i = 0; i < getWidth(); i++)
        {
            if (dataOutName[PORTA] == "") // In case port A is a write only port
            {
                ...
                moduleInstance.add(new PortConnection(new Wire("SPO",1), new Wire("",1)));
            }
            else
            {
                ...
                if (i < dataAttachWidth[PORTA])
                {
                    if (!datScalar[PORTA])
                    {
                        spo_exp = dataOutA.getRange(dataOutStartBit[PORTA] + i, dataOutStartBit[PORTA] + i);
                    }
                    else
                    {
        ...
        for (int i = 0; i < getWidth(); i++)
        {
            ...
            if (i < dataAttachWidth[PORTB])
            {
                if (!datScalar[PORTB])
                {
                    dpo_exp = dataOutB.getRange(dataOutStartBit[PORTB] + i, dataOutStartBit[PORTB] + i);
                }
                else
                {
