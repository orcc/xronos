...
public abstract class ExpressionAssignment extends StatementBlock implements ForgePattern
{
...
//             assert (r_bus != null) : "operand port " + r_port + ") in math operation not attached to a bus.";
//             assert (r_bus.getSource().getValue() != null) : "operand port's source bus (" + r_bus.getSource() + ") has no value.";
         ...
         * always return its identifier when lexicalified.  A BusWire won't
         * do this; if it's constant, it will return a representation of the
         * constant value.  Since you can't assign to a constant, this won't
         * work.  Therefore, use the BusWire constructor as a convenience to
         * get the identifier and width of the Bus, but use a raw Wire as the
         * actual left hand side.
         */
        Net busWire = NetFactory.makeNet((Bus)ex.getDataBuses().iterator().next());
        this.resultWire = new Wire(busWire.getIdentifier(), busWire.getWidth());

        if(_pattern.db) _pattern.d.ln("Expression Assignment: "+busWire.getIdentifier());
