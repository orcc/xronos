...
public class GenericTestbenchWriter
{
    ...
    private String getActorName () { return this.actorName; }
    private Design getDesign () { return this.design; }
    /* Returns the name to be used for the instance of the design
     * being tested */
    private String getInstanceName () { return "dut"; }
    ...
    public void genTestbench ()
    {
        ...
        try { hangTimerExpire = Integer.parseInt(gj.getOption(OptionRegistry.HANG_TIMER).getValue(CodeLabel.UNSCOPED).toString(), 10); }
        catch (Exception e) { hangTimerExpire = 1500; }
        ...
        try
        {
            //pw = new IndentWriter(new PrintWriter(new FileWriter(simRoot + "_atb.v")));
        ...
        for (Iterator iter = getDesign().getFifoInterfaces().iterator(); iter.hasNext();)
        {
            ...
            if (o instanceof FifoInput)
                ...
                throw new IllegalArgumentException("Unknown type of fifo interface");
        }
        ...
        pw.inc();
        for (TBIOHandler ioh : handlers) { ioh.writeInstantiation(pw); }
        ...
        for (TBIOHandler ioh : handlers) { ioh.writeInitial(pw); }
        ...
        // port
        for (TBIOHandler ioh : handlers) { ioh.writeTest(pw); }
    ...
    private abstract class TBIOHandler
    {
        ...
        public String getDataName () { return this.name + "_din"; }
        public String getExistsName () { return this.name + "_exists"; }
        public String getCountName () { return getDataName() + "_count"; }
        public String getHandleName () { return this.name + "_id"; }
        public String getVecFileName () { return GenericTestbenchWriter.this.getActorName() + "_" + getName() + ".vec"; }
    ...
    private class OutputHandler extends TBIOHandler
    {
        ...
        public int getDataWidth () { return this.output.getDataPin().getWidth(); }
        public String getDataName () { return getName() + "_dout"; } // override super
        public String getExpectedName () { return this.getDataName() + "_expected"; }
        public String getAckName () { return getName() + "_ack"; }
        private String getSendName ()  { return getName() + "_send"; }
        public String getActivityName () { return getSendName(); }
        ...
        public void writeInitial (IndentWriter pw)
        {
            /*
              O_dout_count <= 0;
              O_id <= $registerVectorFile("MY_X.vec", X_din, X_exists);
              O_exists <= 0;
              O_expected <= 0;
              O_ack <= 1; // For now say we are always asserting ack
            */
...
//             $vectorPop(O_id);
        ...
        public void writeTest (IndentWriter pw)
        {
         ...
         if (O_write) begin
            if (!O_exists) begin
               $fwrite(resultFile, "FAIL: Token output from port O when no output was expected.  Output token %x at count %d\n", O_dout, O_dout_count);
               #100 $finish;
            end
            else if (O_dout !== O_expected) begin
               $fwrite(resultFile, "FAIL: Incorrect result on port O.  output token count %d expected %d found %d\n", O_dout_count, O_expected, O_dout);
               #100 $finish;
            end
            O_dout_count <= O_dout_count + 1;
            $vectorPop(O_id); // May be invalid if beyond the end of the queue
         end // if (O_write)
         else begin
            $vectorPeek(O_id);
         end
    ...
    private class InputHandler extends TBIOHandler
    {
        ...
        public int getDataWidth () { return this.input.getDataPin().getWidth(); }
        public String getAckName () { return getName() + "_read"; }
        public String getActivityName () { return getAckName(); }
        ...
        public void writeInitial (IndentWriter pw)
        {
            /*
              A_din_count <= 0;
              A_id <= $registerVectorFile("MY_A.vec");
              A_exists <= 0;
              A_din <= 0;
             */
            pw.println(getCountName() + " <= 0;");
...
//               $vectorPop(A_id);
        ...
        public void writeTest (IndentWriter pw)
        {
            /*
              if (A_read) begin
                 if (!A_exists) begin
                    $fwrite(resultFile, "FAIL: Illegal read from empty queue, port A on %d token\n", A_din_count);
                    #100 $finish;
                 end
                 $vectorPop(A_id);
              end
