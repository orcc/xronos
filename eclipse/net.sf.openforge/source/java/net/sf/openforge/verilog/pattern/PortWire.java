...
/**
 * A verilog expression which is based on a LIM {@link Port}. The expression
 * may be as simple as a {@link Net} or a {@link Constant}, but could also
 * be a {@link Concatenation} of bit selects and constant values.<P>
 *
    ...
    implements Expression
{
    ...
    /**
     * Constructs a new PortWire in which the Value attached to the
     * Port is compacted (redundant MSB bits trimmed) prior to
     * building the expression for the port.
     *
     * @param port a value of type 'Port'
     * @param compactPort a value of type 'boolean'
     */
     ...
     *
     * @param port  The LIM Port upon which to base the Net
     */
    ...
    /**
     * Gets a group of similar bits from a Value, starting from 
     * a given bit position.
     *
     * @param value the Value which contains the bits
     * @param start the starting bit position for the group
     * @param end the last bit position to include in the group
     */
    private List getBitGroup(Value value, int start, int end)
    {
        List bitgroup = new ArrayList();
        ...
        if (first_bit.isCare())
        {
            ...
            else if (first_bit.getInvertedBit() != null)
            {
                ...
                int inverted_bit_position = first_bit.getInvertedBit().getPosition();
                for (int i=start; (i < value.getSize()) && (i<=end); i++)
                {
                    Bit bit = value.getBit(i);
                    ...
                    if (!bit.isCare()) break;
                    if (bit.isConstant()) break;
                    if (invertedBit == null) break;
                    if (first_bit.getOwner() != bit.getOwner()) break;
                    if (!(bit.getPosition() == bit_position + 1)) break;
                    if (!(invertedBit.getPosition() == inverted_bit_position + 1)) break;
                    bitgroup.add(bit);
                    bit_position++;
            ...
            else
            {
                ...
                for (int i=start; (i < value.getSize()) && (i<=end); i++)
                {
                    ...
                    if (!bit.isCare()) break;
                    if (bit.isConstant()) break;
                    if (bit.getInvertedBit() != null) break;
                    if (first_bit.getOwner() != bit.getOwner()) break;
                    if (!(bit.getPosition() == bit_position + 1)) break;
                    bitgroup.add(bit);
                    bit_position++;
    ...
    public Expression getRange (int msb, int lsb)
    {
        ...
        else
        {
            ...
            List cat_parts = new ArrayList();
            int i=lsb;
            while (i<=msb)
            {
                List bitgroup = getBitGroup(value, i, msb);
                ...
                {
                    assert false : 
                    "Can't handle don't cares mixed in with cares on Port (" + port + ") at index " + i;
                }
                i += bitgroup.size();
            }

            assert cat_parts.size() > 0 : "Must have at least 1 part to the port value " + value.debug();
            if (cat_parts.size() > 1)
            {
...
//         {
//             /*
//              * All the Bits are from the same source.  Just get the BusWire from the source,
...
//             //
//             //
//             //assert(!sourceBus.getValue().isConstant()) : "Source bus in PortWire is constant and Port value is not!";
...
//             expression = (size < sourceBus.getValue().size()) ? fullWire.getRange(msb, lsb) : fullWire;
//         }
        assert (expression.getWidth() == size) : "PortWire constructed expression (" + expression + " " + expression.getWidth() +
    ...
    public Expression getCompactedRange ()
    {
        ...
        assert compactedWidth > 0 : "Illegal compacted wire size: " + compactedWidth;
