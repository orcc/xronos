...
/**
 * TestbenchWriter is the second generation of the test bench
 * writer (as of version 1.33) and builds a Verilog testbench for a
 * given design and collection of TestVectors as supplied by the
 * {@link Tester} attached to the design.  This testbench is a
 * self-verifying testbench which performs the following functions:
 * <ul>
 * <li>Generates data for each input argument of each task.
 * <li>Generates a unique GO signal for each task.
 * <li>Generates a DONE signal for any task with fixed timing and no
 * DONE pin.
 * <li>Samples the DONE and RESULT outputs from each task.
 * <li>Compares the RESULT of each task to the expected value on
 * assertion of DONE.
 * <li>Counts the number of clock cycles between each assertion of GO
 * to a task and the associated DONE.
 * <li>Asserts GO (and next data set) to each task every 'n' cycles as
 * specified by the -atb_gs command line, or upon successful receipt
 * of the previous DONE.
 * </ul>
 * <b>NOTE:</b> that all tasks share the same argument data memories.
 * There are enough memories to cover the task with the most number of
 * arguments.  Any task with fewer arguments simply ignores the extra
 * argument vectors.  Ditto the result memory (it's shared and if a
 * method doesn't have a result it will be ignored).
 *
 * <p>Created: Wed Jan  8 15:44:34 2003
 *
...
public class TestbenchWriter 
{
    ...
    /** The verilog document being populated to generate the
     * testbench. */
    ...
    public TestbenchWriter (Design design)
    {
        ...
        File benchmarkFile = new File(source.getParent(), name+".benchmarks.report");
        */
        ...
        final File clkFile = fileHandler.getFile(TestBenchEngine.DefaultTestBenchEngine.CLKS);
        final File benchmarkFile = fileHandler.getFile(TestBenchEngine.DefaultTestBenchEngine.REPORT);
        ...
        TestInstantiation instance = new TestInstantiation(design, new HashSet(taskHandles.values()));
        ...
        StateMachine machine = new StateMachine(
            new ArrayList(taskHandles.values()), vectors.size(), mems);
        ...
        // 
        InlineComment comment = new InlineComment("initial begin  $dumpfile(\"waves.vcd\");  $dumpvars;end", Comment.SHORT);
    ...
    /**
     * Returns a List of {@link TestVector} objects in the order that
     * they are generated based on the data supplied by the
     * {@link Tester} supplied in the Design.
     *
    ...
    private static VerilogDocument getInitializedDocument (File source)
    {
        ...
        vdoc.append(new Comment("Automatic Test Bench (MTBW).  Generated: " + df.format(new Date()), Comment.SHORT));
        ...
        vdoc.append(new Comment(Comment.BLANK));
        vdoc.append(new Comment(" Some simulators cannot handle the syntax of the new memory models.  This define uses a simpler syntax for the memory models in the unisims library"));
        vdoc.append(new Comment(Comment.BLANK));
    ...
    /**
     * Creates a default testbench for designs with no vectors which
     * states either:
     * FAIL
     * -or-
     * Test Bench Generator created no vectors ...
     *
     * @param design a value of type 'Design'
     * @param resFile a value of type 'File'
