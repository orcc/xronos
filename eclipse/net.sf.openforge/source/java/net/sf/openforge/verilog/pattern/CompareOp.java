 ...
 * 
 * We use signed compare operations for every case. Currently Icarus 
 * simulates signed <= and >= wrong. This is correct verilog code that
 * simulates correctly with modelsim.
 * <P>
 *
...
public abstract class CompareOp extends StatementBlock implements ForgePattern
{
	...
	public CompareOp(BinaryOp bo)
	{
        ...
        String baseName = ID.toVerilogIdentifier(ID.showLogical(bo.getResultBus()));
        int size = 0;
...
//        System.out.println( bo +"Size = "+ size);
	...
	protected abstract Expression makeExpression(Expression left, Expression right);
	...
	public static final class LessThanEqualTo extends CompareOp
	{
		...
		protected Expression makeExpression(Expression left, Expression right)
		{
			    /* due to an icarus bug involving <= and >= we
			     * change these operations from
			     * a <= b to b > a
			     */
