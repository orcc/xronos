 ...
 *
 *
 * <p>Created: Thu Mar 17 11:25:18 2005
 *
...
public class BlockIOReflection 
{
    ...
    public BlockIOReflection (BlockDescriptor ibd, BlockDescriptor obd)
    {
        ...
        this.targetFile = gj.getFileHandler().getFile(TestBenchEngine.BlockIOTestBenchEngine.REFLECT);
        ...
        {
            gj.fatalError("Could not create writer for reflect file. " + ioe.getMessage());
        }
     ...
     * @param userTestResults is true if the user has supplied input parameter
     * generators
     * @param rootFile is the root of the destination file
     * @param targetFile is the .c file to generate
     */
...
//             pw.println("/* \ninput block format: \n"+ibd+"\n\noutput format: "+obd+"\n*/");
            ...
            pw.println(pad(space)+getInFifoType()+" ATB_iBlockArray["+iBlockSize+"];");
            pw.println(pad(space));
            pw.println(pad(space)+"/* space for the output block + 1 word for the data valid flag */");
            pw.println(pad(space)+getOutFifoType()+" ATB_oBlockArray["+(oBlockSize+1)+"];");
            pw.println(pad(space));
            pw.println(pad(space)+"int main(int argc, char * argv[])");
            pw.println(pad(space)+"{");
            space+=4;
            pw.println(pad(space));
            File inputVec = new File(getReflectFile().getParent(), this.baseName + "_input_blocks.vec");
            File outputVec = new File(getReflectFile().getParent(), this.baseName + "_output_expected.vec");
            pw.println(pad(space)+"FILE *ATBinput=fopen(\""+escapeWindowsSlash(inputVec.getAbsolutePath())+"\",\"w\");");
            pw.println(pad(space)+"FILE *ATBoutput=fopen(\""+escapeWindowsSlash(outputVec.getAbsolutePath())+"\",\"w\");");
            pw.println(pad(space));
            pw.println(pad(space)+"int ATB_i, ATB_j, ATB_num_vectors=0;");
            pw.println(pad(space)+"int ATB_skip;              /* used to handle */");
            pw.println(pad(space)+"int ATB_longjump_return;   /* math exceptions */");
            ...
            if (userTestResults)
            {
                pw.println(pad(space)+"for (ATB_i=0; ATB_i<ATB_"+getFunctionName()+"_numTests(); ATB_i++)");
                pw.println(pad(space)+"{");
                space+=4;
                // declare variables for each parameter in the input block
                for (int i=0; i<ielements.length; i++)
                {
                    String declaration=ielements[i].getDeclaredType().getDeclaration("ATB_arg_"+ielements[i].getFormalName()).trim();
                    // change array declaration to pointer
                    if (declaration.endsWith("]"))
                    {
                        declaration=ielements[i].getDeclaredType().getDeclaration("").trim().split("\\[")[0].trim()+" * "
                                    +"ATB_arg_"+ielements[i].getFormalName();
                    }
                    pw.println(pad(space)+declaration+"=ATB_"+getFunctionName()
                               +"_arg_"+ielements[i].getFormalName()+"(ATB_i);");
                }
                if (returnIndex != -1)
                {
                    pw.println(pad(space)+"/* declare the function return for use later */");
                    pw.println(pad(space)+getFunctionDeclaredType().getDeclaration("ATB_return")+";");
                }
                ...
                pw.println(pad(space)+"/* declare char * for each parameter so we can extract arbitrary bytes */");
                for (int i=0; i<ielements.length; i++)
                {   // if we are referencing a pointer then we don't wan't to dereference
                    boolean pointer=(ielements[i].getDeclaredType().getDeclaration("").trim().endsWith("*") ||
                                     ielements[i].getDeclaredType().getDeclaration("").trim().endsWith("]"));

                    pw.println(pad(space)+"char * ATB_"+ielements[i].getFormalName()
                               +"Ptr=(char *)"
                ...
                pw.println(pad(space)+"char * ATB_blockArrayPtr=(char *) ATB_iBlockArray;");
                if (returnIndex != -1)
                {
                    pw.println(pad(space)+"/* declare char * for the result so we can extract arbitrary bytes */");
                    pw.println(pad(space)+"char * ATB_returnPtr=(char *)"
                               +(oelements[returnIndex].getDeclaredType().getDeclaration("").trim().endsWith("*")?" ":" &")
                               +"ATB_return;");
                }
                // now assemble the blockArray byte by byte according to the BlockDescriptor
                ...
                pw.println(pad(space)+"/* first save the current stack in case of an exception */");
                pw.println(pad(space)+"ATB_skip=0;");
                pw.println(pad(space)+"if ((ATB_longjump_return = setjmp(ATB_returnStack)) != 0)");
                pw.println(pad(space)+"{");
                space+=4;
                ...
                pw.println(pad(space)+"ATB_blockArrayPtr=(char *) ATB_oBlockArray;");
            ...
            {   
                if (userTestResults)
                {
                    pw.println(pad(space)+"if (ATB_"+getFunctionName()+"_doGeneric())");
                    pw.println(pad(space)+"{");
                    space+=4;
                }
                // generic tests case 1: > MAX_VECTORS and case 3 - no input args
                if (numPrimitiveVectors > MAX_VECTORS || (numPrimitiveVectors > 0 && ielements.length == 0))
                {
                    numPrimitiveVectors=MAX_VECTORS;
                    pw.println(pad(space)+"for (ATB_i=0; ATB_i<"+numPrimitiveVectors+"; ATB_i++)\n"+pad(space)+"{");
                    space+=4;
                    // declare variables for each parameter in the input block
                    for (int i=0; i<ielements.length; i++)
                    {
                        pw.println(pad(space)+ielements[i].getDeclaredType().getDeclaration("ATB_arg_"+ielements[i].getFormalName())+"=ATB_"+getFunctionName()+"_generate_"+ielements[i].getFormalName()+"(ATB_i);");
                    }
                }
                ...
                else if (numPrimitiveVectors < MAX_VECTORS && ielements.length > 0)
                {
                    ...
                    for (int i=1; i <= ielements.length; i++)
                    {
                        pw.println(pad(space)+"for ("+variableToElement[i-1]
                                   +"=0; "+variableToElement[i-1]+"< ATB_"+function+"_generate_"+e.getFormalName()
                                   +"_length; "+variableToElement[i-1]+"++)");
                        pw.println(pad(space)+"{");
                        space+=4;
                        pw.println(pad(space)+e.getDeclaredType().getDeclaration("ATB_arg_"+e.getFormalName())+"=ATB_"
                                   +function+"_generate_"+e.getFormalName()+"(ATB_i);");
                ...
                // now close the loops
                if (numPrimitiveVectors < MAX_VECTORS && ielements.length > 0)// case 2
                {
                    for (int i=0; i<ielements.length; i++)
                    {
                        space-=4;
            ...
            pw.println(pad(space)+"/* data/pad flags - ff for each valid byte or 00 for each pad byte in the block */");
    ...
    public int executeBlockIOReflectFile ()
    {
        ...
        File outFile = new File(inFile.getParent(), sourceName.substring(0, sourceName.lastIndexOf(".")));
        File[] inFiles={inFile};
        ...
        {
            EngineThread.getEngine().fatalError("Reflect execution return with non zero exit code");
        }
        int count=-1;
        ...
        {
            EngineThread.getEngine().fatalError("Could not parse the number of vector returned by "+inFile+": "+baos.toString()+": "+nfe);
        }
...
//         pw.println("#endif");
    ...
    */
    private int printPrimitiveGenerators(PrintWriter pw, BlockElement[] ielements)
    {
        ...
        for (int i=0; i<ielements.length; i++)
        {
            String type=ielements[i].getDeclaredType().getDeclaration("").trim();
            ...
            {
                vectors[i]=testVector(ielements[i].getAllocatedSize(), false /* don't support floats yet */);
            }
            else
            {
                EngineThread.getGenericJob().info("can't generate auto vectors for parameter type: "+type);
                numVectors=0;
                return numVectors;
            }
        }
        ...
        for (int i=0; i<ielements.length; i++)
        {
            ...
            pw.println("\n"+pad(space)+"/* auto generate values of type "+e.getDeclaredType().getDeclaration("")+" */");

            pw.println(pad(space)+"#define ATB_"+function+"_generate_"+e.getFormalName()+"_length "+vectors[i].length);
            pw.println(pad(space)+ielements[i].getDeclaredType().getDeclaration("")
                       +" ATB_"+function+"_generate_"+e.getFormalName()+" (int sequence)\n"+pad(space)+"{");
            space+=4;
            pw.println(pad(space)+ielements[i].getDeclaredType().getDeclaration("")+" r[]=\n"+pad(space)+"{");
            space+=4;
            for (int j=0; j<vectors[i].length; j++)
            {
                ...
                {
                    case 1:
                        pw.println(pad(space)+"0x"+Integer.toHexString(0xff & vectors[i][j].byteValue())+",");
                        break;
                    case 2:
                        pw.println(pad(space)+"0x"+Integer.toHexString(0xffff & vectors[i][j].shortValue())+",");
                        break;
                    case 4:
                        pw.println(pad(space)+"0x"+Integer.toHexString(vectors[i][j].intValue())+",");
                        break;
                    case 8:
                        pw.println(pad(space)+"0x"+Long.toHexString(vectors[i][j].longValue())+"LL,");
                        break;
                }
            }
            space-=4;
            pw.println(pad(space)+"};");
            pw.println("\n"+pad(space)+"return(r[sequence % ATB_"+function+"_generate_"+e.getFormalName()+"_length]);");
            space-=4;
            pw.println(pad(space)+"}");
        }
    ...
    /**
     * return the length of the test vectors for the given bit width
     * floating is true for floating point, false for integer 
     */
    ...
    private String getFIFOType (int blockWidth)
    {
        ...
        switch(blockWidth)
        {
            case 1: result="char"; break;
            case 2: result="short"; break;
            case 4: result="int"; break;
            case 8: result="long long"; break;
            default: 
                result="UNKNOWN";
                EngineThread.getEngine().fatalError("Non supported fifo width in generateBlockIOReflectFile: "+blockWidth);
        }
        return result;
    }
     ...
     * @param dataValid true if dataValid to be written, and false if not
     * @param space is the number of leading spaces to print
     */
    private String writeBlockMapping (PrintWriter pw, BlockDescriptor bd, boolean dataValid, int space)
    {
        ...
        for (int i=0; i<blockSize; i++)
        {
            ...
            pw.println(pad(space)+"/* begin block element "+i+" ("+elements[element].getFormalName()+") */");
            /*
             * this makes the code really ugly...  
             * the stream format (data or pad) is endian agnostic, but the
             *  method of choosing data elements with char * is endian
             *  dependent....  so if the target that we are running the _reflect
             *  file is big endian, we need to reverse the offsets to the
             *  ATB_*Ptr[] within the block.
             *
                ...
                {                    
                    if (streamFormat[streamOffsets[element]] == BlockElement.DATA)
                    {
                        pw.println(pad(space)+"ATB_blockArrayPtr["+(index)
                                   +"]=ATB_"+elements[element].getFormalName()+"Ptr["+ptrOffsets[element]+"];");
                    ...
                    {           // if its a PAD then we don't need to write
                                // anything out
                        index++;
                        blockPadInfo.insert(0,"00");
                        throw new net.sf.openforge.app.ForgeFatalException("found pad where only data should be in ATB generation");
                    }
                    padIndex++;
            ...
            else
            {
                ...
                {                    
                    if (streamFormat[streamOffsets[element]] == BlockElement.DATA)
                    {
                        ...
                        pw.println(pad(space)+"ATB_blockArrayPtr["+(base+(blockWidth-offset-1))
                                   +"]=ATB_"+elements[element].getFormalName()+"Ptr["+ptrOffsets[element]+"];");
        ...
        String[] printfType={"ERR","02hhx","04hx","ERR","08x","ERR","ERR","ERR","016llx"};
        String fileStar=(bd.isSlave()?"ATBinput":"ATBoutput");
        
        pw.println(pad(space)+"fprintf("+fileStar+", \"// block for iteration %d\\n\",ATB_i);");
        if (dataValid)
        {
            pw.println(pad(space)+"fprintf("+fileStar+", \"%"+printfType[blockWidth]+"\\n\", ATB_valid);");
        }
        pw.println(pad(space)+"for (ATB_j=0; ATB_j<"+blockSize+"; ATB_j++)");
        pw.println(pad(space)+"{");
        space+=4;
        // print to either input or output FILE*
        pw.println(pad(space)+"fprintf("+fileStar+", \"%"+printfType[blockWidth]+"\\n\",forceToLittleEndian"
                   +(bd.isSlave()?"Input(ATB_i":"Output(ATB_o")+"BlockArray[ATB_j]));");
        space-=4;
        pw.println(pad(space)+"}");
    ...
    private void gcc (File outputFile, File[] srcfiles)
    {
        ...
        try
        {
            if (gcc.exec(args, System.out, System.err) != 0)
            {
                ...
                EngineThread.getEngine().fatalError("GCC returned non zero exit value for: gcc "
                    + buf.toString() + "; can't complete test bench generation: ");
            }
        }
        catch(ExecutionException ioe)
        {
            EngineThread.getEngine().fatalError("IOError during gcc execution: " +
                           ioe.getMessage());
        }
        catch(IOException ioe)
        {
    ...
    private int exec (String commandLine, PrintWriter out)
    {
        ...
        {
            EngineThread.getEngine().fatalError("IOError during reflect execution: " + ioe.getMessage());
        }
        ...
        {
            process.destroy();
            EngineThread.getEngine().fatalError("Thread interrupted during refelct execution: ");
        }
        catch (IOException ioe)
        {
            EngineThread.getEngine().fatalError("IOError during reflect execution: " + ioe.getMessage());
    ...
    public static final boolean[] BOOLEAN_TV = 
    {
        true, false
    };

    public static final byte[] BYTE_TV = 
    {
        Byte.MIN_VALUE,
        Byte.MIN_VALUE + 1,
        Byte.MAX_VALUE,
    ...
    public static final char[] CHAR_TV = 
    {
        Character.MIN_VALUE,
    ...
    public static final short[] SHORT_TV = 
    {
        Short.MIN_VALUE,
    ...
    public static final int[] INT_TV = 
    {
        Integer.MIN_VALUE,
        Integer.MIN_VALUE + 1 ,
        Integer.MAX_VALUE,
    ...
    public static final long[] LONG_TV = 
    {
        Long.MIN_VALUE,
        Long.MIN_VALUE + 1,
        Long.MAX_VALUE,
    ...
    public static final float[] FLOAT_TV = 
    {
        Float.MIN_VALUE,
        (float)(Float.MIN_VALUE + (float)1.0),
        Float.MAX_VALUE,
        (float)(Float.MAX_VALUE - (float)1.0), 
        (float)-2.0,
        (float)-1.0,
        (float)0.0,
        (float)1.0,
    ...
    public static final double[] DOUBLE_TV = 
    {
        Double.MIN_VALUE,
