    ...
    implements MappedModuleSpecifier
{
    ...
    /**
     * Construct a ProcedureModule based on a {@link Procedure}.
     * The output Buses and peer-Buses of input Ports will be
     * added to the defined Bus-Net map.
     *
     * @param proc the Procedure which is being instantiated
     */
    ...
    public void defineInterface()
    {
        ...
        for (Iterator mod_ports = body.getPorts().iterator(); mod_ports.hasNext();)
        {
            addInput((Port)mod_ports.next());
        }
        
        // define output ports (based on the body's buses)
        for (Iterator mod_buses = body.getBuses().iterator(); mod_buses.hasNext();)
        {
